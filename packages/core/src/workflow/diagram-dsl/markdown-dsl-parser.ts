...
const listValue = (fields: Fields, key: string): readonly string[] => {
  const list = fields.lists.get(key);
  if (list) {
    return list;
  }
  const scalar = fields.scalars.get(key)?.trim();
...
const parseModule = (
  ...
  if (id !== block.id) {
    return {
      ...
      line: block.line,
      message: "Module header Id must match field Id",
    };
  }
  if (
    !(
      isOneOf(kind, MODULE_KINDS) &&
      isOneOf(origin, ENTITY_ORIGINS) &&
      isOneOf(status, ENTITY_STATUSES)
    )
  ) {
    return {
      code: "invalid-metadata",
      line: block.line,
      message: `Invalid module enum value for ${id}`,
    };
  }
...
const parseRelation = (
  ...
  if (id !== block.id) {
    return {
      code: "invalid-entity-id",
      line: block.line,
      message: "Relation header Id must match field Id",
    };
  }
  ...
  if (
    ...
    return {
      code: "invalid-metadata",
      line: block.line,
      message: `Invalid relation enum value for ${id}`,
    };
  }
...
export const parseModuleMapDsl = (content: string): MarkdownDslParseResult => {
  ...
  if (!(title && TITLE_RE.test(title.text.trim()))) {
    return buildParseFailure(
      "invalid-title",
      title?.number ?? 1,
