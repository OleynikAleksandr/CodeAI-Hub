...
const EMBEDDED_SOURCE_NO_REREAD_PATTERN =
...
const TRUNCATED_FALLBACK_PATH_PATTERN =
  /Fallback path: \.codeai-hub\/demo\/virtual_simulation\/virtual-simulation\.md/;
...
test("NodeFirstMessageBuilder includes scoped workflow artifacts as prior context", () => {
  ...
  assert.match(result.content, EMBEDDED_SOURCE_NO_REREAD_PATTERN);
  ...
  assert.doesNotMatch(result.content, PRODUCT_PART_ARTIFACT_PATH_PATTERN);
...
test("NodeFirstMessageBuilder keeps truncated excerpts inside the no-read draft boundary", () => {
  ...
  assert.match(result.content, TRUNCATED_FALLBACK_PATH_PATTERN);
