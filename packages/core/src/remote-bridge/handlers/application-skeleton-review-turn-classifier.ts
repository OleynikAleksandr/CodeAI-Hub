...
// Phase 1B revision-vs-discussion classifier.
//
// In Phase 1B the user leads the conversation and the agent answers without
...
//     (Core's Phase 1B structural guard validates and the per-revision
...
// Phases other than `phase_1b_review` return `out_of_scope` because their
// arbitration is owned by separate microtasks (Phase 1A guard, Phase 2 gate).
...
export const classifyApplicationSkeletonReviewTurn = (
  ...
  if (input.phase !== "phase_1b_review") {
    return "out_of_scope";
  }
