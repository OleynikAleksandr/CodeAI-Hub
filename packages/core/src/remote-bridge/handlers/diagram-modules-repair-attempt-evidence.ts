...
const ATTEMPT_DIR = "workflow/revisions/diagram-modules/attempts";
const REPAIR_NUMBER_RE = /repair(\d+)\.task\d+$/u;
const SAFE_SEGMENT_RE = /[^a-z0-9._-]+/gu;
const ATTEMPT_FILE_RE = /^attempt-(\d{4})-/u;

export type DiagramModulesRepairAttemptOutcome =
  | "accepted_after_repair"
  | "no_accepted_diff"
  | "still_invalid";

export interface DiagramModulesRepairAttemptEvidence {
  readonly attemptNumber: number;
  readonly checkedAt: string;
  readonly diagnostics: readonly string[];
  readonly outcome: DiagramModulesRepairAttemptOutcome;
  readonly repairTaskId: string;
  readonly schema: "codeai-diagram-modules-repair-attempt-v1";
  readonly stage: "diagram_modules";
  readonly targetArtifactPath: string;
  readonly validator: string;
}

export interface DiagramModulesRepairAttemptEvidenceResult {
  readonly absolutePath: string;
  readonly evidence: DiagramModulesRepairAttemptEvidence;
...
export const writeDiagramModulesRepairAttemptEvidence = async (params: {
  readonly attemptNumber?: number | null;
  readonly diagnostics: readonly string[];
  readonly now?: Date;
  readonly outcome: DiagramModulesRepairAttemptOutcome;
...
}): Promise<DiagramModulesRepairAttemptEvidenceResult> => {
  ...
  const evidence: DiagramModulesRepairAttemptEvidence = {
    schema: "codeai-diagram-modules-repair-attempt-v1",
