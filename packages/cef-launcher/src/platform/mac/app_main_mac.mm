...
int main(int argc, char* argv[]) {
  ...
  @autoreleasepool {
    [CodeAIHubApplication sharedApplication];
    if (![NSApp isKindOfClass:[CodeAIHubApplication class]]) {
      fprintf(stderr,
              "CodeAIHubLauncher: NSApp is not a CodeAIHubApplication\n");
      return 1;
    }
    ...
    if ([bundleResources length] > 0U) {
      const char* bundle_resources_fs_path =
          [bundleResources fileSystemRepresentation];
    ...
    if ([frameworksDir length] > 0U) {
      ...
      if ([[NSFileManager defaultManager] fileExistsAtPath:frameworkResources]) {
        ...
        if (framework_res_path != nullptr) {
          ...
          if ([[NSFileManager defaultManager] fileExistsAtPath:icuDataPath]) {
            const char* icu_fs_path = [icuDataPath fileSystemRepresentation];
      ...
      if ([[NSFileManager defaultManager] fileExistsAtPath:helperExecutable]) {
        const char* helper_fs_path = [helperExecutable fileSystemRepresentation];
    ...
    CodeAIHubAppDelegate* delegate = [[CodeAIHubAppDelegate alloc] init];
