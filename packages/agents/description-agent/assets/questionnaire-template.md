...
<small><i>Если вы уже представляете, из каких больших частей будет состоять система — перечислите их с коротким пояснением. Здесь не нужны классы и низкоуровневая техника. Нужны именно крупные понятные блоки, из которых потом можно будет собрать архитектуру и диаграмму. Пример: «Клиентский интерфейс», «Серверная часть», «База данных», «Интеграция с внешним API», «Админ-панель», «Фоновый обработчик». Если не знаете — оставьте пустым, агент поможет определить структуру позже.</i></small>
