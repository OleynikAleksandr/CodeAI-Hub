...
### 2.1) Язык итоговых user-facing staged artifacts
- runtime может отдельно прислать явную инструкцию с языком для `Artifacts for the User`;
- если такая инструкция пришла, user-facing значения внутри `product-parts.index.md` и `product-parts/<part-id>.md` должны быть написаны именно на этом языке;
- required DSL markers, headers, field names, ids, staged status tokens и прочие contract-bound элементы оставляй ровно в том виде, который требует staged contract;
- внутренние инструкции этого prompt-а не нужно переписывать под язык артефакта;
- если runtime не прислал отдельную языковую инструкцию, используй язык текущего пользовательского диалога для user-facing значений внутри staged artifacts.
