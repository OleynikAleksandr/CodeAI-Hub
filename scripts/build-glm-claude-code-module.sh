...
INSTALL_ROOT="$HOME/.codeai-hub/providers/glm-claude-code"
RELEASE_ROOT="$HOME/.codeai-hub/releases"
MANIFEST_PATH="$REPO_ROOT/assets/providers/glm-claude-code/manifest.json"

usage() {
  cat <<USAGE
GLM-Claude-Code Module build script (developer mode)
Usage: ./scripts/build-glm-claude-code-module.sh [--version <semver>] [--clean]
USAGE
}
...
echo "📦 Building GLM-Claude-Code module v$MODULE_VERSION"
...
ARCHIVE_NAME="glm-claude-code-module-$MODULE_VERSION.tar.bz2"
ARCHIVE_PATH="$RELEASE_ROOT/$ARCHIVE_NAME"
mkdir -p "$RELEASE_ROOT"
(cd "$STAGE_DIR" && tar -cjf "$ARCHIVE_PATH" .)
find "$RELEASE_ROOT" -maxdepth 1 -type f -name "glm-claude-code-module-*.tar.bz2" ! -name "$ARCHIVE_NAME" -exec rm -f {} +
