...
- **Sticky keepalive & restart flow**: `CoreKeepAlive` (новый модуль поверх `CoreProcessManager`) держит скрытое WebSocket-подключение к `ws://<host>:<port>/api/v1/stream`, чтобы ядро не уходило в idle shutdown, даже если webview скрыто или окно VS Code в фоне. При любом `child.on("exit")` или обрыве соединения keepalive инициирует `ensureStarted()` и переподключение. `HomeViewProvider` дополнительно вызывает `ensureStarted()` при каждом `resolveWebviewView`/`showSettings`, а команда `codeaiHub.launchWebClient` прогревает ядро перед запуском CEF, поэтому разворачивание любого UI повторно активирует core.
