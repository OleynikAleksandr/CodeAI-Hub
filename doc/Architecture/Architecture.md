...
- **Logging**: лаунчер пишет ротационный лог в `~/.codeai-hub/logs/launcher/launcher.log`, а ядро — JSON-журнал в `~/.codeai-hub/logs/core/core.log` (путь прокидывается через `CODEAI_CORE_LOG_FILE`). Провайдеры Claude/Codex/Gemini ведут потоковые jsonl-журналы по шаблону `~/.codeai-hub/logs/<provider>/<provider>-<sessionId>.jsonl`, создавая файл только после получения реального идентификатора сессии.
