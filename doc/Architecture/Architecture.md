...
- **status-block-module** — индикаторы статуса (кредиты, тип провайдера, состояние подключения Remote Bridge); с 1.0.24 выровнен кегль с TODO-блоком (11 px) для единообразия шрифтов.
