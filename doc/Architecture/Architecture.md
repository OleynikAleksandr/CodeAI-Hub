...
**Release Focus:** v1.1.374 — Idea Collector: поддержка прикрепления существующих документов воркспейса (UI `/read`) и чтение через Core endpoint; `приложение`/`кластер` как first-class типы идеи и правило Flow для multi-module (Spec/Plan по модулям).
...
- **Idea Collector artifacts (v1.1.374)**: schema читается из `~/.codeai-hub/templates/full-development-flow/idea/idea-collector-schema.json`, а артефакты пишутся в `.codeai-hub/full-development-flow/idea/` (`idea.md`, `virtual-simulation.md`).
