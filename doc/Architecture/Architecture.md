...
**Release Focus:** v1.1.325 — Runtime Installation Fixes. Исправлена поддержка `file://` URLs для локальной установки Core/Launcher, правильная распаковка Core архива с обработкой вложенных директорий, синхронизация формата `install.json` между build-скриптами и TypeScript типами. Core теперь устанавливается при `build-all.sh`, что ускоряет первый запуск расширения.
...
- **Core Bootstrap (v1.1.325 improvements)**: при старте расширение проверяет наличие автономного ядра и вспомогательных компонентов. Во всех текущих dev-сборках и внутренних релизах manifests (`assets/core/manifest.json`, `assets/providers/gemini/manifest.json`, `assets/ui/manifest.json` и др.) указывают на локальный cache `file://$HOME/.codeai-hub/releases/…` (на основной dev‑машине — `file:///Users/oleksandroliinyk/.codeai-hub/releases/`), поэтому установка и запуск полностью офлайн и не зависят от GitHub Releases. **Runtime installation flow**: скрипт `build-all.sh` теперь устанавливает Core в финальное место (`~/.codeai-hub/core/<platform>/<version>/`) сразу после сборки, а не откладывает распаковку на первый запуск расширения. Модуль `runtime-files.ts` поддерживает `file://` URLs: при обнаружении `file://` протокола функция `downloadFile()` напрямую копирует файл из локального кэша вместо HTTP-запроса, что ускоряет dev-итерации. Core installer (`core-installer.ts`) использует temp-extract-rename паттерн (аналогично Launcher): распаковывает архив во временную директорию, читает первый элемент (вложенную папку с версией), переименовывает её в финальный `runtimeDir` — это корректно обрабатывает структуру Core tarball'а (`1.1.325/app/`, `1.1.325/node/`). Формат `install.json`, создаваемый `build-core.sh`, теперь синхронизирован с TypeScript типом `InstallMarker`: поле `coreVersion` (вместо `version`) и `package` с именем архива; это позволяет функции `verifyExistingCoreInstall()` корректно распознавать установленный Core и не запускать повторную распаковку при каждом старте VS Code. Core Supervisor (`@codeai-hub/core-supervisor`, CLI `codeai-core`) выбирает установленный runtime и запускает ядро с нужным окружением (`CORE_HOST/CORE_PORT/CORE_MANAGED_MODE`, `*_WORKSPACE_PATH`, `*_MODULE_PATH`). Исторически предполагалось, что для публичных релизов `baseUrl` в манифестах будет переключаться на GitHub Releases, но на текущем этапе разработки это намеренно не используется и будет возвращено в отдельной фазе, когда появится внешняя инфраструктура релизов. Инсталляторы используют общие хелперы `runtime-files` и `launcher-install-helpers`, которые переиспользуют локальные кеши, валидируют контрольные суммы (SHA-1) и удерживают файлы в пределах архитектурного лимита. Менеджеры (VS Code и launcher) разделяют "ensure" и "attach" потоки: `CoreProcessManager.attachToRunningCore()` подключается к уже запущенному orchestrator'у, если версия совпадает, не трогая провайдерные директории, а `ensureStarted()` через Supervisor стартует новое ядро только при отсутствии или устаревании текущего экземпляра; ядро живёт до явного `shutdown` или истечения TTL.
