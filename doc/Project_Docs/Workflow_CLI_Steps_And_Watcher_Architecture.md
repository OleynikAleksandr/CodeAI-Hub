...
## 1) Проблема
Текущий Workflow для стадий (Description / Virtual Simulation / Diagrams) опирается на `structured output` и `artifact-upsert` через `artifacts[]`.
...
- Core/UI формирует **один стартовый промпт**, куда включены:
  ...
  - содержимое анкеты/входных файлов (inline, как attachments);
  - содержимое шаблона артефакта (inline);
