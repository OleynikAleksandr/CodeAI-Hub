...
- Манифесты обновлены: VSIX раздаёт `gemini-module-0.1.3.tar.bz2` и `codeai-hub-core-darwin-arm64-0.2.10.tar.bz2`, что устраняет рассинхрон артефактов.

## Запланированные изменения 1.1.33 (2025-10-29)
- `packages/Gemini_Module` будет переведён на ESM и прямую работу с `@google/gemini-cli-core`, что устранит зависимость от долгоживущего процесса `gemini -o json` и унифицирует провайдерный стек.
- Инсталлятор и пайплайн сборки Gemini будут обновлены для поставки ESM-бандла и необходимых `node_modules` в `~/.codeai-hub/providers/gemini/<version>/`, включая сценарии для загрузки `@google/gemini-cli-core`.
- Потребуется повторная валидация RemoteBridge и UI стриминга, фиксация результатов в Telemetry checklist и документации по архитектуре.

## Обновления релиза 1.1.31 (2025-10-28)
- Gemini-модуль пересобран как `0.1.2`: инсталлятор проверяет несколько credential-файлов, предупреждает об их отсутствии и больше не блокирует запуск ядра.
- Core orchestrator обновлён до `0.2.9` — в snapshot включены свежие артефакты провайдеров, а манифесты указывают на `codeai-hub-core-darwin-arm64-0.2.9.tar.bz2`.
- Release-артефакты синхронизированы: `assets/providers/gemini/manifest.json` и `assets/core/manifest.json` указывают на новые SHA-1/размеры, что исключает установку устаревших пакетов.
...
- **Запуск CLI без pkg**: `SDKInstaller` вычисляет глобальный npm prefix и находит бинарь `claude`. `ClaudeSDKManager` передаёт путь в SDK (параметр `pathToClaudeCodeExecutable`), поэтому Claude Code запускается через стандартный shebang Node 20, а не через встроенный в pkg Node 18. Ошибка `ERR_REQUIRE_ESM` устранена.
- **Единый slug для проектов**: `claudeProjectSlug` больше не удаляет ведущий дефис, благодаря чему SDK использует каталоги `~/.claude/projects/-<slug>`, созданные CLI, и возобновление сессий работает из коробки.
- **Реестр провайдеров**: в `ProviderRegistry` остался только Claude-провайдер. UI показывает один активный источник, что исключает путаницу с неготовыми стеками Codex/Gemini.
