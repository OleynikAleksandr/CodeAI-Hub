...
**Состояние:** релиз 1.1.267 (18.11.2025) — Autonomous Core с TTL/idle‑моделью, Core Supervisor как единая точка старта ядра, файловый реестр провайдеров, выравнивание окружения Supervisor с CLI‑скриптом и надёжный запуск через VS Code extension и CEF Launcher (с fallback при отсутствии `codeai-core` в PATH).
