...
**Состояние:** релиз 1.1.175 (09.11.2025) — Gemini CLI 0.11.x поддерживается из коробки, теперь система сама устанавливает `@google/gemini-cli` при отсутствии/устаревании, новые релизы больше не принудительно убивают core, и мы продолжаем работу над владением портами.
...
- **Sticky клиенты и автоперезапуск** — VS Code расширение держит невидимое WebSocket-подключение (`CoreKeepAlive`) к `RemoteBridge`, поэтому ядро остаётся активным, даже если webview свернуто или окно редактора в фоне. Потеря соединения/выход `child` моментально приводит к `ensureStarted()` и переподключению, а orchestrator даёт `shutdownGracePeriodMs` перед `idle`-остановкой, чтобы короткие разрывы соединения не гасили процесс. Лаунчер продолжает собственный health monitoring, а ручные UI-команды лишь прогревают ядро при старте, но не перезапускают его на каждом фокусе.
