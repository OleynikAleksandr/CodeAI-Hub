...
- **Логирование и окружение**: `CodeAIHubLauncher` пишет события в `~/.codeai-hub/logs/launcher/launcher.log`, перед запуском дописывает `node/bin` из установленного core в `PATH`, а оркестратор выводит JSON-записи в `~/.codeai-hub/logs/core/core.log` (путь задаётся `CODEAI_CORE_LOG_FILE`).
