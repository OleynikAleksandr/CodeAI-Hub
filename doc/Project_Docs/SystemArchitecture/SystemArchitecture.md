...
**Состояние:** релиз 1.1.368 (31.12.2025) — Idea Collector получает универсальный контракт из Core API (`/api/v1/orchestrator/idea-contract`), контракт v2 добавляет `readiness`/`handoff_for_spec`, а Core сохраняет `.codeai-hub/orchestrator/idea.md` в workspace через `POST /api/v1/orchestrator/idea-artifact`. VS Code Webview и CEF Launcher загружают интерфейс из независимых пакетов (`~/.codeai-hub/packages/ui/**`). Launcher поддерживает независимые окна для Web Client и Project Manager. Гейты качества унифицированы через Husky и скрипты `build-all.sh` / `build-release.sh`.
