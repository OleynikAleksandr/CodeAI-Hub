...
- **Логирование**: `CodeAIHubLauncher` пишет события в `~/.codeai-hub/logs/launcher/launcher.log`, а оркестратор выводит JSON-записи в `~/.codeai-hub/logs/core/core.log` (путь задаётся переменной `CODEAI_CORE_LOG_FILE`).
