...
- **Клиенты интерфейса**
  ...
  - `Web клиент (локальный браузер)` — PWA, работающая через `http://localhost:<порт>`. Подходит для одновременного просмотра нескольких сессий, запускается параллельно с VS Code или в автономном режиме. Расширение автоматически создаёт ярлык (`.lnk`/`.app`/`.desktop`) для быстрого запуска вне VS Code. Сборка веб-клиента инлайнит ключевые CSS, поэтому браузер получает то же оформление, что и webview.
...
graph TD
    ...
    User --> LocalClient["Web клиент<br/>локальный браузер"]
...
- Клиенты (VS Code, локальный и облачный веб-интерфейсы) подключаются к Remote UI Bridge по WebSocket/HTTPS. Они используют унифицированный контракт событий (`session:update`, `stream:chunk`, `workflow:event`, `settings:changed`), что обеспечивает идентичный опыт независимо от оболочки.
...
- **Веб-клиенты**: два отдельных бандла — локальный PWA (desktop-first) и облачный/mobile UI. Оба используют один контракт, но имеют собственные темы, breakpoints и сценарии (например, быстрые жесты на мобильном).
