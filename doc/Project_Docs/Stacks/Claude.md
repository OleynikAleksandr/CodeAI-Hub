...
## Обзор
Приватный модуль стека Claude (`@codeai-hub/claude-module@1.1.300`) предоставляет серверную интеграцию для CodeAI-Hub: управляет установкой CLI/SDK Anthropic, авторизацией пользователя и адаптацией потоковых событий Claude к унифицированному контракту расширения. Фронтенд вебвью наследуется из open source проекта `claude-code-fusion`, а стек Claude фокусируется на бэкенде и совместимости с мультистековой архитектурой. Начиная с релиза 1.1.300 версии модуля в `package.json` и манифесте `assets/providers/claude/manifest.json` должны оставаться синхронизированными, чтобы ProviderVersionService и Settings UI показывали корректный статус карты провайдера.
