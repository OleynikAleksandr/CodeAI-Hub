...
## Обзор
Приватный модуль стека Claude (`@codeai-hub/claude-module@1.1.325`) предоставляет серверную интеграцию для CodeAI-Hub: управляет установкой CLI/SDK Anthropic, авторизацией пользователя и адаптацией потоковых событий Claude к унифицированному контракту расширения. Фронтенд вебвью наследуется из open source проекта `claude-code-fusion`, а стек Claude фокусируется на бэкенде и совместимости с мультистековой архитектурой. Начиная с релиза 1.1.325 ProviderVersionService читает версии CLI/SDK из глобального npm; манифест `assets/providers/claude/manifest.json` используется только для установки провайдера.
