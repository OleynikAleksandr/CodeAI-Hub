...
## 1. Purpose & Scope
- Document the structure and behaviour of the Codex TypeScript SDK so we can implement and maintain provider module `@codeai-hub/codex-module@1.1.416` inside CodeAI-Hub Core.
...
8. Thinking в UI в MVP берётся из native reasoning событий провайдера; RU thinking summary через отдельное поле structured output не используется (см. deprecated: `doc/Project_Docs/Stacks/Codex_Thinking_RU_Summary_Structured_Outputs.md`).
...
## 12. Reference Links
- CodeAI Hub structured outputs contract: `doc/Project_Docs/Stacks/Codex_Thinking_RU_Summary_Structured_Outputs.md`
