...
## 1. Purpose & Scope
- Document the structure and behaviour of the Codex TypeScript SDK so we can implement and maintain provider module `@codeai-hub/codex-module@1.1.419` inside CodeAI-Hub Core.
...
8. Thinking в UI в MVP берётся из native reasoning событий провайдера; RU thinking summary через отдельное поле structured output не используется (подробнее: `doc/Knowledge/Контролируемое отображение размышлений в Codex.md`).
...
## 12. Reference Links
- CodeAI Hub structured outputs + thinking UX: `doc/Knowledge/Контролируемое отображение размышлений в Codex.md`
