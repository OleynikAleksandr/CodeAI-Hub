...
- **Current launcher version:** `CodeAIHubLauncher` 1.1.417 (macOS arm64)
...
2. The manifest entry (`baseUrl` → `file:///Users/oleksandroliinyk/.codeai-hub/releases/`) resolves to a tarball (`CodeAIHubLauncher-macos-arm64-1.1.417.tar.bz2`).
3. The archive is downloaded or reused from the local cache, verified via SHA-1 and unpacked into `~/.codeai-hub/packages/launcher/darwin-arm64/1.1.417/`.
...
~/.codeai-hub/
  ...
  cef-launcher/
    darwin-arm64/
      1.1.417/  (legacy mirror)
  releases/
    CodeAIHubLauncher-macos-arm64-1.1.417.tar.bz2
```
