...
- `src/client/ui/src/session/status-panel.tsx`
  - уже умеет показывать `Core online / Core unavailable / Starting core...`

Новый requirement:

- этого недостаточно как UX;
...
Связь с [MultiProvider_Orchestration_Scenarios.md](../Plans/MultiProvider_Orchestration_Scenarios.md#L1):

- `dialog:switch:*` и `dialog-switch-orchestrator`
  - это low-level primitive для будущего `multi-provider-orchestrator`
  - соответствует идее общего coordination layer из [раздела 2](../Plans/MultiProvider_Orchestration_Scenarios.md#L34)

- current `switch_provider` / `switch_model`
  - это база для `Adaptive Specialist Routing` из [Scenario D](../Plans/MultiProvider_Orchestration_Scenarios.md#L182)

- provider failure fallback внутри continuity
  - это прямой ответ на open question про graceful fallback, если один provider "умер" в multi-provider flow [раздел 6, пункт 5](../Plans/MultiProvider_Orchestration_Scenarios.md#L354)
