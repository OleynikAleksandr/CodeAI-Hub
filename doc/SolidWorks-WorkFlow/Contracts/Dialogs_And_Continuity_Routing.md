...
- Usage telemetry routing на open/reconnect является replay-first: при `session_opened`, `dialog_opened`, `binding_ready`, `provider_session_rebound` и `reconnect` Core сначала replay-ит cached `usage_limits` / `token_usage`, а bootstrap refresh допускается только один раз на lifecycle ready-binding и только при отсутствии cache.
