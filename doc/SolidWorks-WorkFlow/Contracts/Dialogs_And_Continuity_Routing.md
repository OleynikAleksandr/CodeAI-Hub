...
- Continuity rollover меняет live `sessionId` только после завершения turn-а и materialization выбранного continuation path; до этого ранние usage snapshots могут влиять лишь на post-turn arbitration, но не на visible dialog routing.
- Reopened workflow dialogs обязаны сохранять flow-node context при materialization/reuse: `workspaceSlug` становится `initiativeSlug`, `stage` берётся из continuity chain, `runSlug` остаётся `null` для trunk nodes (`description`, `virtual_simulation`, `diagram_modules`). Если runtime session уже была materialized без `initiativeSlug`, `dialog:send` обязан repair-ить context перед provider dispatch.
- Для trunk Documentation Tree continuity (`description`, `virtual_simulation`, `diagram_modules`) применяется synthetic rollover lifecycle: Core создаёт новый runtime/provider segment, не отправляет Create Report, не ждёт report path, не отправляет standalone resume/bootstrap prompt и разблокирует input после готовности target session.
- Первый реальный user message в target Documentation Tree session получает provider-only continuation envelope с workflow start/step contract, explicit continuation semantics и last user-visible assistant message. UI/history при этом показывают исходный пользовательский текст без служебной обёртки.
- Report-based create-report/resume lifecycle остаётся routing boundary для implementation-heavy / non-Documentation-Tree flows с отдельным continuity contract; branch/run agents не участвуют в trunk Documentation Tree routing contract.
