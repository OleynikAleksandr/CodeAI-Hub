...
Явное утверждение выполняется через Core review-card с inline-кнопкой `Подтверждаю`, а не через требование вручную написать это слово в чат.
...
4. После завершения turn Core показывает review-card с кнопкой `Подтверждаю`; если пользователь отвечает на вопросы или просит правки, следующий turn снова завершается такой же review-card.
...
- пользователь явно подтвердил документ через Core review-card / кнопку `Подтверждаю`,
...
### 4.6 Core review gate

`Description` остаётся provider-direct step: Core не открывает managed technical stage plan и не показывает review-card до первого ответа агента. После каждого завершённого provider turn Core добавляет system card с кнопкой `Подтверждаю`.

Свободный текст пользователя после этой card является ответом/правкой и отправляется агенту. Кнопка `Подтверждаю` принимает текущий `Final_Description.md` и переводит пользователя к карточке `Virtual Simulation`.
...
- Session UI рендерит Core review-card с кнопкой `Подтверждаю` после каждого завершённого turn-а агента до принятия шага.
