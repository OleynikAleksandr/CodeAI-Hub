...
- During the Managed Workflow Orchestration rewrite, `diagram_modules`, `application_skeleton`, and `quality_gates` do not have an active automatic stage-continuation envelope. A future cluster must define any stage context bundle, artifact-mode marker, audit stream, or provider-visible follow-up as a new contract.
- Documentation Tree synthetic rollover remains allowed only as ordinary session continuity. It must not revive the suspended workspace lifecycle, task movement, provider feedback loop, or next-stage release behavior.
...
- Continuity chain/index persistence is the durable UI load boundary for every workflow stage family: `description`, `virtual_simulation`, `diagram_modules`, `application_skeleton`, `quality_gates`, and nested `development_tree/...` node sessions. Writes must be per-path serialized and atomic (temp file + rename), so concurrent Core/provider events cannot leave partial JSON on disk. Reads must recover legacy chain/index files that contain one complete JSON object followed by trailing corrupt bytes; such files remain valid continuity evidence and must be rewritten as clean JSON on the next save.
