...
- Переведённые Gemini thoughts не должны выглядеть как отдельный provider role в UI: текущий продуктовый контракт хранит их как `assistant` + `tag: "thinking"` и показывает как видимые tagged assistant messages; `thinkingDisplaySyncEnabled` — это только presentation toggle (`Thinking in dialog`), поэтому перевод продолжает работать даже когда видимая плашка скрыта.
