...
- Claude final assistant text finalization is order-safe: `claude-stream-event-router.ts` captures whether live `text_delta` content was already materialized before calling `consumeFinalText()`, clears the legacy pending buffer for that session when live text already surfaced, and emits only the unseen tail (or divergent canonical block) through the live-text path. This prevents the confirmed duplicate-tail/orphan-suffix regression when the assembled `assistant` payload arrives before a trailing `content_block_stop`.
...
## Kimi-Claude-Code boundary
- Kimi-Claude-Code (`kimiClaudeCode`) is a separate provider id and runtime profile documented in `doc/SolidWorks-WorkFlow/Modules/Kimi_Claude_Code.md`.
- The Claude provider remains subscription/OAuth based: `HOME=~/.codeai-hub/providers/claude/home`, `CLAUDE_USE_CLI_AUTH=true`, `CLAUDE_SUBSCRIPTION_MODE=true`, and Claude auth bootstrap must not read Kimi API keys.
- Reused Claude SDK helpers must be profile-driven. Adding Kimi-Claude-Code must not weaken the normal Claude invariants: `settingSources: []`, CodeAI-owned `systemPrompt`, explicit workflow tools, provider-home isolation, and Claude usage/context readers.
- Claude usage limits and `/context` telemetry belong only to the Claude provider. Kimi-Claude-Code must render unavailable telemetry unless its own Kimi account/source is proven and wired separately.
- Capture evidence for Kimi-Claude-Code may reuse the Claude native request capture service, but the target host/base URL and auth env are provider-profile facts, not changes to the Claude capture contract.
