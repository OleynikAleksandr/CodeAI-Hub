...
- LM Studio local translation uses Core runtime profiles instead of a single fixed context window. Live reasoning translation keeps the small `8192` context profile for warm, fast short blocks; generic translation uses `16384`; localization bundle materialization estimates the request size and rounds up to `8192`, `16384`, or `32768` while clamping to the model's advertised maximum. Workflow-agent turns use a separate `16384` default and the native LM Studio `/api/v1/chat` response shape, parsing only final `message` output after any model reasoning blocks. Reasoning-only outputs fail explicitly instead of being treated as workflow artifacts. The legacy `CODEAI_LMSTUDIO_CONTEXT_LENGTH` override still forces all profiles, while purpose-specific overrides are available for diagnostics.
- Before loading a local model, Core checks `lms ps --json`. If a matching loaded model already has enough context, Core reuses that identifier for the API call. If idle duplicate clones exist for the same LM Studio model, Core unloads CodeAI-owned identifiers, the base `modelKey` identifier, and LM Studio `modelKey:N` clones that are not the selected instance. Core also unloads idle `codeaihub-*` workers for other local model keys before creating a new selected load, so translation/localization and workflow-agent trials do not accumulate multiple heavy idle workers; generating workers and user/manual LM Studio identifiers are left alone.
