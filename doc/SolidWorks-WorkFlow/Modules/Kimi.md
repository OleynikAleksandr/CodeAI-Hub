...
- LSP-style code navigation (`find references`, `go to definition`, call hierarchy) and semantic code search are not part of the current Kimi ACP executable surface. They should be scoped as shared provider tools for GLM/Kimi after the basic file/search/web/edit tools are stable, not added as provider-specific prompt text.
