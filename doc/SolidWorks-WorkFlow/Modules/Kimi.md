...
- The managed profile asks Kimi to emit visible ordinary assistant progress updates early and repeatedly during long managed artifact work: first within roughly 15-30 seconds of substantial work, then roughly every 45-60 seconds, with at least two visible updates before the final answer for multi-minute turns. These updates must be concise user-safe operational summaries derived from work progress, not raw reasoning text. The profile now gives concrete reusable update patterns: sources reviewed, artifact section being built, coverage achieved, boundary/assumption found, remaining validation, or next section to write. It also gives Kimi a thinking-block budget: each substantial hidden reasoning paragraph, draft chunk, or planning block counts as an internal thinking block, and after every 5-6 such blocks without a visible assistant message Kimi must publish an ordinary progress summary before continuing. This is an instruction-level control, not a protocol guarantee: Core must still rely on Wire lifecycle, `think` chunks, and status events to avoid stuck-working UI.
