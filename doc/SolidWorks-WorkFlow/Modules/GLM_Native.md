...
- The adapter prepends a provider-owned system message to every request. After the `1.2.582` acceptance follow-up, this system context starts with the captured Codex-native system instructions and appends a GLM runtime addendum with the actual workspace/model/tool-loop boundary. Tool definitions are not duplicated inside the system text; they are sent through the GLM `tools` request field.
...
- The GLM request now exposes the complete captured Codex-native tool surface converted to Z.AI/OpenAI-compatible function tools, plus CodeAI Hub's `write_workflow_artifact(relative_path, content)`. Z.AI requires tool definitions in the request `tools` array as `{ type: "function", function: { name, description, parameters } }`; Codex namespace tools are flattened with double underscores, for example `mcp__playwright__browser_click`.
- Local GLM execution currently wires `exec_command` and `write_workflow_artifact`. Other declared Codex-compatible functions are visible to the model in the proper GLM tool format and return an explicit not-wired result until Core provides executors for those tool families. The 1.2.583 retest showed GLM can call these visible-but-unwired tools in real work: it requested `web_search` after codebase inspection, received explicit not-wired tool results, and then hit a provider `EPIPE` on the next large request. Keep this gap visible when deciding whether to prune unwired tools or bridge them.
