...
- The GLM request exposes GLM-owned executable tools in Z.AI/OpenAI-compatible `{ type: "function", function: { name, description, parameters } }` format. The current standalone/workflow tool surface is `exec_command`, `grep_files`, `glob_files`, `read_file`, `write_file`, `apply_patch`, `web_search`, `web_fetch`, and `write_workflow_artifact`.
...
- Codebase search should use `grep_files` / `glob_files`, which run ripgrep through streaming `spawn` with workspace scoping and output limits. `exec_command` also uses streaming process execution instead of Node `child_process.exec`, avoiding the observed `rg` hang under the old executor.
- Web lookup uses `web_search` and `web_fetch` local executors. They are provider tools, not Codex web tool aliases, and failures are returned as tool results with source/status details.
