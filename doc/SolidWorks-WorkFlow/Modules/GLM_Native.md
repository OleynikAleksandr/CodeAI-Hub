...
- Anchored editing is available through `read_file_anchored` and `edit_file_by_anchor`: reads return short per-line string anchors, edits must reference those string anchors rather than line numbers, and stale anchors are rejected before writing. `edit_file` is also available for exact single-occurrence `old_string` -> `new_string` replacements.
...
- `browser_fetch` uses an installed Chrome/Chromium/Edge executable when present to fetch rendered DOM text. On macOS it checks standard `/Applications/*.app/Contents/MacOS/*` paths and also honors `CODEAI_GLM_BROWSER_PATH` for a custom executable. If no supported browser binary exists, it returns an explicit tool error instead of pretending JS-rendered content was fetched.
...
| `edit_file_by_anchor` | Replace/delete/insert по string anchors из `read_file_anchored`. | `glm-native-file-edit-tools.ts` | `old_range` принимает `[start_anchor,end_anchor]`, а не номера строк; stale/missing anchors reject; overlapping ranges reject; edits применяются атомарно через temp-file + rename. |
...
| `browser_fetch` | Rendered DOM text через установленный Chrome/Chromium/Edge. | `glm-native-workspace-tools.ts` | Не тянет Playwright dependency; проверяет стандартные macOS Chrome/Chromium/Edge paths и `CODEAI_GLM_BROWSER_PATH`; если browser binary не найден, возвращает явную ошибку. |
