...
- For `lmstudio:<modelKey>` materialization, Localization remains transport-neutral and passes the selected engine through `TranslationFacade`; Core owns the LM Studio runtime profile. Reasoning overlays keep the small fast profile, while UI bundle materialization receives an adaptive context window based on the bounded batch size and model maximum, so Localization does not load a model at the full advertised context and does not create extra clones itself.
