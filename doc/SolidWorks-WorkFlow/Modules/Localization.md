...
- visible reasoning overlays are translation-first in the Session UI: Core emits source `content` for transcript integrity and may mark the visible projection as pending, while the browser hides untranslated reasoning source until `localizedContent` arrives, then reveals the translated text progressively. Source is only a UI fallback when the translation does not arrive in time.
