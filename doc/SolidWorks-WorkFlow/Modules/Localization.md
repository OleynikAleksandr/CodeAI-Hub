...
- Engine and category-impacting saves still enter the strict sync path and materialize required translated runtime bundles in deterministic order (`user_guidance -> system_feedback -> interactive_templates -> ui_interface -> workflow_terms`), refusing to emit a ready payload while any required bundle still contains fallback or `partial_fallback` entries.
