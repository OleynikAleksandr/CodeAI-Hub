...
- General Settings owns the user-facing entry point for provider native request capture diagnostics. `src/client/ui/src/components/settings/native-request-capture-card.tsx` renders the bottom card with one scenario selector (`Description`, `Virtual Simulation`, `Diagram Modules`, `Translation`), separate Claude/Codex model selectors and command buttons, running/success/error state, and returned `.md` / `.jsonl` artifact paths. The UI sends `settings:native-request-capture` through the Project Manager bridge; `src/client/project-manager/services/native-request-capture-scenario-prompt.ts` resolves only workflow scenarios through the same `buildWorkflowPromptPack(...)` path used by normal workflow sends, while `Translation` is forwarded without a workflow prompt so Core/provider capture can use the real translation profile. Core returns `settings:native-request-capture:result`. The card must not persist model/scenario settings because these controls are one-shot diagnostics, not saved preferences.
