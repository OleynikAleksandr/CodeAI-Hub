...
- Project Manager left sidebar is a projection of Core workflow-state, not a filesystem reader. Development Tree rows render the Core snapshot as `Product Part -> Cluster -> Module -> Operation` where operation rows are `Module / Facade Specification`, `Implementation`, `Workers`, and `Integration`. Long operation labels wrap inside the wider sidebar, and operation selections route to the same session/artifact panes using Core-provided `workflowPath` / `artifactWorkspacePath`.
