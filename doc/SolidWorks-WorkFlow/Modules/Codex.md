...
## Transport cluster
- Runtime transport — это long-lived `codex app-server`, поднятый через `child_process.spawn(..., ["app-server", "--disable", "multi_agent"])`.
- CodeAI Hub disables the Codex `multi_agent` feature at app-server startup for normal runtime and diagnostic capture. This is the current test profile for early workflow turns: it should remove the subagent tool family (`spawn_agent`, `send_input`, `resume_agent`, `wait_agent`, `close_agent`) from provider-native `body.tools` while preserving the rest of the Codex runtime/tool surface until each remaining tool class is tested separately.
