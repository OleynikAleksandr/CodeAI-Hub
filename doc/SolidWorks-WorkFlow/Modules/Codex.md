...
- Progress-update wording in this instruction profile must ask Codex for ordinary user-visible assistant chat messages, not hidden commentary/reasoning/tool-adjacent notes. This is an experimental `1.2.85` prompt tuning based on `gpt-5.2` evidence where the model treated `short commentary messages` ambiguously and produced no visible interim updates.
