...
- Core remote bridge cluster: `packages/core/src/remote-bridge/`
  ...
  - `handlers/session-request-handler-applied-turn-config.ts`, `handlers/session-request-handler-message-dispatch.ts`, and `remote-bridge/types.ts` now form one provider-neutral outbound bridge contract: Core attaches one next-turn identity descriptor derived from `settings.json`; presentation-only flags like `thinkingDisplaySyncEnabled` travel on the same applied-config envelope for Codex/Gemini but never participate in `modelId` computation; outbound `session:model:update` normalizes it so public `modelId` is always the effective identity; `baseModelId` remains optional auxiliary metadata and must never replace `modelId` as the runtime identity key
...
- Config cluster: `packages/core/src/config/`
  ...
  - `provider-settings-snapshot.ts` = persisted provider settings readers, including presentation-only thinking display sync flags for Codex/Gemini
