...
14. **Effective model identity SSOT**: `modelId` в transport/runtime/UI контракте означает полную effective model identity, а не только base model; reasoning/thinking являются частью identity, и единственным source of truth для next-turn model identity является `~/.codeai-hub/settings/settings.json`.
   ...
   - Канон: `doc/SolidWorks-WorkFlow/Contracts/EffectiveModelIdentity_And_Settings_SSOT.md`, `packages/core/src/config/provider-turn-config-resolver.ts`, `packages/core/src/remote-bridge/handlers/session-request-handler-applied-turn-config.ts`, `packages/core/src/remote-bridge/handlers/session-provider-event-router.ts`.
...
- Provider modules: `packages/Claude_Module/`, `packages/Codex_AppServer_Module/`, `packages/Gemini_Module/` (legacy `packages/Codex_Module/` remains fallback-only and is not the active bundled/runtime line)
...
- Shared diagram editor UX обязан сохранять визуальную непрерывность:
  ...
  - visual shell не показывает auto-layout chrome, zoom/fit controls или minimap; зум выполняется через CSS `transform` по Cmd/Ctrl+scroll (25–200%, шаг 1%), reset — Cmd/Ctrl+0 или clickable badge в status bar; drag-based node movement отсутствует — композиция управляется декларативными layout params.
