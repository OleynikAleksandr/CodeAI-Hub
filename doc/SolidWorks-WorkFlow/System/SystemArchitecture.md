...
22. **Localization blocking UI invariant**: busy/save-sync blocking may temporarily hide PM or settings interactive surfaces, but it must not change React hook order or leave the renderer mounted as an empty shell after `busy -> ready`. Blocking placeholders are allowed; hook-graph drift is not. Busy-state copy in the Settings card and the Project Manager blocker must describe incremental sync of the runtime bundles actually affected by the last save, not a blanket rebuild of the entire translated interface — provider-only, response-policy, and continuity-only saves skip the overlay entirely, and engine/category saves rebuild only the planned bundle set.
   - Канон: `src/client/project-manager/components/layout/main-area-panel-content.tsx`, `src/client/ui/src/components/settings-view.tsx`, `doc/SolidWorks-WorkFlow/Modules/Localization.md`.
