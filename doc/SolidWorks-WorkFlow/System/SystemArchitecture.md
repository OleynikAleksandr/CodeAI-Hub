...
25. **Provider live thinking is incremental and dedup-safe**: providers must surface visible reasoning incrementally rather than as one final monolithic block whenever the upstream stream supports delta events. For Claude this is owned by `ClaudeThinkingLiveBuffer` + `ClaudeThinkingStreamHandler` which materialize sentence/paragraph segments from `thinking_delta` fragments, emit them as append-only thinking dialog bubbles, and reconcile the final assembled `thinking` block against the materialized prefix so the same reasoning never appears twice in the dialog. Core-owned translation overlays must be ready to attach `localizedContent` to multiple thinking bubbles per turn, each carrying its own stable `messageId`.
...
- Claude messaging cluster: `packages/Claude_Module/src/messaging/`
  ...
  - `claude-stream-event-router.ts` = assistant/result/structured-output/thinking routing; delegates the live-thinking ingestion path to `claude-thinking-stream-handler.ts` and reconciles the final assembled thinking block against materialized live segments before emitting
  - `claude-thinking-stream-handler.ts` = micro-class watching `content_block_start/delta/stop` for thinking blocks and exposing `consumeFinalThinking` for finalization dedupe
  - `claude-thinking-live-buffer.ts` = per-session accumulator that turns raw `thinking_delta` fragments into readable, sentence-bounded segments emitted as append-only thinking dialog bubbles while reasoning is still streaming
