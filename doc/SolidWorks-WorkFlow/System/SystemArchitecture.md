...
14. **Effective model identity SSOT**: `modelId` в transport/runtime/UI контракте означает полную effective model identity, а не только base model; reasoning/thinking являются частью identity, и единственным source of truth для next-turn model identity является `~/.codeai-hub/settings/settings.json`.
   ...
   - Client-side fallback contract (1.2.15): клиентский `ModelInfo` builder в [`src/client/ui/src/session/model-info-builder.ts`](src/client/ui/src/session/model-info-builder.ts) при fallback-пути к `settings` (initial render до первого `session:model:update`) обязан обернуть raw level из `thinkingLevelByModel` / `reasoningByModel` в тот же provider-specific prefix, который эмитит `parseEffectiveModelId` на effective id (`thinking ${level}` для Gemini, `reasoning ${level}` для Codex). Claude ветка сохраняет отдельную convention (`"thinking off"` для disabled, raw effort для enabled). Без этой consistency UI показывает label в двух разных формах за время первого рендера (начальный fallback → Core broadcast).
