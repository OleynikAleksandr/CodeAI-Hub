...
- Project Manager applied-config sync:
  ...
  - Session dialog markdown list rendering is split by ownership: provider/runtime must not emit marker-only live fragments, `mergeThinkingMessages` in the shared Session UI repairs any already-split thinking list boundary (`2.` + next fragment text), and PM dialog CSS renders markdown list markers with outside positioning so valid loose ordered lists keep the number and the item text on the same visual line
