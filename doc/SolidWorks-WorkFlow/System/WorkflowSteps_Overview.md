...
**2026-05-25 Clear/rollback note, updated 2026-06-03:** workflow stage `Clear` uses Core-owned Git boundary commits, not undo ledgers, checkpoints, runtime-slice copies, or inferred path deletion. Core creates a `codeai-boundary: <Stage Label>` commit before each trunk stage starts, beginning with `Description` at workspace activation after the workspace runtime capsule baseline exists. Git log/boundary commits are the rollback authority; any boundary registry file is a rebuildable projection only. Clear restores the selected boundary with Git reset/clean, resets Core/PM projections from the restored filesystem, and Project Manager must immediately run restart-equivalent status/session/history rehydration after the Clear completes. Workspace settings, localization bundles/cache, and provider auth/settings/cache files are live runtime under `.codeai-hub/<workspaceSlug>/runtime/**`; they are ignored by rollback and preserved across Clear. Workflow session continuity is not preserved: Clear prunes workflow unified histories and provider-native session history files inside the workspace runtime capsule. Development Tree node clear stays fail-closed until a separate node-boundary design exists.
