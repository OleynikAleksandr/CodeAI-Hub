...
Workflow-specific mutable runtime state lives inside the workspace-owned runtime capsule. Rollback-relevant session/provider state is committed directly by Git: `.codeai-hub/<workspaceSlug>/runtime/sessions/unified/**` and `.codeai-hub/<workspaceSlug>/runtime/providers/{codex,claude,gemini,kimi}/home/**`. `.codeai-hub/<workspaceSlug>/runtime/settings/settings.json` is still the active workspace settings source, but it is ignored by workflow rollback and preserved across Clear/Undo; reproducibility comes from committed `Session.modelBinding`, applied turn config, and provider/session artifacts. Clear runs the selected boundary rollback as a normal Git transaction (`reset --hard` plus `clean -fd`), so downstream unified sessions, provider-native sessions, scaffold files, workflow state, and accepted artifacts disappear only because they are tracked or untracked files inside the workspace tree. Global auth/secret bridges, installed provider binaries, release caches, generic logs, and browser caches remain outside rollback truth; Project Manager saves settings through Core into the workspace capsule settings file without adding them to workflow history.

When a workflow start card changes the active provider model or reasoning, the workspace settings file is saved but not committed. Core creates the next `codeai-boundary: <Stage Label>` anchor after ensuring any legacy tracked settings entry is removed from the index; the stage/session itself captures the immutable applied config/model binding needed to reproduce what the provider used.
