...
Runtime session state that lives outside the workspace is captured as per-workspace/per-session slices under `.codeai-hub/<workspaceSlug>/runtime-slices/` with a committed manifest. Clear restores the selected Git boundary and then rehydrates those slices back into user-space session stores. Provider-native session files created after the captured boundary are pruned inside the recorded provider session directories, while older session history is preserved. Global settings, auth files, logs, provider binaries, and local timer state such as `.codeai-hub/state/` are not development artifacts and must not be committed as rollback truth.
