...
Workflow-specific mutable runtime state lives inside the workspace-owned runtime capsule. Rollback-relevant Core session state is committed directly by Git under `.codeai-hub/<workspaceSlug>/runtime/sessions/unified/**`, along with workflow state, accepted artifacts and other recovery-critical capsule files. `.codeai-hub/<workspaceSlug>/runtime/settings/settings.json`, `.codeai-hub/<workspaceSlug>/runtime/localization/**`, and complete provider homes under `.codeai-hub/<workspaceSlug>/runtime/providers/**/home/**` are active workspace runtime but ignored by workflow rollback and removed from legacy indexes; reproducibility comes from committed `Session.modelBinding`, applied turn config, Core logical sessions and accepted artifacts rather than rolling live settings/localization/provider-home state backward. Translation/localization provider-native sessions are one-shot runtime implementation detail and are deleted after successful translation; finalized workspace localization bundles remain the retained output. Clear runs the selected boundary rollback as a normal Git transaction (`reset --hard` plus `clean -fd`) while preserving current legacy-tracked rollback-ignored runtime files, so downstream unified sessions, scaffold files, workflow state, and accepted artifacts disappear only because they are tracked or untracked workflow files inside the workspace tree. Once Clear returns success, Project Manager treats it like a Core restart signal for session surfaces: it reloads `/api/v1/status` and session histories instead of waiting for manual restart. Global auth/secret bridges, installed provider binaries, release caches, generic logs, legacy localization caches, and browser caches remain outside rollback truth; Project Manager saves settings/localization changes through Core into the active workspace runtime without adding them to workflow history.
...
- Quality Gates Baseline remains locked until Core validates that Application Skeleton materialization includes the accepted project foundation, not just `materialized: true`, and the user accepts the final post-materialization review gate;
