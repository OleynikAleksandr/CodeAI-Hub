...
**2026-05-25 Clear/rollback note, updated 2026-06-03 after cleanup rollback:** workflow stage `Clear` uses Core-owned Git boundary commits, not undo ledgers, checkpoints, runtime-slice copies, or inferred path deletion. Core creates a `codeai-boundary: <Stage Label>` commit before each trunk stage starts, beginning with `Description` at workspace activation after the workspace runtime capsule baseline exists. Git log/boundary commits are the rollback authority; any boundary registry file is a rebuildable projection only. Clear restores the selected boundary with Git reset/clean, rematerializes the pruned `workflow/boundaries.json` registry projection before the `codeai-clear:*` commit, resets Core/PM projections from the restored filesystem, and Project Manager must immediately run restart-equivalent status/session/history rehydration after the Clear completes. Workspace settings, localization bundles/cache, and provider auth/settings/cache files are live runtime under `.codeai-hub/<workspaceSlug>/runtime/**`; they are ignored by rollback and preserved across Clear. The broad release `1.2.447` cleanup contract that pruned all workflow unified histories and provider-native session containers has been reverted because it can break provider-native resume durability. Current active code removes live Core runtime session registrations at/after the target stage and only attempts the older narrow Codex/Claude provider-native file cleanup when a file name contains the live `providerSessionId`. Full provider-native cleanup remains a separate resume-safe design task. Development Tree node clear stays fail-closed until a separate node-boundary design exists.
...
Workflow-specific mutable runtime state lives inside the workspace-owned runtime capsule. Rollback-relevant Core session state is committed directly by Git under `.codeai-hub/<workspaceSlug>/runtime/sessions/unified/**`, along with workflow state, accepted artifacts and other recovery-critical capsule files. `.codeai-hub/<workspaceSlug>/runtime/settings/settings.json`, `.codeai-hub/<workspaceSlug>/runtime/localization/**`, and provider homes under `.codeai-hub/<workspaceSlug>/runtime/providers/**/home/**` are active workspace runtime but ignored by workflow rollback and removed from legacy indexes; reproducibility comes from committed `Session.modelBinding`, applied turn config, Core logical sessions and accepted artifacts rather than rolling live settings/localization/provider-home state backward. Translation/localization provider-native sessions are one-shot runtime implementation detail and are deleted after successful translation; finalized workspace localization bundles remain the retained output. Clear runs the selected boundary rollback as a normal Git transaction (`reset --hard` plus `clean -fd`), writes the pruned rebuildable boundary registry projection even when `boundaries.json` did not exist in the target boundary commit, and uses restart-equivalent Project Manager rehydration after success. After the cleanup rollback on 2026-06-03, active code no longer scans and removes all workflow unified `.jsonl` histories, Codex `sessions` / `shell_snapshots`, Claude project/session logs, Gemini chat sessions, or provider SQLite state. It only deletes live Core runtime session registrations at/after the target stage and attempts the older narrow Codex/Claude native-file cleanup for files whose basename contains the live `providerSessionId`. Provider auth, settings, installation ids, caches, models, memories and other non-session runtime files remain preserved. Global auth/secret bridges, installed provider binaries, release caches, generic logs, legacy localization caches, and browser caches remain outside rollback truth; Project Manager saves settings/localization changes through Core into the active workspace runtime without adding them to workflow history.

When a workflow start card changes the active provider model or reasoning, the workspace settings file is saved but not committed. Core creates the next `codeai-boundary: <Stage Label>` anchor after ensuring any legacy tracked rollback-ignored runtime entry is removed from the index; the stage/session itself captures the immutable applied config/model binding needed to reproduce what the provider used.
...
- нажатие `Подтверждаю` принимает текущий preliminary artifact only after Core commits the accepted artifact plus tracked workspace runtime capsule state directly and verifies that `git status --porcelain` is clean; if Git remains dirty, Core blocks the next-step return path with a visible validation message.
