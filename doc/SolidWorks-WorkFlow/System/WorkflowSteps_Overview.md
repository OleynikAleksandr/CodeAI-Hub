...
- Lead Product Part downstream coordination: after user acceptance of `DevelopmentOrderPlan.draft.json`, Core validates `codeai-development-order-plan-v2`, writes Product Part unlock-state, and starts only the first eligible wave. Cluster Contract nodes run as sub-agents in their own Git worktree/branch, produce `ClusterSpecification` and `ClusterFacadeContract` artifacts, then stop at a user/lead review gate. Acceptance merges accepted contract artifacts back into the main workspace, records merge evidence, marks the cluster `merged`, and keeps dependent modules `locked` until a later module-contract wave is explicitly unlocked.
