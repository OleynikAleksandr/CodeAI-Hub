...
**2026-05-25 Clear/rollback note, updated 2026-06-07 for Product Part Clear/Undo MVP:** workflow stage `Clear` uses Core-owned Git boundary commits, not undo ledgers, checkpoints, runtime-slice copies, inferred path deletion, or separate session cleanup ownership. Core creates a `codeai-boundary: <Stage Label>` commit before each trunk stage starts, beginning with `Description` at workspace activation after the workspace runtime capsule baseline exists. Git log/boundary commits are the rollback authority; any boundary registry file is a rebuildable projection only. Clear restores the selected boundary with Git reset/clean, rematerializes the pruned `workflow/boundaries.json` registry projection before the `codeai-clear:*` commit, resets Core/PM projections from the restored filesystem, and Project Manager must immediately run restart-equivalent status/session/history rehydration after the Clear completes. Workflow unified histories and provider-native workflow session histories under `.codeai-hub/<workspaceSlug>/runtime/**` are Git-owned recovery state while their step exists; after Clear, sessions created at or after the selected boundary disappear through the same Git rollback as artifacts. Preliminary `Description` / `Virtual Simulation` acceptance must wait until Core's completion message and any translation overlay writes are persisted before the accepted-step Git commit, otherwise the next-step unlock remains blocked by a dirty Git tree. After `Application Skeleton` materialization, Core must also persist the final managed completion message and translation overlay, commit that terminal residue, and only then unlock `Quality Gates Baseline`. Workspace settings and localization remain live mutable runtime and are preserved across Clear. Provider auth tokens, API/OAuth credentials, installed packages, caches, SQLite databases, logs, binaries, `Caches/` folders, `*-cache.json` files and other noisy provider runtime files stay ignored by `.gitignore` and are removed from legacy tracked indexes before accepted-step clean-Git checks. For the MVP, local Git and GitHub push carry tracked workflow session histories; repository privacy is the user's access-control boundary. Development Tree Product Part root-node Clear/Undo is supported only as clear-and-restart from current Development Tree filesystem/Git truth: Core deletes the Product Part runtime session, provider-native/unified history files, continuity folder/index entry, managed decision, Product Part root draft/order/research files, and `doc/TODO/stages/development-tree/product-parts/<partId>/todo-plan.md`, then immediately reruns Product Part bootstrap to recreate the fresh plan/draft/session. Cluster/module node clear remains fail-closed until separate branch/worktree boundaries exist.
...
После materialized `Application Skeleton` и verified `Quality Gates Baseline` ствол заканчивается и начинается дерево разработки (Development Tree). Работа ведётся по веткам, привязанным к структуре продукта и уже наложенным на созданный project skeleton:
...
Workflow-specific runtime state lives inside the workspace-owned runtime capsule. Git commits include workflow state, accepted artifacts, recovery-critical capsule files, provider-neutral unified histories under `.codeai-hub/<workspaceSlug>/runtime/sessions/unified/**`, and provider-native workflow session histories under `.codeai-hub/<workspaceSlug>/runtime/providers/**/home/**` when those files are needed to resume provider turns. Current provider-native session histories are Codex `home/sessions/**/*.jsonl`, Claude `home/.claude/projects/**/*.jsonl`, GLM-Claude-Code `home/.claude/projects/**/*.jsonl`, Gemini `home/.gemini/tmp/<workspaceSlug>/chats/*.jsonl`, and Kimi `home/wire.jsonl`; Gemini chat history stays Git-owned even though the provider stores it below `.gemini/tmp`, while non-session provider tmp/cache files remain ignored. Clear runs the selected boundary rollback as a normal Git transaction (`reset --hard` plus `clean -fd`), writes the pruned rebuildable boundary registry projection even when `boundaries.json` did not exist in the target boundary commit, and uses restart-equivalent Project Manager rehydration after success. Settings and localization are the main preserved live runtime: `.codeai-hub/<workspaceSlug>/runtime/settings/settings.json` and `.codeai-hub/<workspaceSlug>/runtime/localization/**` are not rolled backward by workflow Clear. Provider auth tokens, API/OAuth credentials, login files, package installs, caches, SQLite databases, logs, binaries, model caches, provider `Caches/` folders, provider `*-cache.json` files and other non-session runtime files remain ignored and untracked. Accepted-step cleanup must untrack legacy provider cache/log files before the clean-Git gate, but must not untrack unified histories or provider-native session histories that are required for resume. Translation/localization provider-native sessions are one-shot runtime implementation detail and may be deleted after successful translation; finalized workspace localization bundles remain the retained output. Project Manager saves settings/localization changes through Core into the active workspace runtime without adding them to workflow history.

When a workflow start card changes the active provider model or reasoning, the workspace settings file is saved but not committed. Core creates the next `codeai-boundary: <Stage Label>` anchor after ensuring any legacy tracked rollback-ignored runtime entry is removed from the index; the stage/session itself captures the immutable applied config/model binding needed to reproduce what the provider used. If a tracked workflow session transcript is dirty, the next boundary is blocked until the accepted step commit records it, because otherwise Clear could not reliably distinguish current-step state from future-step state.
...
- нажатие `Подтверждаю` принимает текущий preliminary artifact only after Core persists the Core completion message, waits for session message/translation overlay writes, commits the accepted artifact plus tracked workspace runtime capsule state directly, and verifies that `git status --porcelain` is clean; if Git remains dirty, Core blocks the next-step return path with a visible validation message.
...
- Materialization gate: Core держит Quality Gates/Development Tree readiness disabled, пока `application-skeleton-map.json` не содержит committed `materialized: true` with `foundationReady: true`, а `quality-gates.json` не содержит committed `integrated: true`, `verificationState: "verified"` and formal command evidence for the required gate scripts and hook paths. `foundationReady` requires empty `openQuestions`, package manager metadata, package manifest/lockfile, required scripts, config files, and first-wave production entrypoints. Sidebar показывает locked-row вместо раннего session bootstrap.
...
- **Formal verification:** после integration commit Core запускает отдельный Phase 4 prompt. Агент обязан проверить и, где возможно, выполнить все formal gate commands, затем записать `verificationState: "verified"`, `verificationEvidence.checkedAt` and per-command results into `quality-gates.json`.
