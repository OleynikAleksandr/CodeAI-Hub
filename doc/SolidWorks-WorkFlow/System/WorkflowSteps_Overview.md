...
**2026-06-11 Development Tree Product Part / downstream note, updated 2026-06-13 for Product Part worktree lanes and review cursor:** Product Part review sessions are Core-projected Development Tree node sessions, including non-lead Product Parts, and managed startup must persist the primary unified dialog history before provider/translation side effects can race ahead. Accepted Diagram Modules now bootstraps every planned Product Part pre-code agent from the complete Product Part leadership order in deterministic worktree lanes under `<workspace>.worktrees/<workspaceSlug>/product-parts/<partId>/precode`; Quality Gates terminal handoff keeps the same Core bootstrap path only as recovery/idempotency for missing sessions. Product Part manual `Start node` and Product Part root Clear/Restart use the same Core bootstrap path for recovery. Project Manager hides stale Development Tree dialog continuity entries that have neither a live runtime session nor persisted unified history. One Core-owned user-gate cursor chooses the active review gate, keeps queued gates read-only, and drives the sidebar amber attention marker/auto-focus. The lead Product Part `DevelopmentOrderPlan` assignment is behind a Core-owned all-brief barrier: every planned Product Part brief must be user-accepted and sequentially checkpointed from its lane into main, and the lead prompt embeds the full accepted brief markdown for all Product Parts inline. Diagram Modules acceptance persists `workflow/state.json.lastActive.stage = "application_skeleton"` so reconnecting Project Manager clients recover the next-step card even if the realtime activation event was missed. First-wave Cluster Contract acceptance is now `boundary_accepted`, not a mainline merge: Core writes a coordination checkpoint, leaves the cluster worktree active, and reserves `merged` for later code-ready integration.
...
Diagram Modules
 └─ Product Part Specification (ветка per part)
     ├─ Cluster Facade Boundary (ветка/worktree per cluster)
...
- Product Part worktree-lane lifecycle: after accepted Diagram Modules, Core bootstraps every planned Product Part root node from `product-parts.index.md` in one transaction, but each agent writes in its own lane worktree under `<workspace>.worktrees/<workspaceSlug>/product-parts/<partId>/precode`. The bootstrap creates/reuses the managed todo-plan, branch-root draft, projected workflow session, persisted start prompt, and first provider turn from the lane, while main workspace remains a clean orchestration/checkpoint surface for `Application Skeleton` and `Quality Gates Baseline`. The expected set is not hard-coded; Core uses `leadProductPartId`, declared `productPartLeadershipOrder`, and any remaining planned Product Part ids. Quality Gates terminal handoff reruns this bootstrap only as recovery/idempotency for missing Product Part sessions. If any expected Product Part session cannot be started or recovered, the handoff fails closed instead of leaving the lead all-brief barrier to wait silently.
- Downstream node lifecycle: Cluster / Module nodes remain downstream-controlled. They start only through accepted Product Part order waves or explicit node commands; Core validates clean Git and materialized node folder, writes draft artifacts only for that node, and creates the matching session with `runSlug: "development-tree"`.
...
- Product Part review lifecycle: after a Product Part agent creates the branch-root draft in its lane and Core opens the review card, ordinary user text remains a provider-directed revision turn. Explicit acceptance is a Core-owned review action scoped to the active review message. Core records `reviewState: "accepted"`, checkpoints the accepted brief from the lane back into main sequentially with lane provenance, and advances secondary Product Parts to `User Return And Revisions`. Product Part managed review sessions, including non-lead Product Parts, are projected as first-class Development Tree node sessions in Project Manager and must have a persisted primary unified dialog history before later provider/translation messages can appear. Manual `Start node` for a Product Part is only a recovery path through Product Part bootstrap, not a generic session creation shortcut.
- Product Part brief barrier: the lead Product Part does not enter `Development Order Plan Draft` until every planned Product Part brief has a Core-owned user-accepted managed review state and an accepted checkpoint in main. Core evaluates the barrier from `product-parts.index.md`, managed Product Part review decisions, and the checkpointed accepted brief markdown. When the final secondary brief opens the barrier, Core dispatches the lead `DevelopmentOrderPlan` assignment into the lead Product Part session and embeds every accepted brief's full markdown in the prompt.
- Lead Product Part downstream coordination: after user acceptance of `DevelopmentOrderPlan.draft.json`, Core validates `codeai-development-order-plan-v2`, writes Product Part unlock-state, and starts only the first eligible wave. Cluster Contract nodes run as sub-agents in their own Git worktree/branch, produce the cluster facade boundary artifacts, then stop at a user/lead review gate. Acceptance records `boundary_accepted` coordination evidence and keeps the worktree active; it does not copy draft cluster docs into the main workspace, does not mark the cluster `merged`, and does not unlock dependent module work as a completed integration. `merged` is reserved for a later code-ready cluster/standalone-module return that includes code artifacts and validation evidence.
