...
**2026-06-11 Development Tree Product Part / downstream note:** Product Part review sessions are Core-projected Development Tree node sessions, including non-lead Product Parts, and managed startup must persist the primary unified dialog history before provider/translation side effects can race ahead. Quality Gates terminal handoff must bootstrap every planned Product Part agent from the complete Product Part leadership order; Product Part manual `Start node` and Product Part root Clear/Restart use the same Core bootstrap path for recovery. Project Manager hides stale Development Tree dialog continuity entries that have neither a live runtime session nor persisted unified history. The lead Product Part `DevelopmentOrderPlan` assignment is behind a Core-owned all-brief barrier: every planned Product Part brief must be user-accepted, and the lead prompt embeds the full accepted brief markdown for all Product Parts inline. First-wave Cluster Contract acceptance is now `boundary_accepted`, not a mainline merge: Core writes a coordination checkpoint, leaves the cluster worktree active, and reserves `merged` for later code-ready integration.
...
- Product Part fan-out lifecycle: after verified Quality Gates terminal handoff, Core bootstraps every planned Product Part root node from `product-parts.index.md` in one transaction: managed todo-plan, branch-root draft, workflow session, persisted start prompt, and first provider turn. The expected set is not hard-coded; Core uses `leadProductPartId`, declared `productPartLeadershipOrder`, and any remaining planned Product Part ids. If any expected Product Part session cannot be started or recovered, the handoff fails closed instead of leaving the lead all-brief barrier to wait silently.
