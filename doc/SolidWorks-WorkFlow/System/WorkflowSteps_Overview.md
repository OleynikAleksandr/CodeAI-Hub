...
`Application Skeleton` uses the Core-owned managed lifecycle implemented by the replacement cluster. Before the provider turn, Core prepares the deterministic stage artifact directory under `.codeai-hub/<workspaceSlug>/application_skeleton/` and removes only safe empty file conflicts left by a failed tool call; non-empty path conflicts stop the stage instead of being silently deleted. Phase 1 drafts `application-skeleton.md` and `application-skeleton-map.json` only. The draft must include `projectFoundation` decisions and an `openQuestions` list; materialization is illegal while that list is non-empty or while stack/package/build/test/source-layout ambiguity remains. Descriptive metadata in the draft map, including repository-shape descriptions, is not a hard blocker by itself when the actionable foundation fields are present. Phase 2 opens user review from Core state. A direct acceptance immediately closes the Core-managed input gate and advances the stage plan to accepted-only materialization without forwarding the user acceptance text as an agent task; requested corrections stay in the active review task and are sent as a scoped revision prompt. Phase 3 materializes the accepted installable project foundation and Product Part / Cluster / Module folder projection only after acceptance, then Core validates and records the managed commit. A successful Core-owned materialization is not a new semantic user decision: Core auto-completes `Application Skeleton`, persists the final managed completion message and translation overlay, commits that terminal session residue, opens persistent user return, and activates `Quality Gates Baseline` without a second post-materialization `managed-workflow-user-review`. If draft or materialization validation fails, Core keeps the input gate closed, records failed state, and sends a repair turn to the agent when the failure can be expressed as contract/scaffold diagnostics; after three failed repair attempts for the same managed stage path, Core opens a user review gate with the remaining diagnostics instead of continuing the repair loop. Only unrecoverable Core boundary failures are surfaced as validation/blocker messages.
...
- `projectFoundation`, empty `openQuestions`, package manifest/lockfile, required scripts, config files, and first-wave source/facade entrypoints are validated by Core before downstream unlock; descriptive fields do not block the draft unless they hide one of these actionable decisions;
