...
**2026-05-13 lifecycle note:** release `1.2.249` accepts the managed trunk sequence for Diagram Modules, Application Skeleton, and Quality Gates. Quality Gates Baseline runs on the managed child-plan lifecycle. Phase 1 is the Core-gated draft (`docs: draft quality gates contract`); Phase 2 is user-led review where every artifact-changing revision turn commits `docs: revise quality gates contract - revision N`; acceptance is a Core-owned event committed as `docs: accept quality gates contract`; Phase 3 integration starts only after the acceptance commit and ends with `feat: integrate quality gates baseline`; Phase 4 is a persistent post-completion user-return loop committing `docs: revise quality gates user return revision N` for later user changes. Core injects each child-plan task pair before provider-visible feedback, including repair attempts with tracked evidence under `.codeai-hub/<slug>/quality_gates/attempts/`. Any final validated split-integration task `quality-gates.phase3.integration.taskN` opens the Phase 4 idle anchor instead of another generic Phase 3 continuation.
...
- **Phase 2 — User-led review (Type B).** The user drives the conversation. Phase 2 turns are classified diff-based: any tracked owned diff is a revision (Core injects a `phase2.review.revisionN.task1 + Git Commit` pair before the open-ended review task and the managed commit boundary fires `docs: revise application skeleton contract — revision N`); a turn without an owned diff is a discussion / no-op recorded only in session history. Phase 2 ends with the explicit accept commit `docs: accept application skeleton contract` (acceptance commit policy: **Option C**). Internal classifier value: `phase_2_review`.
- **Phase 3 — Core-led materialization (Type A).** The materialization continuation prompt fires when the read-model snapshot reports `accepted: true` on `application-skeleton-map.json`. Internal classifier value: `phase_3_materialization`.

**Acceptance Commit Policy: Option C (Core observes map.json).** Any path that sets `accepted: true` in `application-skeleton-map.json` triggers the Phase 2 accept commit and Phase 3 continuation: (1) the Core accept-contract command handler reached via the Project Manager `Accept Contract` button at `/api/v1/orchestrator/managed-stage-accept-contract` (POST); (2) the typed-fallback acceptance phrase router, broadened to match bare Russian verbs (`принимаю`/`подтверждаю`/`утверждаю`) and bare English acceptance verbs (`accept`/`accepted`/`confirm`/`confirmed`/`approve`/`approved`) without requiring the noun `контракт`, and to reject negated forms (`не принимаю …`, `not accepted`, `don't accept`, `cannot confirm`); (3) the agent self-set per its Phase 2 prompt after the user explicitly accepts in chat. The Phase 3 continuation dispatcher (`application-skeleton-continuation-dispatcher.ts::sendApplicationSkeletonContinuationIfReady`) gates on `progress.accepted === true` plus `progress.substep !== "artifact"` and `!progress.materialized`. The legacy `recentlyAcceptedSessions` marker stays as an optional hint for the Core handler's own write path, no longer an exclusive gate. The PM Accept Contract button remains as a UI shortcut, not the exclusive entry point. Core feedback dispatches (`workflow-agent-acceptance-feedback.ts`) send plain-string payloads (no `userMessageVisibility: "deferred"`), so the codex-cli session jsonl records them as `role: "user"` entries and PM transcript shows what Core told the agent — symmetric with the Diagram Modules continuation dispatcher.

A premature-materialization validator derives the blocked-path set from the Application Skeleton map (`materializedPaths` plus every `codePath` in the productParts tree) and runs from both Phase 1 and Phase 2 post-turn guards: any owned write inside the materialization scope before acceptance produces a single corrective `repair_premature_materialization` turn at the readiness + terminal boundary; the block lifts the moment `accepted: true` is recorded. The decision is derived from the skeleton map, never from a hardcoded `product-parts/**` glob. The materialization validator normalizes `materializedPaths` entries (trims whitespace, strips trailing slashes, deduplicates) before checking filesystem existence, so noisy-but-real path lists from the agent do not raise spurious validation errors.

**Phase B / acceptance / materialisation runtime contract (superseded by accepted managed lifecycle).** Older Phase 10 notes described the first acceptance bridge. The current contract is the Phase 1/2/3/4 lifecycle above: acceptance can arrive through the PM Accept Contract command, typed acceptance fallback, or the agent updating the accepted field after explicit user approval; Core then commits acceptance and dispatches the next Core-led materialization/integration prompt from post-turn state. `Application Skeleton` materialization ends with `feat: materialize application skeleton`; `Quality Gates Baseline` integration ends with `feat: integrate quality gates baseline`. Both steps then open a persistent user-return phase.
...
Lifecycle этого шага является прямым аналогом `Application Skeleton`: Core-gated draft contract, user-led review, explicit Core-owned acceptance commit, Core-led integration, then post-completion user-return revision phase. Integration не начинается из draft/review turn-а и не начинается от одного только `accepted: true`; сначала должен существовать commit `docs: accept quality gates contract`, после чего Core создаёт integration microtask и paired `Git Commit:`. Если Integration требует несколько managed commits, final validated `quality-gates.phase3.integration.taskN` открывает `Phase 4 - Persistent Quality Gates User Return`; новый generic Phase 3 continuation после финальной проверки недопустим.
...
- **Phase 4 - User-return revisions:** после accepted integration Core создаёт idle user-return anchor, и шаг остаётся доступен для будущих пользовательских правок; каждая такая правка получает concrete revision task pair and commit, while Development Tree handoff remains a separate workspace lifecycle transition.
