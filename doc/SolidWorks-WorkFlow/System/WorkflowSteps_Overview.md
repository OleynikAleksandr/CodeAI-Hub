...
Workflow-specific mutable runtime state lives inside the workspace-owned runtime capsule. Rollback-relevant Core session state is committed directly by Git under `.codeai-hub/<workspaceSlug>/runtime/sessions/unified/**`, along with workflow state, accepted artifacts and other recovery-critical capsule files. `.codeai-hub/<workspaceSlug>/runtime/settings/settings.json`, `.codeai-hub/<workspaceSlug>/runtime/localization/**`, and provider-native session logs under `.codeai-hub/<workspaceSlug>/runtime/providers/**/home/sessions/**` are active workspace runtime but ignored by workflow rollback and removed from legacy indexes; reproducibility comes from committed `Session.modelBinding`, applied turn config, Core logical sessions and accepted artifacts rather than rolling live settings/localization/native logs backward. Clear runs the selected boundary rollback as a normal Git transaction (`reset --hard` plus `clean -fd`), so downstream unified sessions, scaffold files, workflow state, and accepted artifacts disappear only because they are tracked or untracked workflow files inside the workspace tree. Global auth/secret bridges, installed provider binaries, release caches, generic logs, legacy localization caches, and browser caches remain outside rollback truth; Project Manager saves settings/localization changes through Core into the active workspace runtime without adding them to workflow history.
