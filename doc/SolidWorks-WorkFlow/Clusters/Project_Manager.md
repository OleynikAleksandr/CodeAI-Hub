...
- **Development Tree operation projection**: Project Manager never scans `.codeai-hub` or `product-parts` to derive branch truth. It renders only the Core-owned `developmentTree` snapshot. The active module projection is compact: Product Part / Cluster / Module rows stay in the left tree, while module workflow details (`Facade Contract`, `Module Specification`, `Implementation TODO Plan`, worker progress, semantic integration) belong to the selected module's right-panel artifact/work surface. Module phase/operation rows such as `Module / Facade Specification`, `Implementation`, `Workers`, and `Integration` are not projected under module nodes. `artifactWorkspacePath` points to Core-owned Development Tree artifact folders; `codeWorkspacePath` is shown/used only when Core exposes it from a materialized Application Skeleton map. Non-module operation rows route through the same branch selection/session surfaces as other branch nodes when Core explicitly exposes them.
