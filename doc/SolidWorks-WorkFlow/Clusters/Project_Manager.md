...
- **Workflow Tree provider tint** (1.2.106, idle-neutral 1.2.107, branch-neutral 1.2.108): каждая строка sidebar-дерева (trunk Documentation Tree + Development Tree branch nodes) получает `data-provider="claude" | "codex" | "gemini"` атрибут от `useStepProviderResolver`, который читает `WorkflowStateSnapshot.continuity.chains` и `description.primarySession.providerId`. **Branch nodes (P/C/M) v1 всегда возвращают `null`** — `forBranchPart/forBranchCluster/forBranchModule` не наследуют провайдера от `diagram_modules` chain; они остаются нейтральными до тех пор, пока для конкретного P/C/M не появится своя per-branch session (`Cluster Design` / `Module Design`). **Idle trunk stage без continuity attribution также возвращает `null` от resolver'а**, и `workspace-tree.tsx` омитит `data-provider` атрибут — такая строка рендерится в нейтральном `--pm-text-primary` без provider tint. Это исправляет две регрессии v1.2.106: cyan-default для idle trunk stages и trunk-inheritance для branch nodes. CSS-схема в `packages/ui/project-manager/styles.css` блок `.pm-tree__item[data-provider]` определяет `--row-accent`/`--row-fill`/`--row-fill-hover`/`--row-border`/`--row-soft` per provider; значения зеркалят `doc/SolidWorks-WorkFlow/DesignSystem/CorporateDesign.html`. Замены legacy hardcodes: in-progress `pm-tree__type-marker` (`#d9a441` → `var(--row-soft)`), done marker (`--pm-accent-strong` → `var(--row-border)`), has-children outline (`--pm-accent-strong` → `var(--row-accent)`), PP frame border (`--pm-accent-strong` → `var(--row-border)`), cluster connector lines (`--pm-accent-strong` → `var(--row-soft)`). Selected row: `--row-fill` + `--row-border` + текст `#cfcfcf`. Sidebar font-weight: `300` (light). См. SystemArchitecture §3 Invariant 36.
