...
- **Runtime factory callback wiring**: `createSessionRequestHandlerRuntimeCore` держит циклические callback-dependencies (`messageDispatch`, `sessionResolution`) через явные deferred refs. Continuity callbacks должны вызываться только после полной сборки runtime core; если порядок будет нарушен, factory обязан падать явной initialization error вместо скрытого `undefined` access.
