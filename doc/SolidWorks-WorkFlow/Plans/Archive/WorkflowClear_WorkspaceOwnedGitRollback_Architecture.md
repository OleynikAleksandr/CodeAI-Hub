...
These are workflow-specific and must be owned by the workspace capsule. Files that are rollback truth are controlled by the workspace Git repo; live mutable runtime files are ignored by rollback Git but still live under the workspace capsule.
...
| `~/.codeai-hub/providers/codex/home/**` | `.codeai-hub/<slug>/runtime/providers/codex/home/**` | Codex workspace provider home. The complete provider home is live mutable runtime and rollback-ignored; Core logical sessions, applied config and accepted artifacts carry rollback/recovery truth. Auth/secret files are ignored or provided by a global auth bridge. |
| `~/.codeai-hub/providers/claude/home/**` | `.codeai-hub/<slug>/runtime/providers/claude/home/**` | Claude workspace provider home. The complete provider home is live mutable runtime and rollback-ignored; Core logical sessions, applied config and accepted artifacts carry rollback/recovery truth. Auth/secret files are ignored or provided by a global auth bridge. |
| `~/.codeai-hub/providers/kimi/home/**` | `.codeai-hub/<slug>/runtime/providers/kimi/home/**` | Workspace-specific Kimi managed-agent state and generated runtime profile files. The complete provider home is live mutable runtime and rollback-ignored. |
| No current `~/.codeai-hub/providers/gemini/home`; current Gemini writes to `~/.gemini/**` | `.codeai-hub/<slug>/runtime/providers/gemini/home/.gemini/**` | Gemini needs a new per-workspace home. `~/.codeai-hub/providers/gemini/cli` stays global. Gemini settings, temp, history, chats, memory, commands, skills and policies must resolve under the workspace provider home for workflow sessions, and the complete workspace provider home remains rollback-ignored. |
...
| `~/.codeai-hub/localization/**` workspace-generated overlays | `.codeai-hub/<slug>/runtime/localization/**` | Workspace effective localization bundles/glossary/bootstrap cache follow workspace settings, are Project Manager runtime truth, and are rollback-ignored. Global localization data is seed/compat only, not live workspace truth. |
...
| `~/.codeai-hub/localization/catalogs/**`, `localization/cache/**`, `localization/metadata.json` | Legacy/seed/compat localization data only. Project Manager live localization bundles/cache for an opened workspace must resolve under `.codeai-hub/<slug>/runtime/localization/**`. |
...
Workspace provider homes are live mutable runtime. Provider config files, native histories, caches and provider-native session logs under `runtime/providers/**/home/**` must not be committed as workflow rollback truth. Provider homes must not commit secrets.
...
- Ignore complete workspace provider homes such as `runtime/providers/codex/home/**`; Core logical sessions, applied config and accepted artifacts are the recovery contract.
- Use provider-specific ignore rules and tests to prove that Core logical sessions/accepted artifacts remain tracked while provider homes, secrets and caches are not.
...
9. Route workspace capture artifacts and workspace localization overlays into `runtime/logs` and `runtime/localization` where they affect workflow behavior; keep workspace localization runtime rollback-ignored.
...
- **Secrets vs rollback state:** provider homes must not commit auth, caches, config files or provider-native session logs. `.gitignore` must be tested so Core logical sessions and accepted artifacts remain rollback-owned while provider homes stay outside workflow history.
...
- **Mutable runtime rollback exclusion:** workspace settings, workspace localization runtime and complete provider homes must be ignored by capsule Git, migrated out of legacy indexes with `git rm --cached`, and kept from dirtying Clear/Undo. Workflow reproducibility must come from committed binding/applied-config/Core-session/accepted-artifact evidence, not from rolling live mutable runtime back.
...
- **Session retention split:** old Core unified sessions after a cleared boundary are rollback-owned and must disappear through Git reset/clean because they live in the capsule. Provider homes are diagnostic/runtime state and may remain ignored; they must not be used as rollback truth or leave Git dirty.
...
- **Mutable runtime rollback semantics:** model/workflow settings, workspace localization and provider homes are workspace-owned live state and survive workflow Clear/Undo. Accepted steps must commit immutable applied model/config/Core-session evidence so the workflow timeline remains auditable without tracking mutable runtime files.
