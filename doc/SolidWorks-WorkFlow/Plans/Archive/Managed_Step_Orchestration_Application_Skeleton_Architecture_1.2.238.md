...
> Phase orchestration pilot (shipped, acceptance flow refactor — Phase 16). The runtime models the stage as `Phase 1 → Phase 2 → Phase 3`: Core-gated draft, user-led review with diff-based revision-vs-discussion classification and per-revision managed commits, Core-led materialization. Type A / Type B is a domain attribute describing phase ownership (Core-gated vs user-led), not a phase number suffix. A premature-materialization validator derives blocked paths from the skeleton map and runs from the Phase 1 / Phase 2 post-turn guards; it normalizes `materializedPaths` (trim, strip trailing slashes, dedupe) before filesystem-existence checks. Acceptance Commit Policy: **Option C** — any path that sets `accepted: true` in `application-skeleton-map.json` (Core accept-contract handler via PM button, typed-fallback recognizer, or agent self-set per Phase 2 prompt) is observed by the Phase 3 continuation dispatcher and triggers the explicit Phase 2 commit `docs: accept application skeleton contract` plus the Phase 3 materialization continuation. The legacy `recentlyAcceptedSessions` marker is kept as an optional hint, not the exclusive gate. The typed-fallback recognizer accepts bare Russian verbs (`принимаю`/`подтверждаю`/`утверждаю`) and bare English acceptance verbs (`accept`/`accepted`/`confirm`/`confirmed`/`approve`/`approved`) without requiring the noun `контракт`, with explicit negation handling for `не …` / `not …` / `don't …` / `cannot …`. Core feedback dispatches go through the visible plain-string path (the same as Diagram Modules), so PM transcript records what Core told the agent. Read-model paths remain side-effect free for provider messages — only the post-turn arbitration boundary may dispatch corrective turns.

> Managed orchestration refinement (current release candidate). The Application Skeleton child plan is now dynamic: bootstrap seeds only the draft task and its paired commit, draft commit opens review, acceptance commit opens materialization, and materialization commit opens the post-completion user-return revision loop while the workspace ledger hands off to Quality Gates. Core rejection is tracked as first-class repair task pairs; repeated failed attempts write tracked evidence and are committed, so failed turns are not lost from Git history. Quality Gates child plans are created before handoff, and user-return revisions use concrete `revisionN` task pairs with a paired commit before the next revision opens.
