...
- Lifecycle state: archived after user acceptance of release `1.2.142`; closeout archive is `doc/TODO/Archive/todo-plan-closeout-development-tree-materialization-implementation-2026-05-04.md`.
- User acceptance: Development Tree materialized structure, Project Manager sidebar readiness, selected node sessions, selected node artifacts, scoped node prompt context, chat language, and draft artifact language were confirmed by user retest on 2026-05-05.
- Follow-up disposition: workflow-wide prompt language contract and inline source prompt inputs are adjacent future work, not part of this closed materialization scope; they require a separate planning/todo cycle.
...
- Retest follow-up `1.2.140`: user confirmed the session polish fixes but reported that new Development Tree node agents still start from an empty local context and ask what Project Manager is, even though this is already described upstream. The first node-agent prompt now includes bounded existing-workflow artifact context from `Final_Description.md`, `virtual-simulation.md`, `diagram_modules/product-parts.index.md`, and the selected Product Part artifact (`diagram_modules/product-parts/<part-id>.md`) when those files exist in the workspace. The prompt explicitly treats these artifacts as prior context and instructs the agent not to ask the user to re-explain information already present there. Targeted evidence: `npx tsx --test packages/core/src/development-tree/node-bootstrap/node-first-message-builder.test.ts packages/core/src/development-tree/node-bootstrap/node-agent-session-bootstrapper.test.ts`.
- Retest follow-up `1.2.141`: user reported that the bounded upstream context was still too broad because it effectively pasted large `Final_Description.md` / `virtual-simulation.md` content into every Development Tree node prompt, and cluster/module draft artifacts were written in English despite `Settings > General > Artifacts for the User` being Russian. The fix adds deterministic `NodePromptContextExtractor` scoping by Product Part / Cluster / Module anchors across Description, Virtual Simulation, Diagram Modules index, and the selected Product Part file; the node-agent prompt now receives only matching excerpts. The same first prompt now separates response language (`Settings > General > Reasoning`) from draft artifact language (`Settings > General > Artifacts for the User`) and instructs agents to localize descriptive prose inside draft `agent-fill` sections while preserving canonical ids, filenames, frontmatter, generated blocks, DSL/contract markers, and structural labels. Targeted evidence: `npx tsx --test packages/core/src/development-tree/node-bootstrap/node-first-message-builder.test.ts packages/core/src/development-tree/node-bootstrap/node-agent-session-bootstrapper.test.ts packages/core/src/development-tree/node-bootstrap/node-prompt-context-extractor.test.ts`.
