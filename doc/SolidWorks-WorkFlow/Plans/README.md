...
4. После завершения реализации planning-док обязан пройти один из путей:
   ...
   - быть заменён короткой `compat/redirect`-нотой, если сам путь нужен только для старых ссылок из session reports или archived TODO;
   - быть удалённым, если это был временный refactoring/cleanup design и все живые SSOT уже синхронизированы.
5. `Contracts/`, `Modules/`, `Clusters/`, `System/` содержат только реализованные и актуальные документы, плюс минимальные compat-redirect notes для старых ссылок.
