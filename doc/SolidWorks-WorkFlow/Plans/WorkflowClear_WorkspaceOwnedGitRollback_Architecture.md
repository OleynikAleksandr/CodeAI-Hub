...
| `~/.codeai-hub/providers/codex/home/**` | `.codeai-hub/<slug>/runtime/providers/codex/home/**` | Codex workspace provider home. Provider-native session logs under `home/sessions/**` are live mutable runtime and rollback-ignored; Core logical sessions and accepted artifacts carry rollback/recovery truth. Auth/secret files are ignored or provided by a global auth bridge. |
| `~/.codeai-hub/providers/claude/home/**` | `.codeai-hub/<slug>/runtime/providers/claude/home/**` | Claude workspace provider home. Provider-native session logs under `home/sessions/**` are live mutable runtime and rollback-ignored; Core logical sessions and accepted artifacts carry rollback/recovery truth. Auth/secret files are ignored or provided by a global auth bridge. |
...
Workspace provider homes can contain rollback-relevant provider configuration/state, but provider-native session logs under `runtime/providers/**/home/sessions/**` are live mutable runtime and must not be committed as workflow rollback truth. Provider homes must not commit secrets.
...
- Do not ignore broad provider directories such as `runtime/providers/codex/home/**`, because that would hide potentially rollback-relevant provider state. Do ignore provider-native session logs (`runtime/providers/**/home/sessions/**`), secrets and caches; Core logical sessions and accepted artifacts are the recovery contract.
- Use provider-specific ignore rules and tests to prove that Core logical sessions/accepted artifacts remain tracked while provider-native session logs, secrets and caches are not.
...
- **Secrets vs rollback state:** provider homes must not commit auth, caches or provider-native session logs. `.gitignore` must be narrow and tested so Core logical sessions and accepted artifacts remain rollback-owned while native provider runtime logs stay outside workflow history.
...
- **Mutable runtime rollback exclusion:** workspace settings, workspace localization runtime and provider-native session logs must be ignored by capsule Git, migrated out of legacy indexes with `git rm --cached`, and kept from dirtying Clear/Undo. Workflow reproducibility must come from committed binding/applied-config/Core-session/accepted-artifact evidence, not from rolling live mutable runtime back.
...
- **Session retention split:** old Core unified sessions after a cleared boundary are rollback-owned and must disappear through Git reset/clean because they live in the capsule. Provider-native session logs are diagnostic/runtime state and may remain ignored; they must not be used as rollback truth or leave Git dirty.
...
- **Mutable runtime rollback semantics:** model/workflow settings and workspace localization are workspace-owned live state and survive workflow Clear/Undo. Provider-native session logs are likewise live mutable runtime, not workflow truth. Accepted steps must commit immutable applied model/config/Core-session evidence so the workflow timeline remains auditable without tracking mutable runtime files.
