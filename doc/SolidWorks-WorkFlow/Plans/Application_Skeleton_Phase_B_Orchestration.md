...
7. **Phase 1A corrective prompt builder placement.** Decision ownership for Phase 1A and Phase 1B corrective is fixed inside `managed-workflow-post-turn-service.ts` (see Existing Code Inventory and Implementation Surfaces). The remaining choice is purely cosmetic: should the corrective message text live in a small dedicated pure prompt-builder file (e.g. `application-skeleton-phase-1a-corrective-prompt.ts` — easier to test the prompt copy in isolation) or inline in the post-turn service until it grows beyond a comfortable size?
