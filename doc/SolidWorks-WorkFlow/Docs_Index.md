...
- `Plans/DiagramModules_InitialAutolayout_OverlapAwarePacking_Architecture.md` — active corrective scope after release `1.1.911`; replaces exact-column initial packing with overlap-aware measured sibling packing for wide clusters and standalone modules inside the same product part before the next release build.
