...
- `Plans/DiagramModules_InitialAutolayout_HierarchicalPacker_Architecture.md` — active corrective scope after release `1.1.910`; separates seed autolayout from persisted sidecar mode, replaces repair-style first-open layout with a measured-first hierarchical packer, and adds a fixed-point gap-validation loop for `Cluster` and `Product Part` boundaries.
