...
- `Plans/Codex_FinalSummary_Reasoning_Rendering_Architecture.md` — active planning-doc для нового Codex bugfix scope: переносит user-facing reasoning emission с live readable chunking на completed `item.summary[]` block emission и отдельно фиксирует follow-up retest для standalone bold heading spacing в Session UI.
