...
- `Plans/DiagramModules_SharedVisualBounds_And_ManualAutolayout_Architecture.md` — active corrective scope after release `1.1.909`; replaces duplicated auto/manual boundary math with one shared visual-bounds contract so `Cluster` and `Product Part` lower bounds derive from the deepest direct child visual bottom in both layout paths.
