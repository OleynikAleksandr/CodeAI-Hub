...
- `Plans/Codex_AppServer_Module_Architecture.md` — active planning-doc для нового параллельного Codex provider module на базе `codex app-server`; фиксирует migration strategy через существующий provider-loader seam (`CODEX_MODULE_PATH` / `providers/codex/latest`), сохранение внешнего `codexCli` contract, immediate-binding requirement для app-server `threadId`, и замену legacy `codex-sdk` / rollout-tail transport на JSON-RPC app-server runtime без user-visible второго провайдера.
