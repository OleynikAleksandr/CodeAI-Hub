...
- `Plans/HaikuTranslation_PostReleaseBugfixes_Architecture.md` — активный accepted planning-doc для post-release repair scope Claude Haiku translation после релиза `1.1.986`; закрывает fail-closed runtime wiring, Core-only localization path, provider-native JSONL persistence и diagnostics truth-path.
