...
- `Plans/DiagramModules_Sidecar_v2_LayoutParams_Architecture.md` — completed planning-doc для release `1.1.922` (Phase 1); sidecar schema v2 с `layoutParams` section (per-ProductPart `columns`/`targetAspectRatio`, per-Cluster `moduleColumns`), backwards compat с v1, load path `applyFlowSidecarLayoutParams`, persist path через shell `onNodesChange`. Итоговые инварианты синхронизированы в `SystemArchitecture.md §6.2/§6.4` и `Clusters/Project_Manager.md §3`; planning-doc остаётся в active `Plans/` до phase closeout следующей сессии.
- `Plans/DiagramModules_Projection_Naming_And_Archive_Compression_Architecture.md` — completed planning-doc для Phase 3 (Session025); косметический rename `react-flow` adapter naming → `projection` + zip-компрессия архивных директорий. Планирующие выводы также остаются до phase closeout следующей сессии.
...
- `Plans/Archive.zip` + `Plans/Archive.README.md` — сжатый архив 77 исторических planning-документов (Session023+ rollback волны Diagram Modules autolayout, Foundation Envelope история, локализационные волны, PM hotfixes `1.1.899–1.1.916`, рефакторинг рантайма и т.п.), compressed в Phase 3 cleanup (Session025, 2026-04-09). Для доступа к конкретному плану — см. инструкции в `Archive.README.md`. Git history для каждого архивного документа сохранена и доступна через `git log --all --follow`.
