...
- `Plans/Foundation_Envelope_VisualProjection_Architecture.md` — design intake для visual wave шага `Foundation Envelope`; формализует React Flow projection, runtime-owned sidecar `foundation-envelope.flow.json`, projection-friendly markdown contract и обязательное обновление agent instructions/prompt.
