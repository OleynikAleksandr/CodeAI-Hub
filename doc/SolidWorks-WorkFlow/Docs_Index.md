...
- `Plans/Codex_AppServer_LiveReasoning_And_SDK_Log_Architecture.md` — active planning-doc для bugfix scope после релиза `1.2.22`; фиксирует, как довести `packages/Codex_AppServer_Module` до real-time reasoning stream через app-server delta notifications и вернуть file-backed SDK/app-server transport log в `~/.codeai-hub/logs/codex` без отката к legacy rollout как canonical path.
