...
- Live stream: входящие события после всех фильтров/нормализации/дедупа дают каноническое UI-сообщение; этот же канонический результат отображается в UI и записывается в накопительный `<dialogId>.jsonl`.
