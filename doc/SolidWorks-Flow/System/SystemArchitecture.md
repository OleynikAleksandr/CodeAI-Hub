...
Node.js сервис (`@codeai-hub/core@1.1.610`), упакованный как JS-бандл + официальный Node 20 runtime.
...
- Tokens label formatting: remove `/<limit>` from the status row; show `Tokens: <used> (<percent>%)` and widen the visual separator between Models and Tokens.
...
Phase 128 launcher drag-and-drop recovery (same Session input path):
...
Phase 130 launcher drag-and-drop native file-drop bridge (same Session input path):
...
- Gemini CLI sandbox: `includeDirectories` включает `workspacePath` + `~/.codeai-hub/templates`, чтобы tool-calls (`read_file`/`write_file`) могли работать с workspace артефактами (например, `.codeai-hub/<workspace>/description/**`).
...
**Phase 141 legacy template cleanup:**
- Extension activation больше не устанавливает legacy `idea-collector` шаблоны (`full-development-flow/idea/*`) в `~/.codeai-hub/templates`.
- На чистой установке остаётся только актуальный набор runtime шаблонов (`description/*`, `virtual_simulation/*`, `diagram_*/*`, `flow/continuity/*`).
...
~/.codeai-hub/
├── core/
│   └── darwin-arm64/1.1.610/
│       ├── node/
│       ├── app/
│       └── install.json
├── packages/
│   ├── launcher/macos-arm64/1.1.610/
│   └── ui/
│       ├── vscode-webview/
│       │   ├── 1.1.551/
│       │   └── current -> 1.1.610
...
└── releases/
    ├── CodeAIHubLauncher-macos-arm64-1.1.610.tar.bz2
    ├── vscode-webview-1.1.610.tar.bz2
    ├── project-manager-1.1.610.tar.bz2
    ├── claude-module-1.1.610.tar.bz2
    ├── codex-module-1.1.610.tar.bz2
    ├── gemini-module-1.1.610.tar.bz2
    └── codeai-hub-core-darwin-arm64-1.1.610.tar.bz2
```
...
- Эти JSONL файлы используются для восстановления **истории диалога в UI** (канон: `dialog:history` по `dialogId`; legacy: `/api/v1/sessions/:sessionId/history`) и являются независимыми от provider-home файлов CLI.
- `providerSessionId` остаётся provider-native id для resume/связки событий, но имя файла истории должно быть стабильным `dialogId` (1.1.585+), иначе при rollover/resume история распадается на сегменты и после рестарта Core UI показывает только “последний кусок”.
- В провайдерах возможна promotion схемы `temp id -> real providerSessionId` (например, Codex `codex-<uuid> -> <thread_id>`). Для сессий без `dialogId`-override unified-session writer может выполнять rename файла, чтобы история оставалась в одном JSONL.
- Требование на будущее: для всех **следующих агентов** (и новых flow-ноды/шагов) Core обязан выделять `dialogId` и писать unified-session историю в один накопительный JSONL.
...
| Core | 1.1.610 |
| UI Bundles | 1.1.610 |
| Claude Module | 1.1.610 |
| Codex Module | 1.1.610 |
| Gemini Module | 1.1.610 |
