...
2. UI хранит отложенные привязки: `session-store` теперь буферизует binding, пришедший до создания снапшота; требуется дальнейшая работа, чтобы Info Panel обновлялась без смены фокуса.
3. Выпущен релиз `v1.1.112`: README/CHANGELOG обновлены, SystemArchitecture синхронизирована, core пересобран до `0.2.28`, VSIX `codeai-hub-1.1.112.vsix` положен в корень.

---

## Текущее состояние
- Info Panel всё ещё отображает сообщение ожидания (`Waiting for provider session ID…`) до смены фокуса, несмотря на полученный `sessionId`.
- `~/.codeai-hub/releases/` содержит `codeai-hub-core-darwin-arm64-0.2.28.tar.bz2`, в репозитории — `codeai-hub-1.1.112.vsix`.
...
- Info Panel по-прежнему требует смены фокуса, чтобы обновить текст с ожидания на реальный `sessionId` после подтверждения провайдера — необходимо найти первопричину и устранить.
