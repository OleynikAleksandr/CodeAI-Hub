...
**Время:** 12:15 – 13:20
**Ветка:** main
**Версии:** 1.1.106 → 1.1.107

---

## Артефакты, обязательные к изучению
- `README.md` (Current Release — v1.1.107)
- `CHANGELOG.md` (entries up to 1.1.107)
...
3. Выпущен релиз `v1.1.107`: README/CHANGELOG обновлены, SystemArchitecture синхронизирована, core пересобран до `0.2.25`, VSIX `codeai-hub-1.1.107.vsix` положен в корень.

---

## Текущее состояние
- Info Panel показывает только подтверждённые `providerSessionId`; временные UUID не появляются.
- `~/.codeai-hub/releases/` содержит `codeai-hub-core-darwin-arm64-0.2.25.tar.bz2`, в репозитории — `codeai-hub-1.1.107.vsix`.
