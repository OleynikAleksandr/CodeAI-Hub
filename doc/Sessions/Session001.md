...
- Moved `doc/SolidWorks-WorkFlow/WorkflowSteps_Overview.md` into `doc/SolidWorks-WorkFlow/System/WorkflowSteps_Overview.md` and rewired the live references to treat it as a system-level SSOT instead of a root-level stray document.
- Synced the previously completed `Session053` report, the active placeholder `doc/TODO/todo-plan.md`, and the archived `1.1.898` TODO plan with the new recovery/restart protocol before the archive reset.
- Archived the previous active session block `Session001–Session053` into `doc/Sessions/Archive/Original/Reset_2026-04-06.zip` and promoted the previous `Session054` into the new active `doc/Sessions/Session001.md`.
- Preserved the user-driven archival of the legacy `doc/TODO/Archive` payload as `Archive.zip`.
...
- `d15b37d4ddd4 docs(process): reset active session numbering`
- `12e4efc84 docs(process): archive legacy todo archive`
- `11efb1374 docs(workflow): relocate workflow steps overview`
