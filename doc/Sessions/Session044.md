# Session 044 — Foundation Envelope tree parity release
...
## Work summary
- Opened a post-release corrective cycle after user validation showed that `Foundation Envelope` did create `foundation-envelope.md`, but the Project Manager workflow tree and right-panel empty state still behaved unlike the mature workflow steps.
- Updated `doc/SolidWorks-WorkFlow/Plans/Foundation_Envelope_Architecture.md` with an explicit post-release tree/session parity contract covering the required `session + artifact` child-node shape, stage-selection sync, and stage-aware empty-state behavior.
- Added a canonical artifact-availability probe for `foundation-envelope.md` and wired `Foundation Envelope` into the same left-tree artifact/session contract used by the mature continuity stages.
- Fixed toolbar/stage click and workspace auto-select parity so the same `Foundation Envelope` artifact/session pair is restored consistently across PM entrypoints.
- Made the shared session empty-state surface workflow-aware and localized for `Foundation Envelope`, so the right panel no longer falls back to Description-specific onboarding copy when Foundation Envelope is active.
...
- `d7be369f8 docs(plan): define foundation envelope tree parity hotfix scope`
- `7ba121752 feat(pm-afe): probe foundation envelope artifact availability`
- `e730bb9a5 feat(pm-afe): add foundation envelope branch nodes`
- `3c655783c fix(pm-afe): sync foundation envelope stage selection`
...
- `a6dcb3d0e docs(release): prepare foundation envelope tree parity notes`
- `08b35c089 build(release): assemble foundation envelope tree parity release`
- `TBD - this commit docs(session): record foundation envelope tree parity release`
...
7. `doc/TODO/Archive/todo-plan-up-to-phase1-foundation-envelope-tree-parity-release-1.1.890-2026-04-05.md`
...
- Keep `Foundation_Envelope_Architecture.md` active only for deferred next waves after this tree/session parity hotfix.
