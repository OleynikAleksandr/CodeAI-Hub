...
3. Обновлены Ultracite/нарратив (6.1.0), документация и README; выпущены новые модули (`claude-module-0.1.9`, `codex-module-0.1.4`, `gemini-module-0.3.8`) и VSIX `codeai-hub-1.1.105.vsix` через `./scripts/build-release.sh`.

---

## Текущее состояние
- `~/.codeai-hub/logs/{claude,codex,gemini}` содержат только финальные jsonl-файлы с фактическими sessionId.
- Provider manifests указывают на свежие тарболы; VSIX 1.1.105 готов в корне репозитория.
