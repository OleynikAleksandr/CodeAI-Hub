...
1. [TODO] Claude: добавить понятие provider-home для Claude (аналогично Codex), и запускать `claude` с `HOME=.../providers/claude/home` (scope: `packages/Claude_Module/*` runner/spawn, provider config; expected commit message: `feat(claude): isolate claude home under provider directory`)
