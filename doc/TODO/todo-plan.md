...
2. [DONE] Git Commit: `feat(codex): add runtime model capability registry` (hash: pending current commit)
3. [TODO] Добавить unit-test runtime registry в Codex module: assert Spark `supportsReasoningSummary === false`, non-Spark `=== true`, reasoning options include `xhigh`, unknown model keeps summary enabled. Scope: 1 файл; commit message: `test(codex): cover runtime model capability registry`.
4. [TODO] Git Commit: `test(codex): cover runtime model capability registry` (hash: TBD)
