...
2. [TODO] Git Commit: `docs: start codex progress guard scope` (hash: TBD)
3. [TODO] Add a short non-terminal progress-update rule to the shared Codex workflow prompt and synced prompt artifact; scope: `packages/Codex_AppServer_Module/src/app-server/codex-workflow-instruction-profile.ts`, `doc/SolidWorks-WorkFlow/Plans/Instruction_Stack_Control_Experiment_Results/codex-instruction-analysis/Codex_My_System_Prompt.md`; expected commit message: `fix: keep codex progress updates non-terminal`
