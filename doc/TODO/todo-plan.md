...
2. [TODO] Git Commit: `refactor(pm): rename description submit service` (hash: TBD)
3. [TODO] Переименовать provider picker текущего `Description` шага в explicit `description-*` naming, сохранив тонкий legacy re-export для существующих импортов на время migration (scope: `src/client/project-manager/components/description/idea-collector-provider-picker.tsx`, `src/client/project-manager/components/description/description-provider-picker.tsx`, `src/client/project-manager/components/description/description-questionnaire-panel.tsx`; expected commit: `refactor(pm): rename description provider picker`).
4. [TODO] Git Commit: `refactor(pm): rename description provider picker` (hash: TBD)
5. [TODO] Переименовать current questionnaire UI/helpers, которые реально обслуживают шаг `Description`, из `idea-*` naming в explicit `description-*` naming при сохранении текущего поведения и обратной совместимости импортов на время migration (scope: `src/client/ui/src/services/idea-questionnaire-template.ts`, `src/client/ui/src/services/description-questionnaire-template.ts`, `src/client/ui/src/components/idea-questionnaire/idea-questionnaire-view.tsx`; expected commit: `refactor(ui): rename description questionnaire template`).
6. [TODO] Git Commit: `refactor(ui): rename description questionnaire template` (hash: TBD)
7. [TODO] Переименовать оставшиеся current questionnaire messaging/view слои в explicit `description-*` naming и переключить current imports с сохранением тонких legacy bridges на время migration (scope: `src/client/ui/src/services/idea-questionnaire-messages.ts`, `src/client/ui/src/services/description-questionnaire-messages.ts`, `src/client/ui/src/components/idea-questionnaire/idea-questionnaire-view.tsx`; expected commit: `refactor(ui): rename description questionnaire messaging`).
8. [TODO] Git Commit: `refactor(ui): rename description questionnaire messaging` (hash: TBD)

### Stream: Remove active compat bridges from current workflow
1. [TODO] Переключить PM pre-submit bootstrap c `/idea-contract` и `stage: "idea"` на explicit `Description` contract/session semantics и убрать `idea -> description` schema remap в current PM flow (scope: `src/client/project-manager/services/description-questionnaire-service.ts`, `src/client/project-manager/components/sessions/session-schema-stage.ts`, `src/client/project-manager/components/sessions/session-message-sender.ts`; expected commit: `refactor(workflow): switch description bootstrap off idea alias`).
