...
- [TODO] Внедрить единый реестр версий core/launcher/CEF/VSIX и форсировать переустановку при расхождении
  - Notes: завести `~/.codeai-hub/state/runtime-registry.json` + `current` указатели; extension и launcher сверяют registry с manifest и реконструируют артефакты (core/launcher/CEF) до запуска UI/ядра; фиксировать номер установленного VSIX.
- [TODO] Перестроить lifecycle ядра: запускать core при появлении первого клиента и останавливать сразу после ухода последнего
  - Notes: убрать 60-секундный grace period, добавить сравнение версии рантайма с `/api/v1/status`, принудительно рестартовать при несоответствии; менеджеры extension/launcher должны синхронно следить за состоянием.
