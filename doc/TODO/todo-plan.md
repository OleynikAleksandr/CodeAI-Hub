...
5. [IN_PROGRESS] Add targeted tests for the Claude `Read` / `Write` / `Edit` tool profile — scope: `packages/Claude_Module/src/sdk/claude-sdk-manager.test.ts`, `packages/Claude_Module/src/diagnostics/claude-native-request-capture-service.test.ts`; verification: `npm run build --workspace=@codeai-hub/claude-module`, direct node tests for both files; expected commit message: `test: cover claude workflow tool allowlist`
