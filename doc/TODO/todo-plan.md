...
2. [DONE] Git Commit: `fix(diagram-workflow): define canonical staged product part templates` (hash: TBD)
...
1. [TODO] Встроить содержимое canonical product-part template прямо в continuation prompt при part turns (hidden continuation), аналогично тому, как index turn уже получает все шаблоны через `appendDiagramPromptAppendix`. Сейчас `buildDiagramModulesContinuationPrompt` отправляет агенту минимальный prompt без шаблона, из-за чего агент полагается на «память» из первого turn и постепенно дрифтует в формате. Нужно: (a) обеспечить доступ к загруженному contract/template content в orchestration hook, (b) встроить canonical product-part template content в каждый continuation prompt для substep `generate_product_part` (scope: `src/client/project-manager/components/sessions/use-diagram-modules-orchestration.ts`, `src/client/project-manager/services/description-submit-service.ts`, `doc/TODO/todo-plan.md`; expected commit: `fix(diagram-workflow): inject canonical template into part turn continuation prompt`).
2. [TODO] Git Commit: `fix(diagram-workflow): inject canonical template into part turn continuation prompt` (hash: TBD)
