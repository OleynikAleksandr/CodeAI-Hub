...
1. [TODO] Добавить типы/store/facade для `.codeai-hub/<workspaceSlug>/runtime/execution-profile.json` и legacy bootstrap существующих workspace без profile (scope: `packages/core/src/workflow/execution-profile/`; expected commit: `feat(core): add workspace execution profile store`).
2. [TODO] Git Commit: `feat(core): add workspace execution profile store` (hash: TBD)
