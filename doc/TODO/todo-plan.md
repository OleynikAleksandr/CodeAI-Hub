...
9. [DONE] Удалить stale compile tail в core, обнаруженный таргетной сборкой после inventory-only cleanup: `http-api-router.ts` больше не должен типизировать DSL validator через несуществующий `parseModuleMapDsl` reference (scope: `packages/core/src/remote-bridge/handlers/http-api-router.ts`; expected commit: `fix(core): remove stale module map parser type reference`).
10. [DONE] Git Commit: `fix(core): remove stale module map parser type reference` (hash: `56160cfd`)
