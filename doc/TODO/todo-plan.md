...
8. [IN_PROGRESS] Git Commit: `feat: add workbench remote command router` (hash: TBD)
9. [TODO] Wire validator: `packages/core/src/remote-bridge/handlers/incoming-message-validator.ts` — accept new workbench intents after main router delegate wiring; validation stays structural and reuses Core workbench type guards where possible (scope: 1 файл; expected commit: `feat: validate workbench state and artifact bridge commands`).
10. [TODO] Git Commit: `feat: validate workbench state and artifact bridge commands` (hash: TBD)
