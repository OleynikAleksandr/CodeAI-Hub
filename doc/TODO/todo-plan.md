...
3. [DONE] Реализовать transaction/debt primitives: `scripts/plan-orchestrator/plan-debt.mjs`, `scripts/plan-orchestrator/plan-transaction.mjs`, `scripts/plan-orchestrator/plan-transaction.test.mjs`; debt file path `.git/codeai-plan-debt`, JSON payload with task id, expected message, pre-commit head and transaction stage. Verification: `node --test scripts/plan-orchestrator/*.test.mjs`; Husky pre-commit passed with existing near-limit warnings only (scope: 3 files; expected commit: `feat: add plan transaction debt state`).
4. [DONE] Git Commit: `feat: add plan transaction debt state` (hash: a4be3c37d)
