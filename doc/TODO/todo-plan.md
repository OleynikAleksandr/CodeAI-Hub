...
1. [TODO] Docs: зафиксировать UX правила (после `Final_Description.md` в ветке остаются только `Final_Description.md` + `Reviewer session`; промежуточные артефакты/сессии показываем только до появления финала; continuity/handoff не отображаем в дереве) — scope: `doc/SolidWorks-Flow/Architecture/DescriptionNode_ReviewSession_Architecture.md`, `doc/SolidWorks-Flow/Architecture/WorkflowTree_UI_Architecture.md`; expected commit message: `docs(project-manager): simplify description branch UX`
2. [TODO] Git Commit: `docs(project-manager): simplify description branch UX` (hash: TBD)

### Stream: Implementation — убрать “зверинец” из дерева Description
1. [TODO] Fix(project-manager): полностью скрыть continuity/handoff chains из Workspace Tree (они нужны ядру, но не UI дерева) — scope: `src/client/project-manager/components/layout/workspace-tree.tsx`; expected commit message: `fix(project-manager): hide continuity nodes in tree`
2. [TODO] Git Commit: `fix(project-manager): hide continuity nodes in tree` (hash: TBD)

3. [TODO] Fix(project-manager): убрать кнопку `Continue`; строка сессии кликабельна и по клику создаёт/возобновляет сессию (чтобы после перезагрузки пользователь мог восстановить reviewer-сессию из persisted координат) — scope: `src/client/project-manager/components/layout/workspace-tree.tsx`; expected commit message: `fix(project-manager): open session on tree click`
4. [TODO] Git Commit: `fix(project-manager): open session on tree click` (hash: TBD)

5. [TODO] Feat(project-manager): клик по `Final_Description.md` открывает встроенный viewer в правой панели Artifacts (не VS Code editor tab) — scope: `src/client/project-manager/components/layout/main-area.tsx`, `src/client/project-manager/components/layout/workspace-tree.tsx`, `src/client/project-manager/api.ts`; expected commit message: `feat(project-manager): open artifacts in built-in viewer`
6. [TODO] Git Commit: `feat(project-manager): open artifacts in built-in viewer` (hash: TBD)

### Stream: Integration — минимальный API для чтения артефактов (безопасно)
1. [TODO] Feat(core): добавить allowlisted API для чтения workflow-артефактов (Project Manager viewer) — scope: `packages/core/src/remote-bridge/handlers/http-api-router.ts`, `packages/core/src/workflow/paths/workflow-artifact-paths.ts`, `src/client/project-manager/services/workflow-state-client.ts`; expected commit message: `feat(core): expose artifact read endpoint for project-manager`
2. [TODO] Git Commit: `feat(core): expose artifact read endpoint for project-manager` (hash: TBD)
