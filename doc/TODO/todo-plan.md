...
1. [TODO] Обновить `README.md` (Current Release → v1.2.10, 1.2.9 → "(previous)") и `CHANGELOG.md` ([1.2.10] секция с Changed/Fixed описывающая audit cleanup) — scope: 2 файла; ожидаемый commit: `docs: prepare 1.2.10 release notes for audit cleanup`
2. [TODO] Git Commit: `docs: prepare 1.2.10 release notes for audit cleanup` (hash: TBD)

### Stream 2: Direction A — Docs + config fix
1. [TODO] Определить актуальную структуру `.codeai-hub/` templates (что реально лежит после bundling). Обновить `doc/SolidWorks-WorkFlow/Docs_Index.md:80-82` на правильные пути (убрать ссылки на несуществующие `description-template.md` / `description-collector-prompt.md` если они консолидированы в `description-step.json`). Проверить `knip.json` exclusion для `packages/core/src/workflow/diagram-dsl/diagram-legacy-ownership-parser.ts` — если файл используется нормально, снять exclusion; если нет — добавить комментарий с причиной. Проверить исходник `packages/agents/spec-creator/src/` (не `dist/.d.ts`) — актуален ли TODO "Implement full contract building logic when assets are ready". — scope: 2-3 файла; ожидаемый commit: `fix: correct Docs_Index template paths + audit knip exclusion and spec-creator TODO`
2. [TODO] Git Commit: `fix: correct Docs_Index template paths + audit knip exclusion and spec-creator TODO` (hash: TBD)

### Stream 3: Direction B dry-run — localization keys partial-usage verification
1. [TODO] Построить список 99 unused ключей из 4 approved dicts (`ui_labels.json`, `ui_helper_text.json`, `messages_for_the_user.json`, `artifacts_for_the_user.json`). Для каждого ключа прогнать 3 вида grep: (a) exact match по полному ключу, (b) partial match по последнему сегменту, (c) partial match по префиксу без последнего сегмента (для template-literal dynamic keys). Сохранить список confirmed-dead в `doc/tmp/audit-loc-keys-1.2.10.md` (git-tracked tmp) с категоризацией: certainly-dead / suspicious-keep / dynamic-ref-found. — scope: 1 файл (новый md); ожидаемый commit: `docs: record 1.2.10 localization cleanup dry-run report`
2. [TODO] Git Commit: `docs: record 1.2.10 localization cleanup dry-run report` (hash: TBD)
