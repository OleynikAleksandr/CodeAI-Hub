...
21. [TODO] Prepare release-facing docs for the persistent localization bootstrap patch so `README.md` and `CHANGELOG.md` already point at the next packaged version before any version bump/build scripts run. Scope: `README.md`, `CHANGELOG.md`. Target commit: `docs(release): prepare persistent localization bootstrap release notes`
22. [TODO] Git Commit: `docs(release): prepare persistent localization bootstrap release notes` (hash: TBD)
23. [TODO] Run `./scripts/build-all.sh`, archive the completed bootstrap planning/todo docs, and commit the version/manifests/release-cache surface on a clean tree before VSIX packaging. Scope: release-generated version files/manifests, `doc/TODO/Archive`, `doc/SolidWorks-WorkFlow/Plans/Archive`. Target commit: `build(release): assemble persistent localization bootstrap release`
24. [TODO] Git Commit: `build(release): assemble persistent localization bootstrap release` (hash: TBD)
25. [TODO] Run `./scripts/build-release.sh --use-current-version`, record the packaged VSIX outcome in `Session033.md`, and finish the session closeout with the released version. Scope: `doc/Sessions/Session033.md`, packaged release verification outputs. Target commit: `docs(session): record persistent localization bootstrap packaged release`
