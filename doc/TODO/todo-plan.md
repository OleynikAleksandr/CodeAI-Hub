...
```json
{
  ...
  "branch": "main",
  "baseHead": "cb93c430b",
  "lastRecordedCommit": "cce6cde1a",
  "planningSource": "doc/SolidWorks-WorkFlow/Plans/Archive/Kimi_2_6_Module_Implementation_Planning_RU.md",
  "currentTaskId": "phase8-kimi-provider-inheritance-release-build",
  "expectedCommitMessage": "chore: release kimi provider inheritance build",
  "debt": {
    "expectedCommitMessage": "chore: release kimi provider inheritance build",
    "preCommitHead": "cce6cde1a",
    "stage": "commit_pending",
    "taskId": "phase8-kimi-provider-inheritance-release-build"
  }
}
```
...
6. [DONE] Git Commit: `fix: wire kimi reasoning toggle in settings` (hash: bbf0f0f45)
7. [DONE] `phase8-kimi-reasoning-core-policy` Подключить Kimi settings persistence, applied turn config и translation visibility policy — scope: `packages/core/src/config/provider-settings-snapshot.ts, packages/core/src/config/provider-turn-config-resolver.ts, doc/TODO/todo-plan.md`; expected commit: `fix: apply kimi reasoning display policy`.
8. [DONE] Git Commit: `fix: apply kimi reasoning display policy` (hash: e7d07fa35)
...
14. [DONE] Git Commit: `fix: stream kimi reasoning chunks earlier` (hash: 068b483a4)
...
4. [DONE] Git Commit: `docs: mark kimi reasoning hotfix release build started` (hash: 101aff7e4)
5. [DONE] `phase8-kimi-reasoning-hotfix-release-build` Выполнить hotfix release checklist: `./scripts/build-all.sh`, `./scripts/build-release.sh --use-current-version`, release artifacts handoff — scope: `package.json, package-lock.json, packages/*/package.json, assets/**/manifest.json, media/react-chat.js, README.md, CHANGELOG.md, doc/TODO/todo-plan.md, doc/tmp/releases/**`; expected commit: `chore: release kimi reasoning hotfix build`.
   - Result: `./scripts/build-all.sh --allow-dirty` passed for version `1.2.311`; `--allow-dirty` used because the active release task kept machine-managed `doc/TODO/todo-plan.md` dirty after post-commit advancement.
...
2. [DONE] Git Commit: `docs: capture kimi default agent baseline` (hash: 9af474c08)
...
4. [DONE] Git Commit: `docs: document kimi managed agent profile` (hash: a2b3fb891)

### Stream: Kimi Managed Agent Profile Release Confirmation Gate
1. [DONE] `phase8-kimi-managed-agent-release-confirmation` Остановиться после managed agent profile фикса и запросить отдельное подтверждение на release build; не готовить release notes/version bump и не запускать release scripts до подтверждения — scope: без изменения файлов; expected commit: none. Result: User confirmed release build on 2026-05-19: «Собери новый релиз».

### Stream: Kimi Managed Agent Profile Release Build
1. [DONE] `phase8-kimi-managed-agent-release-notes` Подготовить README/CHANGELOG под будущую версию 1.2.312 до запуска release scripts — scope: `README.md, CHANGELOG.md, doc/TODO/todo-plan.md`; expected commit: `docs: prepare kimi managed agent release notes`.
2. [DONE] Git Commit: `docs: prepare kimi managed agent release notes` (hash: bddfd2a5b)
3. [DONE] `phase8-kimi-managed-agent-release-build-start` Зафиксировать post-commit advancement перед release scripts — scope: `doc/TODO/todo-plan.md`; expected commit: `docs: mark kimi managed agent release build started`.
4. [DONE] Git Commit: `docs: mark kimi managed agent release build started` (hash: a00f69188)
5. [DONE] `phase8-kimi-managed-agent-release-build` Выполнить hotfix release checklist: `./scripts/build-all.sh`, `./scripts/build-release.sh --use-current-version`, release artifacts handoff — scope: `package.json, package-lock.json, packages/*/package.json, assets/**/manifest.json, media/react-chat.js, README.md, CHANGELOG.md, doc/TODO/todo-plan.md, doc/tmp/releases/**`; expected commit: `chore: release kimi managed agent profile build`.
   - Result: `./scripts/build-all.sh --allow-dirty` passed for version `1.2.312`; `--allow-dirty` used because the active release task kept machine-managed `doc/TODO/todo-plan.md` dirty after post-commit advancement.
...
2. [DONE] Git Commit: `docs: prepare kimi progress summary release notes` (hash: 709d82c4d)
3. [DONE] `phase8-kimi-progress-summary-release-build-start` Зафиксировать post-commit advancement перед release scripts — scope: `doc/TODO/todo-plan.md`; expected commit: `docs: mark kimi progress summary release build started`.
4. [DONE] Git Commit: `docs: mark kimi progress summary release build started` (hash: f82a4de71)
5. [DONE] `phase8-kimi-progress-summary-release-build` Выполнить hotfix release checklist: `./scripts/build-all.sh`, `./scripts/build-release.sh --use-current-version`, release artifacts handoff — scope: `package.json, package-lock.json, packages/*/package.json, assets/**/manifest.json, media/react-chat.js, README.md, CHANGELOG.md, doc/TODO/todo-plan.md, doc/tmp/releases/**`; expected commit: `chore: release kimi progress summary build`.
   - Result: `./scripts/build-all.sh --allow-dirty` passed for version `1.2.313`; `--allow-dirty` used because the active release task keeps machine-managed `doc/TODO/todo-plan.md` dirty after post-commit advancement.
...
4. [DONE] Git Commit: `docs: mark kimi provider inheritance release build started` (hash: cce6cde1a)
5. [DONE] `phase8-kimi-provider-inheritance-release-build` Выполнить hotfix release checklist: `./scripts/build-all.sh`, `./scripts/build-release.sh --use-current-version`, release artifacts handoff — scope: `package.json, package-lock.json, packages/*/package.json, assets/**/manifest.json, media/react-chat.js, README.md, CHANGELOG.md, doc/TODO/todo-plan.md, doc/tmp/releases/**`; expected commit: `chore: release kimi provider inheritance build`.
   - Result: `./scripts/build-all.sh --allow-dirty` passed for version `1.2.314`; `--allow-dirty` used because the active release task keeps machine-managed `doc/TODO/todo-plan.md` dirty after post-commit advancement.
   - Result: `./scripts/build-release.sh --use-current-version --allow-dirty` passed; VSIX created at `codeai-hub-1.2.314.vsix`, package size `49M`.
   - Result: release tarballs available in `~/.codeai-hub/releases/` and `doc/tmp/releases/`: `kimi-module-1.2.314.tar.bz2`, `codeai-hub-core-darwin-arm64-1.2.314.tar.bz2`, `CodeAIHubLauncher-macos-arm64-1.2.314.tar.bz2`, provider/UI tarballs.
   - Result: release validation confirmed Kimi provider bundle loads, Core runtime includes Kimi provider module, SDK/provider module exclusions verified, markdown links OK, duplication check within threshold, VSIX runtime package surface verified.
6. [PENDING] Git Commit: `chore: release kimi provider inheritance build` (hash: TBD)
