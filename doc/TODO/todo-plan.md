...
29. [DONE] Build release with normal version bump; scope: `./scripts/build-all.sh`, `./scripts/build-release.sh --use-current-version`, `doc/tmp/releases/`; expected commit message: `chore: build provider sdk log removal release`; result: `build-all.sh` first exposed stale Claude `dist/logging/sdk-session-logger.*`, provider package builds were fixed to clean `dist`, `build-all.sh --version 1.2.94` regenerated clean tarballs, archive/VSIX scans found no removed SDK logger files, and `build-release.sh --use-current-version` produced `codeai-hub-1.2.94.vsix`
30. [DONE] Git Commit: `chore: build provider sdk log removal release` (hashes: `28df4cd5c`, `ba59c3aec`, `f53b6ab96`; final VSIX packaging produced no tracked source delta)
