...
2. [IN_PROGRESS] Git Commit: `feat: preserve provider usage telemetry for reopened sessions` (hash: TBD)
3. [TODO] Seed-ить новые runtime/dialog snapshots из provider-scoped usage cache и ввести явный pending/unavailable state для usage bar без fake `0%`, сохранив `Resets ...` в скобках для 5-часового и недельного окна при наличии `resetsAt` — scope: `src/client/ui/src/session/helpers.ts`, `src/client/ui/src/session/session-id-bar.tsx`, `src/client/project-manager/components/sessions/dialog-session-bootstrap.ts`; ожидаемый commit message: `feat: seed reopened session usage bars from provider cache`
4. [TODO] Git Commit: `feat: seed reopened session usage bars from provider cache` (hash: TBD)
5. [TODO] Добавить PM regression coverage для replay-before-snapshot, provider-wide seeding, reopened dialog display state и reset labels в скобках для 5-часового/недельного окна — scope: `src/client/project-manager/components/sessions/usage-limits-stream.test.ts`, `src/client/project-manager/components/sessions/project-manager-session-view.test.tsx`, `src/client/ui/src/session/session-id-bar.test.tsx`; ожидаемый commit message: `test: cover reopened session usage telemetry seeding`
6. [TODO] Git Commit: `test: cover reopened session usage telemetry seeding` (hash: TBD)
...
1. [TODO] Уточнить Claude/Codex provider bridges так, чтобы open-time refresh оставался account-scoped и не требовал eager session hydration — scope: `packages/Claude_Module/src/provider/claude-provider-adapter.ts`, `packages/Codex_AppServer_Module/src/provider/codex-provider-adapter.ts`, `packages/core/src/provider-registry/provider-usage-limits-bridge-factory.ts`; ожидаемый commit message: `fix: keep provider usage refresh account scoped on dialog open`
2. [TODO] Git Commit: `fix: keep provider usage refresh account scoped on dialog open` (hash: TBD)
3. [TODO] Синхронизировать SSOT с новым pre-turn contract для reopened dialogs и provider-scoped seeding в PM — scope: `doc/SolidWorks-WorkFlow/System/SystemArchitecture.md`, `doc/SolidWorks-WorkFlow/Contracts/SessionUI_Behavior.md`, `doc/SolidWorks-WorkFlow/Contracts/Dialogs_And_Continuity_Routing.md`; ожидаемый commit message: `docs: define pre-turn usage refresh for reopened dialogs`
4. [TODO] Git Commit: `docs: define pre-turn usage refresh for reopened dialogs` (hash: TBD)
