...
1. [TODO] Обновить bundled templates для `virtual_simulation`: prompt+template должны требовать чтение `Final_Description.md`, задавать до 3 вопросов и поддерживать «бесконечную» сессию (scope: `packages/core/src/templates/bundled-templates.ts`; expected commit: `feat(core): add virtual simulation bundled templates`).
2. [TODO] Git Commit: `feat(core): add virtual simulation bundled templates` (hash: TBD)
