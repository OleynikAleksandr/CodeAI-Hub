...
1. [TODO] Добавить в remote bridge полный write-transport для `Settings`: `settings:save`, `settings:reset`, `settings:saved`, `settings:save-error`, `settings:localization-sync-status`; routing должен вести PM intents в core-owned handler, а не в extension-only webview path. scope: `packages/core/src/remote-bridge/types.ts`, `packages/core/src/remote-bridge/remote-bridge-message-router.ts`, `packages/core/src/remote-bridge/handlers/settings-request-handler.ts`; ожидаемый commit message: `feat(core): add settings write transport`
2. [TODO] Git Commit: `feat(core): add settings write transport` (hash: TBD)
3. [TODO] Перенести persistence/reset/localization save flow из extension-side handler в Core с сохранением selective sync, broadcaster parity и blocking semantics для PM/new session sends. scope: core settings save/reset implementation (`<= 3 файлов`); ожидаемый commit message: `feat(core): own settings persistence and localization sync`
4. [TODO] Git Commit: `feat(core): own settings persistence and localization sync` (hash: TBD)

### Stream B: Provider update + glossary intents
1. [TODO] Перенести provider versions/update orchestration в core-owned settings path и сделать `settings:versions` / `settings:update-provider` доступными PM как runtime contract. scope: provider update/version settings flow (`<= 3 файлов`); ожидаемый commit message: `feat(core): expose provider update actions through settings`
2. [TODO] Git Commit: `feat(core): expose provider update actions through settings` (hash: TBD)
3. [TODO] Добавить core-owned intent/result seam для `open-user-glossary-file`, чтобы PM settings window больше не зависел от extension-only message handler. scope: glossary-open settings flow (`<= 3 файлов`); ожидаемый commit message: `feat(core): add glossary open intent for settings`
4. [TODO] Git Commit: `feat(core): add glossary open intent for settings` (hash: TBD)

## Phase 2 — Project Manager Settings Window (owner: Codex, updated: 2026-04-22)

### Stream C: PM settings transport and state
1. [TODO] Расширить PM websocket contracts/API для `settings:save`, `settings:reset`, `settings:update-provider`, `settings:versions`, `settings:save-error`, `settings:localization-sync-status`; PM должен стать полноценным клиентом core-owned settings flow. scope: `src/client/project-manager/core-stream-message-types.ts`, `src/client/project-manager/api.ts`, `src/client/project-manager/components/settings/use-project-manager-settings.ts`; ожидаемый commit message: `feat(pm): wire settings write transport`
...
1. [TODO] Добавить dedicated PM-owned settings window mode/host, который открывает текущий `SettingsView` как отдельное независимое окно и не трогает state текущих Sessions/Artifacts в основном окне. scope: PM settings window host (`<= 3 файлов`); ожидаемый commit message: `feat(pm): add dedicated settings window`
