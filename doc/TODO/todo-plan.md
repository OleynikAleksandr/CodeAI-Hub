...
3. [IN_PROGRESS] На clean tree запустить targeted verification: `npm test --workspace @codeai-hub/core`, `npm run build --workspace @codeai-hub/core`, `npm run build --workspace @codeai-hub/codex-app-server-module`; если UI lock behavior touched, also `npm run typecheck:webview` (scope: commands only; expected commit: `test: verify codex continuity rollover fix`).
