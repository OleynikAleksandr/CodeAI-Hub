...
1. [IN_PROGRESS] Вынести Documentation Tree rollover regression в focused test file: update `packages/core/src/remote-bridge/handlers/session-request-handler.rollover.test.ts`, add `packages/core/src/remote-bridge/handlers/session-request-handler.documentation-rollover.test.ts`, and keep architecture file-size limit below 500 lines before legacy cleanup (scope: 2 test files; expected commit: `test: extract documentation rollover regression`).
2. [TODO] Git Commit: `test: extract documentation rollover regression` (hash: TBD)
3. [TODO] Очистить Documentation Tree stages from old Create Report/resume eligibility: update `packages/core/src/remote-bridge/handlers/session-request-handler-flow-node-rollover.ts`, `packages/core/src/remote-bridge/handlers/session-request-handler-flow-node-report-state.ts`, and focused `packages/core/src/remote-bridge/handlers/session-request-handler.documentation-rollover.test.ts`; `description`, `virtual_simulation`, `diagram_modules` must no longer enter report-ready/report-wait state after synthetic rollover is selected (scope: 3 files; expected commit: `fix: remove documentation tree report rollover path`).
