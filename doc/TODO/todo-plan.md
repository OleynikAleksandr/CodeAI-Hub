...
2. [DONE] Git Commit: `feat: add codex translation prompt profile` (hash: `765074535`)
3. [DONE] Add provider-owned `CodexAppServerTranslationService` with isolated temporary workspace, `approvalPolicy=never`, `sandbox=read-only`, `persistExtendedHistory=false`, `effort=low`, and strict translation process profile (scope: `packages/Codex_AppServer_Module/src/translation/*`, `packages/Codex_AppServer_Module/src/app-server/process/*`; checks: `npx ultracite check`, `npm run build --workspace=@codeai-hub/codex-app-server-module`, `node --test packages/Codex_AppServer_Module/dist/translation/codex-app-server-translation-service.test.js`, `node --test packages/Codex_AppServer_Module/dist/app-server/process/codex-app-server-process.test.js`, `node --test packages/Codex_AppServer_Module/dist/translation/codex-translation-prompt-profile.test.js`; commit: `feat: add codex app-server translation service`)
4. [DONE] Git Commit: `feat: add codex app-server translation service` (hash: `fc8bb93d9`)
...
4. [DONE] Git Commit: `feat: route native capture through translation profile` (hash: `742485551`)
...
4. [DONE] Git Commit: `docs: close invocation profile planning docs` (hash: `76bcdc7f6`)
5. [DONE] Run targeted builds for Core, Codex module, translation, webview, and Project Manager typecheck/build surfaces; fix any failures in scoped follow-up tasks before release (scope: `packages/core`, `packages/Codex_AppServer_Module`, `packages/translation`, `src/client`; checks: `npm run build --workspace=@codeai-hub/codex-app-server-module`, `npm run build --workspace=@codeai-hub/translation`, `npm run typecheck:webview`, `npm run build --workspace=@codeai-hub/core`, `npm run build:webview`, `npm run build:project-manager`; no fixes required; commit: `fix: stabilize invocation profile release checks`)
6. [DONE] Git Commit: `fix: stabilize invocation profile release checks` (hash: `6c10c916a`)
7. [DONE] Prepare release metadata for future version `1.2.99` before `build-all.sh` (scope: `README.md`, `CHANGELOG.md`, `doc/TODO/todo-plan.md`; checks: `npx ultracite check`; commit: `docs: prepare invocation profile release metadata`)
8. [DONE] Git Commit: `docs: prepare invocation profile release metadata` (hash: `833a9d524`)
...
10. [IN_PROGRESS] Git Commit: `fix: include invocation templates in release generation` (hash: TBD)
11. [TODO] Re-run `./scripts/build-all.sh --version 1.2.99`, run `./scripts/build-release.sh --use-current-version`, move release artifacts as required, archive/close planning docs if the cycle is complete, and create the session closeout report (scope: release docs/scripts output/session report; commit: `chore: build invocation profile release`)
12. [TODO] Git Commit: `chore: build invocation profile release` (hash: TBD)
