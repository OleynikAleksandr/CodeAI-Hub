...
5. [DONE] Обновить `Docs_Index.md` — убрать bullet'ы на оба completed planning-doc'а и обновить описание `Plans/Archive.zip` (уже не «77 документов», а «историческая коллекция, пополняется при каждом closeout»). Scope: `doc/SolidWorks-WorkFlow/Docs_Index.md` (1 файл).
6. [DONE] Git Commit: `docs: point Docs_Index at compacted Plans/Archive.zip after Session025 closeout` (hash: `6b558b1b8`)
