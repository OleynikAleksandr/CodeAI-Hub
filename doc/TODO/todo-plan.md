...
2. [TODO] Git Commit: `docs(plan): start idea collector legacy cleanup scope` (hash: TBD)

### Stream: Rename active PM Description pipeline
1. [TODO] Переименовать живой PM submit/orchestration слой текущего `Description` шага, чтобы active current-flow больше не использовал `idea-collector` naming без изменения поведения (scope: `src/client/project-manager/services/idea-collector-submit-service.ts`, `src/client/project-manager/components/description/idea-collector-provider-picker.tsx`, `src/client/project-manager/components/description/description-questionnaire-panel.tsx`; expected commit: `refactor(pm): rename description submit pipeline`).
2. [TODO] Git Commit: `refactor(pm): rename description submit pipeline` (hash: TBD)
3. [TODO] Переименовать current questionnaire UI/helpers, которые реально обслуживают шаг `Description`, из `idea-*` naming в explicit `description-*` naming при сохранении текущего поведения и обратной совместимости импортов на время migration (scope: `src/client/ui/src/services/idea-questionnaire-template.ts`, `src/client/ui/src/services/idea-questionnaire-messages.ts`, `src/client/ui/src/components/idea-questionnaire/idea-questionnaire-view.tsx`; expected commit: `refactor(ui): rename description questionnaire helpers`).
4. [TODO] Git Commit: `refactor(ui): rename description questionnaire helpers` (hash: TBD)
