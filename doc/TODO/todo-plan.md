...
175. [TODO] Fix(ui): treat `turn_state=idle` (provider `turn_completed`) as the canonical "agent is waiting for user" marker; always clear `Agent is working…` on idle even if UI previously entered `blocked` — scope: `src/client/project-manager/components/sessions/token-usage-stream.ts`, `src/client/ui/src/session/session-view-helpers.tsx`; expected commit message: `fix(ui): clear working banner on turn idle`
176. [TODO] Git Commit: `fix(ui): clear working banner on turn idle` (hash: TBD)
177. [TODO] Feat(core): emit explicit continuity handoff state events to UI (`handoff:start`, `handoff:ready`) as `session:stream` events; do NOT expose as user-facing chat messages — scope: `packages/core/src/remote-bridge/handlers/session-request-handler.ts`, `packages/core/src/remote-bridge/types.ts`; expected commit message: `feat(core): stream continuity handoff state`
178. [TODO] Git Commit: `feat(core): stream continuity handoff state` (hash: TBD)
179. [TODO] Fix(ui): during handoff lock input+send and show the usual working-strip (no new copy/UX); unblock only on `handoff:ready` (new session active) — scope: `src/client/ui/src/session/session-view.tsx`, `src/client/ui/src/session/input-panel.tsx`, `src/client/ui/src/session/session-view-helpers.tsx`; expected commit message: `fix(ui): lock input during continuity handoff`
