...
3. [DONE] На чистом дереве выполнить финальный release pipeline: `./scripts/build-all.sh` -> `./scripts/build-release.sh --use-current-version`; VSIX `codeai-hub-1.2.36.vsix` (2.3 MB) и tarball'ы в `doc/tmp/releases/`
4. [DONE] Git Commit: `build: release 1.2.36` (hash: 59d9155dd)
