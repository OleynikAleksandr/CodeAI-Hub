...
- **Read this context before implementation:**
  ...
  - `doc/SolidWorks-WorkFlow/Plans/Archive/DevelopmentTree_BranchWorkflow_Architecture.md`
...
9. [DONE] `phase7.stream1.task1` Update `DevelopmentTree_BranchWorkflow_Architecture.md` with the Implementation TODO Plan microtask/Git Commit pairing, pre-commit Quality Gates behavior, module-agent commit ownership, worker sandbox/workspace rules, and parallel execution constraints (scope: `doc/SolidWorks-WorkFlow/Plans/Archive/DevelopmentTree_BranchWorkflow_Architecture.md, doc/TODO/todo-plan.md`; expected commit: `docs: add module worker commit policy`).
...
25. [DONE] `phase17.stream1.task1` Update Development Tree documentation so filesystem scaffolding matches the compact tree model: product parts, clusters, modules, and right-panel artifacts instead of operation subfolders (scope: `doc/SolidWorks-WorkFlow/Plans/Archive/DevelopmentTree_BranchWorkflow_Architecture.md, doc/SolidWorks-WorkFlow/System/WorkflowSteps_Overview.md, doc/TODO/todo-plan.md`; expected commit: `docs: sync development tree scaffolding model`).
...
146. [DONE] Git Commit: `docs: prepare release 1.2.447` (hash: 05321721b)

## Phase 78 - Release 1.2.447 Build (owner: Codex, updated: 2026-06-03)

### Stream: Build Artifacts

147. [DONE] `phase78.stream1.task1` Run the release build flow and record the resulting VSIX/tarball artifacts for version 1.2.447 (scope: `doc/TODO/todo-plan.md, package.json, package-lock.json, packages/**/package.json, src/client/ui/package.json, src/client/project-manager/package.json, assets/**/manifest.json, scripts/**, doc/tmp/releases/**`; expected commit: `chore: build release 1.2.447`). Result: `./scripts/build-all.sh --allow-dirty` passed; `./scripts/build-release.sh --use-current-version --allow-dirty` passed; VSIX created at `codeai-hub-1.2.447.vsix`; release tarballs created in `doc/tmp/releases/` and `~/.codeai-hub/releases/`.
148. [DONE] Git Commit: `chore: build release 1.2.447` (hash: 9d04aaae3)

## Phase 79 - User Workflow Acceptance Testing (owner: Oleksandr, updated: 2026-06-03)

### Stream: User Retest

149. [DONE] `phase79.stream1.task1` User installs/tests release 1.2.447 and verifies Clear/Undo removes workflow unified histories plus provider-native session histories after rolling back from downstream steps (scope: `doc/TODO/todo-plan.md`). Result: Release 1.2.447 accepted by user; Clear/Undo now correctly removes workflow unified histories and provider-native session histories after rollback.
