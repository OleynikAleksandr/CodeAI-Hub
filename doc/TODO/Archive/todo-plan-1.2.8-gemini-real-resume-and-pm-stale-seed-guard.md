...
1. [DONE] User wiped `~/.codeai-hub/` перед build; `./scripts/build-all.sh` + `./scripts/build-release.sh --use-current-version` — VSIX `codeai-hub-1.2.8.vsix` (2.3M) + tarballs в `doc/tmp/releases/` и `~/.codeai-hub/releases/`.
2. [DONE] Git Commit: `chore: bump version to 1.2.8 for Gemini real resume + stale-seed guard + install layout release` (hash: 432356dec)
