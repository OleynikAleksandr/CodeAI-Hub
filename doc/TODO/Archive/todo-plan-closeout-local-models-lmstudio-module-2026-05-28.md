...
- **Planning source:** `doc/SolidWorks-WorkFlow/Plans/Archive/Local_Models_LMStudio_Module_Planning.md`
...
221. [DONE] Git Commit: `docs: document claude live tail dedupe` (hash: 54474f57e)

### Stream: Verification
222. [DONE] `local-models.phase31.verify.task1` Run targeted Core/Claude tests and package builds for the live/final tail dedupe regression; record exact commands/results in this plan (scope: `packages/core, packages/Claude_Module, doc/TODO/todo-plan.md`; expected commit: `test: verify claude live tail dedupe`).
    - Verification 2026-05-29: `npx tsx --test packages/core/src/remote-bridge/handlers/session-request-handler-live-tail-dedupe.test.ts packages/core/src/remote-bridge/handlers/session-request-handler-event-messages.test.ts packages/Claude_Module/src/messaging/claude-text-live-buffer.test.ts` — PASS (23 tests).
    - Verification 2026-05-29: `npm run build --workspace @codeai-hub/claude-module` — PASS.
    - Verification 2026-05-29: first parallel `npm run build --workspace @codeai-hub/core` failed because the simultaneous Claude build temporarily removed `@codeai-hub/claude-module/dist`; rerun sequentially after Claude build completed.
...
226. [DONE] Git Commit: `docs: prepare claude live tail dedupe release` (hash: 367261cf4)
...
237. [DONE] Git Commit: `test: cover lm studio memory cleanup` (hash: ebb2762b7)

### Stream: Documentation And Verification
238. [DONE] `local-models.phase32.docs.task1` Document LM Studio TTL, CodeAI-owned identifier cleanup, and the boundary that user-loaded LM Studio models are not unloaded by Core (scope: `doc/SolidWorks-WorkFlow/Plans/Archive/Local_Models_LMStudio_Module_Planning.md, doc/SolidWorks-WorkFlow/System/SystemArchitecture.md, doc/TODO/todo-plan.md`; expected commit: `docs: document lm studio model lifecycle`).
    ...
    - Verification 2026-05-29: `npm exec -- ultracite check doc/SolidWorks-WorkFlow/Plans/Archive/Local_Models_LMStudio_Module_Planning.md doc/SolidWorks-WorkFlow/System/SystemArchitecture.md` — PASS.
...
244. [DONE] Git Commit: `docs: prepare lm studio memory lifecycle release` (hash: 546e596bc)
...
248. [DONE] Git Commit: `chore: package lm studio memory lifecycle vsix` (hash: 4fa38be59)
249. [DONE] `local-models.phase32.user-acceptance.task1` User retests the new release and confirms CodeAI-owned LM Studio models receive TTL and idle local workers do not persist across Core restart/shutdown, while user-loaded LM Studio models remain untouched (scope: user workflow observation; expected commit: none). Result: User accepted release 1.2.405 as-is after LM Studio memory lifecycle discussion and requested scope closeout plus GitHub push.
