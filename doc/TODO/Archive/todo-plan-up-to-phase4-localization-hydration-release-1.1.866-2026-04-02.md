...
32. [DONE] Exclude repo-only workflow files from VSIX packaging and re-run `./scripts/build-release.sh --use-current-version` validation for the clean `1.1.866` package surface. Scope: `.vscodeignore`, `doc/TODO/todo-plan.md`. Target commit: `fix(release): exclude repo workflow files from vsix`
33. [DONE] Git Commit: `fix(release): exclude repo workflow files from vsix` (hash: `dfa694e1`)
34. [DONE] Archive this completed TODO plan and record the session report after the clean `build-release.sh --use-current-version` validation. Scope: `doc/TODO/Archive/*`, `doc/TODO/todo-plan.md`, `doc/Sessions/Session022.md`. Target commit: `docs(session): record localization hydration release`
