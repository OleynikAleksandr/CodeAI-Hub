...
- **Planning source:** `doc/SolidWorks-WorkFlow/Plans/Archive/Codex_StatusPanel_ModelSwitch_Architecture.md`
...
## Archive note after release 1.2.118

Финальный принятый контракт отличается от промежуточной гипотезы в ранних Streams G/M: для `gpt-5.3-codex-spark` нельзя опускать `turn/start.summary`, потому что Codex app-server default'ит omitted field в `detailed`. Итог release `1.2.118`: Spark получает explicit `summary = "none"` во всех workflow/model-switch/native-capture/translation/profile paths; non-Spark Codex models получают explicit `summary = "detailed" | "none"` из shared settings snapshot.
...
1. [DONE] Создать `doc/SolidWorks-WorkFlow/Plans/Codex_StatusPanel_ModelSwitch_Architecture.md` + этот todo-plan; after closeout the planning-doc was archived to `doc/SolidWorks-WorkFlow/Plans/Archive/Codex_StatusPanel_ModelSwitch_Architecture.md`. Scope: 2 файла; commit message: `docs: open codex status panel model switch scope`.
...
3. [DONE] Добавить Codex raw payload regression test: switch `gpt-5.2` → `gpt-5.3-codex-spark` с reasoning low → send turn → captured `turn/start` payload не содержит `summary` field, содержит `<model_switch>` в input array. Historical assertion from initial implementation; superseded by Phase 5 final contract `summary = "none"`. Scope: ≤2 файла; commit message: `test(codex): cover spark model switch raw payload`.
...
5. [DONE] SSOT docs sync: update Codex/provider invocation docs and SystemArchitecture to state that provider-home summary is neutralized; non-Spark live reasoning visibility comes only from explicit turn-level `summary`, Spark receives no summary field. Historical intermediate wording; superseded by Phase 5 final contract `summary = "none"`. Scope: `doc/SolidWorks-WorkFlow/Modules/Codex.md`, `doc/SolidWorks-WorkFlow/Modules/Codex_ProviderInvocationFlags.md`, `doc/SolidWorks-WorkFlow/System/SystemArchitecture.md`; commit message: `docs(ssot): document codex provider-home summary neutralization`.
...
2. [DONE] Git Commit: `fix(codex): keep translation spark summary explicit none` (hash: 3af6e430f)
3. [DONE] Update translation service/native translation capture tests for explicit Spark summary none. Verification: `npx tsx --test packages/Codex_AppServer_Module/src/translation/codex-app-server-translation-service.test.ts packages/Codex_AppServer_Module/src/diagnostics/codex-native-request-capture-service.test.ts`. Scope: `packages/Codex_AppServer_Module/src/translation/codex-app-server-translation-service.test.ts`, `packages/Codex_AppServer_Module/src/diagnostics/codex-native-request-capture-service.test.ts`, `doc/TODO/todo-plan.md`; commit message: `test(codex): cover translation spark summary none`.
4. [DONE] Git Commit: `test(codex): cover translation spark summary none` (hash: 72fd8be06)
5. [DONE] Change Core model invocation profile resolver so Spark profiles never advertise omitted summary; update smoke coverage. Verification: `npx tsx --test packages/core/src/model-invocation/model-invocation-profile-resolver.smoke.test.ts`. Scope: `packages/core/src/model-invocation/model-invocation-profile-resolver.ts`, `packages/core/src/model-invocation/model-invocation-profile-resolver.smoke.test.ts`, `doc/TODO/todo-plan.md`; commit message: `fix(core): model spark profiles use summary none`.
6. [DONE] Git Commit: `fix(core): model spark profiles use summary none` (hash: cfcfb6ff6)

### Stream R — SSOT docs and release 1.2.118

1. [DONE] Update SSOT docs: Spark does not support visible reasoning summary, but CodeAI Hub must send explicit `summary: "none"` because app-server omission defaults to `detailed`. Scope: `doc/SolidWorks-WorkFlow/Modules/Codex.md`, `doc/SolidWorks-WorkFlow/Modules/Codex_ProviderInvocationFlags.md`, `doc/SolidWorks-WorkFlow/System/SystemArchitecture.md`; commit message: `docs(ssot): document spark summary none contract`.
...
1. [DONE] Передать VSIX `codeai-hub-1.2.118.vsix` пользователю. Scope остается ACTIVE до явного acceptance.
2. [DONE] User retest passed: native rollout confirmed three turns in one Codex session used three different models: `gpt-5.4-mini` (`summary=detailed`) → `gpt-5.3-codex-spark` (`summary=none`) → `gpt-5.5` (`summary=detailed`); no `reasoning.summary` error.
