...
1. [DONE] Зафиксировать blocker `1.2.130`: reopened Codex session получает `Provider codexCli unavailable`, потому что `codex app-server` из `codex-cli 0.128.0` падает на startup overrides `-c mcp_servers.codex.enabled=false` / `-c mcp_servers.playwright.enabled=false` с `invalid transport in mcp_servers.codex`; managed `CODEX_HOME` clean, binary найден, это compatibility break в startup profile (scope: `doc/BugRegistry.md` + `doc/TODO/todo-plan.md`; expected commit: `fix: start codex app-server with 0.128 config schema`).
2. [DONE] Убрать устаревшие MCP server `enabled=false` overrides из Codex app-server process profile и обновить regression test: `packages/Codex_AppServer_Module/src/app-server/process/codex-app-server-process-profile.ts`, `packages/Codex_AppServer_Module/src/app-server/process/codex-app-server-process.test.ts`; сохранить feature disables `multi_agent`, `browser_use`, `in_app_browser`, `computer_use`, `image_generation`, `plugins`, `apps`, `tool_search` (scope: 2 files; expected commit: `fix: start codex app-server with 0.128 config schema`).
3. [DONE] Обновить Codex SSOT под новый verified invocation contract: `doc/SolidWorks-WorkFlow/Modules/Codex.md`, `doc/SolidWorks-WorkFlow/Modules/Codex_ProviderInvocationFlags.md`; отметить, что explicit `mcp_servers.*.enabled=false` больше не используется с `codex-cli 0.128.0`, а будущая MCP suppression требует отдельного evidence-gated contract (scope: 2 docs; expected commit: `fix: start codex app-server with 0.128 config schema`). Verification: `npx tsx --test packages/Codex_AppServer_Module/src/app-server/process/codex-app-server-process.test.ts`; `npm run build --workspace @codeai-hub/codex-app-server-module`; direct built-process smoke `CodexAppServerProcess.start()` -> `codex-process-start-ok`.
4. [DONE] Git Commit: `fix: start codex app-server with 0.128 config schema` (hash: 2b48db841)
...
3. [DONE] На clean tree запустить `./scripts/build-all.sh`; проверить fresh tarball'ы `1.2.131` в `~/.codeai-hub/releases/` и `doc/tmp/releases/`, включая Codex provider artifact with updated startup profile (scope: command + generated release artifacts; expected commit: `chore: bump release manifests for codex startup hotfix`). Verified: seven tarballs present in both release locations (`claude-module`, `codex-module`, `gemini-module`, `codeai-hub-core-darwin-arm64`, `CodeAIHubLauncher-macos-arm64`, `vscode-webview`, `project-manager`).
...
3. [DONE] Repair already-materialized sessions before `dialog:send`: add a minimal context update path in `packages/core/src/session-manager/index.ts`, use it in `packages/core/src/remote-bridge/handlers/session-request-handler-session-resolution.ts`, and cover missing-initiative reuse in `packages/core/src/remote-bridge/handlers/session-request-handler-codex-model-switch.test.ts`; existing runtime sessions with matching providerSessionId must not stay `initiativeSlug: null`. Verification: `npx tsx --test packages/core/src/remote-bridge/handlers/session-request-handler-codex-model-switch.test.ts` (scope: ≤3 files; expected commit: `fix: repair restored dialog workflow context before send`).
4. [DONE] Git Commit: `fix: repair restored dialog workflow context before send` (hash: 9450453f8)

### Stream 11R — Flow-Node Rollover Eligibility And Codex Usage Arbitration

1. [DONE] Extend flow-node rollover eligibility from description-only to trunk workflow nodes: `packages/core/src/flow-node-continuity/flow-node-continuity-types.ts`, `packages/core/src/flow-node-continuity/flow-node-continuity-facade.ts`, `packages/core/src/flow-node-continuity/flow-node-continuity-facade.test.ts`; eligible nodes are `description`, `virtual_simulation`, `diagram_modules` with `runSlug: null`; collector/reviewer/branch run slugs remain rejected. Verification: `npx tsx --test packages/core/src/flow-node-continuity/flow-node-continuity-facade.test.ts` (scope: 3 files; expected commit: `feat: enable continuity rollover for trunk workflow nodes`).
2. [DONE] Git Commit: `feat: enable continuity rollover for trunk workflow nodes` (hash: dd075d245)
...
6. [DONE] Git Commit: `docs: document model binding continuity persistence` (hash: 7b8cd02fc)

### Stream 11T — Stale Binding Recovery Safety

1. [DONE] Add stale-binding retry coverage that preserves restored workflow context and applied Codex turn config: created `packages/core/src/remote-bridge/handlers/session-request-handler-stale-rebind.test.ts`; stale binding recovery re-dispatches with current `session.modelBinding`, keeps `initiativeSlug/stage/runSlug`, preserves pending Codex model-switch injection through retry, and clears it only after successful resend. Verification: `npx tsx --test packages/core/src/remote-bridge/handlers/session-request-handler-stale-rebind.test.ts` (scope: ≤2 files; expected commit: `test: cover codex stale rebind with continuity context`).
2. [DONE] Git Commit: `test: cover codex stale rebind with continuity context` (hash: d32aff953)
...
2. [DONE] Git Commit: `docs: prepare codex continuity rollover fix release` (hash: 70b8fcd47)
3. [DONE] Repair full Core verification fixtures exposed by `npm test --workspace @codeai-hub/core`: update `packages/core/src/remote-bridge/handlers/session-request-handler.test-helpers.ts` and `packages/core/src/remote-bridge/handlers/session-request-handler.test.ts` so stop-rebind tests expose `providerRegistry.getDescriptor()` and model update expectations match the current explicit `modelBinding: undefined` payload shape. Verification: `npx tsx --test packages/core/src/remote-bridge/handlers/session-request-handler.test.ts` (scope: 2 files; expected commit: `test: repair core session handler verification fixtures`).
4. [DONE] Git Commit: `test: repair core session handler verification fixtures` (hash: 5b55bb08c)
5. [DONE] Repair TypeScript-only verification gaps exposed by `npm run build --workspace @codeai-hub/core`: updated `packages/core/src/remote-bridge/handlers/session-request-handler.rollover.test.ts` and `packages/core/src/session-continuity/continuity-tracker.test.ts` so the rollover mock accepts the production-broad option type and continuity model-binding refresh keeps required binding fields. Verification: `npm run build --workspace @codeai-hub/core` and `npx tsx --test packages/core/src/remote-bridge/handlers/session-request-handler.rollover.test.ts packages/core/src/session-continuity/continuity-tracker.test.ts` (scope: 2 files; expected commit: `test: repair continuity rollover verification types`).
6. [DONE] Git Commit: `test: repair continuity rollover verification types` (hash: ec2f876e5)
...
12. [DONE] Git Commit: `docs: record codex continuity rollover fix release build` (hash: 62c57b463)
...
1. [DONE] Добавить Core owner для pending Documentation Tree synthetic rollover: новый `packages/core/src/remote-bridge/handlers/session-request-handler-documentation-rollover-state.ts`, update `packages/core/src/remote-bridge/handlers/session-request-handler-flow-node-rollover.ts`, and focused `packages/core/src/remote-bridge/handlers/session-request-handler.rollover.test.ts`; для `description`, `virtual_simulation`, `diagram_modules` skip Create Report/resume bootstrap, capture source/target session ids, stage/workspace/provider binding, and last user-visible assistant message. Verification: `npx tsx --test packages/core/src/remote-bridge/handlers/session-request-handler.rollover.test.ts` (scope: 3 files; expected commit: `fix: add documentation tree synthetic rollover state`).
2. [DONE] Git Commit: `fix: add documentation tree synthetic rollover state` (hash: 94c5779e6)
3. [DONE] Разблокировать input после materialized continuation state, not after report/resume: update `packages/core/src/remote-bridge/handlers/session-request-handler-flow-node-rollover.ts`, `packages/core/src/remote-bridge/handlers/session-continuity-lock-service.ts`, and focused lock regression test so Documentation Tree fast path emits unlock without `report_in_progress` wait. Verification: `npx tsx --test packages/core/src/remote-bridge/handlers/session-request-handler.rollover.test.ts` (scope: ≤3 files; expected commit: `fix: unlock documentation rollover without report bootstrap`).
4. [DONE] Git Commit: `fix: unlock documentation rollover without report bootstrap` (hash: c5029245e)

### Stream 11X — Lazy Continuation Prompt Envelope

1. [DONE] Refactor Documentation Tree synthetic rollover state into shared runtime plumbing: update `packages/core/src/remote-bridge/handlers/session-request-handler-flow-node-rollover.ts`, `packages/core/src/remote-bridge/handlers/session-request-handler-message-dispatch.ts`, and `packages/core/src/remote-bridge/handlers/session-request-handler-runtime.ts` so rollover and dispatch use the same state owner. Verification: `npx tsx --test packages/core/src/remote-bridge/handlers/session-request-handler.rollover.test.ts` (scope: 3 files; expected commit: `fix: share documentation rollover state with dispatch`).
2. [DONE] Git Commit: `fix: share documentation rollover state with dispatch` (hash: 79713e1ee)
3. [DONE] Add one-shot consume API for Documentation Tree continuation context: update `packages/core/src/remote-bridge/handlers/session-request-handler-documentation-rollover-state.ts` and `packages/core/src/remote-bridge/handlers/session-request-handler-message-dispatch.ts` so the continuation context can be consumed exactly once by the first real user turn. Verification: `npx tsx --test --test-name-pattern "Documentation Tree synthetic rollover" packages/core/src/remote-bridge/handlers/session-request-handler.rollover.test.ts` (scope: 2 files; expected commit: `fix: consume documentation rollover context once`).
4. [DONE] Git Commit: `fix: consume documentation rollover context once` (hash: 62c649a4d)
5. [DONE] Добавить lazy next-user-message envelope: новый `packages/core/src/remote-bridge/handlers/session-request-handler-documentation-continuation-envelope.ts`, update `packages/core/src/remote-bridge/handlers/session-request-handler-message-dispatch.ts`, and focused dispatch test so first real user send after synthetic rollover prepends existing workflow start prompt/step contract + `Continuation Mode` + `Last Assistant Message Before Rollover` + user text. Verification: `npx tsx --test packages/core/src/remote-bridge/handlers/session-request-handler.rollover.test.ts packages/core/src/remote-bridge/handlers/session-request-handler.documentation-continuation.test.ts` (scope: 3 files; expected commit: `fix: attach documentation continuation envelope to next user turn`).
6. [DONE] Git Commit: `fix: attach documentation continuation envelope to next user turn` (hash: 4ba1945f8)
7. [DONE] Ensure envelope content excludes thinking/internal/translation/system messages: update `packages/core/src/remote-bridge/handlers/session-request-handler-documentation-continuation-envelope.ts`, `packages/core/src/remote-bridge/handlers/session-request-handler-documentation-rollover-state.ts`, and focused unit test; only user-visible assistant text is eligible. Verification: `npx tsx --test packages/core/src/remote-bridge/handlers/session-request-handler.documentation-continuation.test.ts` (scope: ≤3 files; expected commit: `fix: filter documentation rollover assistant context`).
8. [DONE] Git Commit: `fix: filter documentation rollover assistant context` (hash: 77eab4327)

### Stream 11Y — Documentation Tree Legacy Rollover Cleanup

1. [DONE] Вынести Documentation Tree rollover regression в focused test file: update `packages/core/src/remote-bridge/handlers/session-request-handler.rollover.test.ts`, add `packages/core/src/remote-bridge/handlers/session-request-handler.documentation-rollover.test.ts`, and keep architecture file-size limit below 500 lines before legacy cleanup. Verification: `npx tsx --test packages/core/src/remote-bridge/handlers/session-request-handler.documentation-rollover.test.ts packages/core/src/remote-bridge/handlers/session-request-handler.rollover.test.ts` (scope: 2 test files; expected commit: `test: extract documentation rollover regression`).
2. [DONE] Git Commit: `test: extract documentation rollover regression` (hash: acbc6b2c6)
3. [DONE] Очистить Documentation Tree stages from old Create Report/resume eligibility: update `packages/core/src/remote-bridge/handlers/session-request-handler-flow-node-rollover.ts`, `packages/core/src/remote-bridge/handlers/session-request-handler-flow-node-report-state.ts`, and focused `packages/core/src/remote-bridge/handlers/session-request-handler.documentation-rollover.test.ts`; `description`, `virtual_simulation`, `diagram_modules` must no longer enter report-ready/report-wait state after synthetic rollover is selected. Verification: `npx tsx --test packages/core/src/remote-bridge/handlers/session-request-handler.documentation-rollover.test.ts packages/core/src/remote-bridge/handlers/session-request-handler.rollover.test.ts` (scope: 3 files; expected commit: `fix: remove documentation tree report rollover path`).
4. [DONE] Git Commit: `fix: remove documentation tree report rollover path` (hash: 3040dc35f)
5. [DONE] Очистить resume-bootstrap side effects for Documentation Tree synthetic path: update `packages/core/src/remote-bridge/handlers/session-request-handler-resume-lifecycle.ts`, `packages/core/src/remote-bridge/handlers/session-continuity-materializer.ts`, and focused `packages/core/src/remote-bridge/handlers/session-request-handler.resume-embedding.test.ts`; no standalone internal bootstrap/`Ready to continue working` turn for Documentation Tree fast rollover. Verification: `npx tsx --test packages/core/src/remote-bridge/handlers/session-request-handler.resume-embedding.test.ts packages/core/src/remote-bridge/handlers/session-request-handler.documentation-rollover.test.ts` (scope: 3 files; expected commit: `fix: remove documentation tree resume bootstrap path`).
6. [DONE] Git Commit: `fix: remove documentation tree resume bootstrap path` (hash: cffd34ad1)

### Stream 11Z — Continuation Contract Regression Coverage And SSOT

1. [DONE] Add production-path regression for Documentation Tree fast path: update focused `packages/core/src/remote-bridge/handlers/session-request-handler.documentation-rollover.test.ts` and validate `packages/core/src/remote-bridge/handlers/session-request-handler.resume-embedding.test.ts` coverage so `description`/`virtual_simulation`/`diagram_modules` rollover never sends Create Report, never waits for report path, and sends the combined continuation envelope only with the next real user message. Verification: `npx tsx --test packages/core/src/remote-bridge/handlers/session-request-handler.documentation-rollover.test.ts packages/core/src/remote-bridge/handlers/session-request-handler.resume-embedding.test.ts` (scope: 1 test file + targeted regression run; expected commit: `test: cover documentation tree fast synthetic rollover`).
2. [DONE] Git Commit: `test: cover documentation tree fast synthetic rollover` (hash: 0fdaafa73)
...
2. [DONE] Git Commit: `docs: prepare documentation rollover fix release` (hash: 595668b1f)
3. [DONE] На clean tree запустить targeted verification: `npm test --workspace @codeai-hub/core`, `npm run build --workspace @codeai-hub/core`, `npm run build --workspace @codeai-hub/codex-app-server-module`, and `npm run typecheck:webview`; fixed a `tsc` narrowing issue in the focused Documentation Tree rollover test exposed by `npm run build --workspace @codeai-hub/core`. Verification passed: `npx tsx --test packages/core/src/remote-bridge/handlers/session-request-handler.documentation-rollover.test.ts`; `npm run build --workspace @codeai-hub/core`; `npm test --workspace @codeai-hub/core`; `npm run build --workspace @codeai-hub/codex-app-server-module`; `npm run typecheck:webview` (scope: commands + 1 test file + todo-plan; expected commit: `test: verify documentation rollover fix`).
4. [DONE] Git Commit: `test: verify documentation rollover fix` (hash: bdd554d46)
...
1. [DONE] Установить fast-rollover fix VSIX, открыть workspace `/Users/oleksandroliinyk/VSCODE/CodeAI-Hub codex 5.5`, повторить Codex Description flow-node continuity with `remainingPercentThreshold: 80`, and verify Documentation Tree rollover skips Create Report/report file creation. User acceptance 2026-05-03: багов не замечено, смена сессии моментальная.
