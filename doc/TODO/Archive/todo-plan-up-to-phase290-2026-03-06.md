...
4. [DONE] Git Commit: `chore(release): build-all v1.1.715 workflow commentary restore` (hash: `0b3b080f`)
5. [DONE] После commit на чистом дереве прогнать `./scripts/build-release.sh --use-current-version`; release verification пройдена честно без флагов: подтверждены `Verifying SDK exclusions`, `Removing dev dependencies before packaging...`, `✅ Package created`, собран `codeai-hub-1.1.715.vsix` (scope: VSIX packaging, final release verification; actual commit: `chore(release): build-release v1.1.715 workflow commentary restore`).
6. [DONE] Git Commit: `chore(release): build-release v1.1.715 workflow commentary restore` (hash: `94b73703`)
7. [DONE] После успешного релиза оформить сессионный отчёт и закрытие Phase: создать `doc/Sessions/Session065.md`, обновить/заархивировать завершённый `todo-plan.md` и зафиксировать финальные артефакты релиза; Phase 290 переведён в архив, открыт follow-up plan для post-release validation `v1.1.715` (scope: `doc/Sessions/Session065.md`, `doc/TODO/todo-plan.md`, `doc/TODO/Archive/todo-plan-up-to-phase290-2026-03-06.md`; actual commit: `docs(session): record workflow commentary restore release`).
