...
4. [DONE] Git Commit: `build(release): assemble codex raw rollout release` (hash: `2da43b214`)
5. [DONE] Run `./scripts/build-release.sh --use-current-version` and validate the packaged VSIX before final docs closeout; scope: `doc/TODO/Archive/*`, `doc/TODO/todo-plan.md`, `doc/Sessions/Session048.md`; expected commit message: `docs(plans): archive codex rollout source of truth scope`
   Release result: `./scripts/build-release.sh --use-current-version` -> passed, VSIX created at `/Users/oleksandroliinyk/VSCODE/CodeAI-Hub/codeai-hub-1.1.893.vsix`.
   Release result: advisory-only broken absolute markdown links remain in `doc/Sessions/Session040.md` and `doc/Sessions/Session041.md`.
6. [DONE] Git Commit: `docs(plans): archive codex rollout source of truth scope` (hash: `c121901e5`)
7. [DONE] Archive the completed migration plan, restore a new placeholder active `todo-plan.md`, and record the release session report; scope: `doc/TODO/Archive/*`, `doc/TODO/todo-plan.md`, `doc/Sessions/Session048.md`; expected commit message: `docs(session): record codex raw rollout release`
8. [TODO] Git Commit: `docs(session): record codex raw rollout release` (hash: TBD)
