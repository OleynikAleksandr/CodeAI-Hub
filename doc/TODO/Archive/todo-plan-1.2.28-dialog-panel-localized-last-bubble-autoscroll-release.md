...
- **Planning source:** `doc/SolidWorks-WorkFlow/Plans/Archive/DialogPanel_LocalizedLastBubble_Autoscroll_Release_1.2.28.md`
...
4. [DONE] Git Commit: `build: release 1.2.28` (hash: `f5cb71b75`)
5. [DONE] Выполнить `./scripts/build-release.sh --use-current-version`, проверить VSIX/tarball артефакты и закрыть scope архивированием planning/todo; results: `build-release.sh --use-current-version` passed with `Step 7: Verifying SDK exclusions`, `Removing dev dependencies before packaging`, `✅ Package created`; VSIX: `codeai-hub-1.2.28.vsix`; tarballs: `doc/tmp/releases/*1.2.28*`; expected commit: `docs: close 1.2.28 release scope`
