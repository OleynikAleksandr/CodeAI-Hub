...
1. [DONE] Прогнать таргетные тесты/сборки, затем `./scripts/build-all.sh`, затем `./scripts/build-release.sh --use-current-version`. — scope: release/build artifacts; commit: `chore: bump version to 1.2.17 for Claude pre-tool thinking fix release`
2. [DONE] Git Commit: `chore: bump version to 1.2.17 for Claude pre-tool thinking fix release` (hash: `65bce2dc6`)
3. [DONE] Архивировать `todo-plan.md` и вернуть placeholder `Execution Scope Status: EMPTY`. — scope: `doc/TODO/todo-plan.md`, `doc/TODO/Archive/todo-plan-1.2.17-claude-pre-tool-thinking.md`; commit: `docs: close 1.2.17 todo-plan after build`
4. [IN_PROGRESS] Git Commit: `docs: close 1.2.17 todo-plan after build` (hash: TBD)
