...
> Stopped / superseded snapshot, 2026-05-11: this implementation scope reached release v1.2.227 but user retesting showed the plan shape itself is wrong for managed step orchestration. The unfinished Phase 34 retest is intentionally not marked DONE. Follow-up work continues in the new active plan `managed-step-orchestration-plans-revision`.
...
```json
{
  ...
  "currentTaskId": "application-skeleton-orchestration.phase34.acceptance.task1",
  "expectedCommitMessage": null,
  "debt": {
    "reason": "User stopped this expanded scope on 2026-05-11 after v1.2.227 retest exposed that the Application Skeleton plan shape and managed correction lifecycle need redesign, not another hot-fix on this plan.",
    "stage": "superseded_scope",
    "taskId": "application-skeleton-orchestration.phase34.acceptance.task1"
  }
}
```
...
135. [DONE] Git Commit: `docs: prepare application skeleton acceptance wiring release` (hash: 75b342573)
...
148. [DONE] Git Commit: `build: release application skeleton handoff anchor` (hash: 5f596b7d9)

## Phase 34 - User Workflow Acceptance Testing Rerun (owner: user, updated: 2026-05-11)

### Stream: VSIX Retest

149. [BLOCKED] `application-skeleton-orchestration.phase34.acceptance.task1` Before installing v1.2.227, manually delete `doc/TODO/stages/application-skeleton/todo-plan.md` in the test workspace so the new seed (with the regex-matchable handoff anchor) is generated fresh; then install the v1.2.227 VSIX and rerun Application Skeleton end-to-end. Phase 1 / 2 / 3 commits must land as in Phase 30; after `feat: materialize application skeleton`, the plan must advance cleanly to `application-skeleton.handoff.task1` with `expectedCommitMessage: null`, no spurious `phase3.materialize.task2` continuation pair, and the workspace stage transition (`recordWorkspaceCommit`) must switch `activeStage` to `quality_gates`. (scope: chat/process observation only; no commit required). **BLOCKED / SUPERSEDED 2026-05-11:** user stopped this scope after confirming that the remaining defects are caused by the managed step orchestration model itself: Core correction turns are not represented as tracked microtasks with paired Git commits, Application Skeleton uses a pre-seeded static phase shape instead of incremental task injection, and the post-completion phase must be a user-return revision loop rather than a handoff anchor.
