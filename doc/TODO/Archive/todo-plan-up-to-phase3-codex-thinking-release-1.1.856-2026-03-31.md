...
- Этот scope был открыт через planning-док, который после завершения реализации удалён из `Plans`; финальный SSOT теперь живёт в `doc/SolidWorks-WorkFlow/Modules/Codex.md`
...
- Новый релиз `1.1.855` был собран как основной release stream этого плана.
- После post-release startup hotfix собран и проверен релиз `1.1.856`, которым scope был фактически закрыт.
