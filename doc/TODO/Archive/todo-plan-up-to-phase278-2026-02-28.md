...
3. [DONE] Выполнить `./scripts/build-release.sh --use-current-version`, проверить release output checklist (`Verifying SDK exclusions`, `Removing dev dependencies`, `✅ Package created`) и зафиксировать итоги в session report (scope: `doc/Sessions/Archive/Session050.md`; expected commit: `docs(session): record release verification for phase271`).
...
1. [DONE] Провести аудит root-файлов в `doc/SolidWorks-WorkFlow/` и зафиксировать решение «нужен/архивировать/перенести» в session report (scope: `doc/Sessions/Archive/Session050.md`; expected commit: `docs(session): record solidworks root docs audit`).
...
1. [DONE] Синхронизировать `todo-plan` + session report по факту удаления reviewer из active template delivery (scope: `doc/TODO/todo-plan.md`, `doc/Sessions/Archive/Session050.md`; expected commit: `docs(todo): sync reviewer-removal progress in session050`).
...
3. [BLOCKED] Выполнить `./scripts/build-release.sh --use-current-version`, проверить output checklist и зафиксировать результаты в session report (scope: `doc/Sessions/Archive/Session050.md`; expected commit: `docs(session): record release after reviewer removal`).
...
3. [DONE] Выполнить `./scripts/build-release.sh --use-current-version`, проверить checklist (`Verifying SDK exclusions`, `Removing dev dependencies`, `✅ Package created`) и зафиксировать результаты в session report (scope: `doc/Sessions/Archive/Session050.md`; expected commit: `docs(session): record release after phase276`).
...
1. [DONE] Прогнать валидацию (`npx tsc -p . --noEmit` + target builds при необходимости), обновить `todo-plan` + `Session050` и подготовить итоговый отчет сессии (scope: `doc/TODO/todo-plan.md`, `doc/Sessions/Archive/Session050.md`; expected commit: `docs(session): finalize phase278 reviewer purge report`).
...
7. [DONE] Зафиксировать release results в session report (`checklist` + VSIX + tarballs) (scope: `doc/Sessions/Archive/Session050.md`, `doc/TODO/todo-plan.md`; expected commit: `docs(session): record post-purge release results`).
