...
- **Planning source:** `doc/SolidWorks-WorkFlow/Plans/Archive/SessionDialog_ThinkingTextOpacity_Release_1.2.35.md`
...
2. [DONE] Git Commit: `docs: prepare 1.2.35 release notes` (hash: `1f72b05d1`)
3. [DONE] Выполнить `./scripts/build-all.sh` и зафиксировать массовые version/manifest updates release-пайплайна `1.2.35`; unified version поднята до `1.2.35`, provider/core/ui/launcher tarball артефакты собраны успешно; scope: release packaging surface, version manifests, generated bundle metadata; commit: `build: release 1.2.35`
4. [DONE] Git Commit: `build: release 1.2.35` (hash: `b79cb0d15`)
5. [DONE] Выполнить `./scripts/build-release.sh --use-current-version`, проверить VSIX/tarball артефакты и закрыть scope архивированием planning/todo; results: `build-release.sh --use-current-version` passed with `Step 7: Verifying SDK exclusions`, `Removing dev dependencies before packaging`, `✅ Package created`; VSIX: `codeai-hub-1.2.35.vsix`; tarballs: `doc/tmp/releases/*1.2.35*`; expected commit: `docs: close 1.2.35 release scope`
6. [TODO] Git Commit: `docs: close 1.2.35 release scope` (hash: TBD)

Build notes:
- `./scripts/build-all.sh` ✅
- Unified version after build: `1.2.35`
