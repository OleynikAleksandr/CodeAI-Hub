...
5. [DONE] На чистом дереве успешно выполнен `./scripts/build-release.sh --use-current-version`: в логе подтверждены `Verifying SDK exclusions`, `Removing dev dependencies before packaging...`, `✅ Package created`, собран `codeai-hub-1.1.716.vsix`, а свежие tarball'ы лежат в `doc/tmp/releases/`; первый прогон упал на `typecheck:webview` из-за слишком широкого `IncomingMessage.payload` в PM trace client, после чего release-blocker был снят коммитом `fix(pm): narrow dialog trace incoming payload types` и скрипт повторён без возврата к `build-all` (scope: release pipeline, VSIX packaging; actual commit: `chore(release): build-release v1.1.716 workflow submit diagnostics`).
6. [DONE] Git Commit: `chore(release): build-release v1.1.716 workflow submit diagnostics` (hash: `8933c383`)
7. [DONE] Обновить `doc/Sessions/Session066.md`, заархивировать завершённый `todo-plan.md` в `doc/TODO/Archive/` и открыть следующий план только после успешного релиза и чистого дерева (scope: `doc/Sessions/`, `doc/TODO/`; actual commit: `docs(session): record workflow submit diagnostics release`).
8. [DONE] Git Commit: `docs(session): record workflow submit diagnostics release` (hash: `40c7f0d1`)
