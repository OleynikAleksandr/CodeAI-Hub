# Plan Closeout: local-models-standalone-chat-selection-2026-06-23

**Created:** 2026-06-23T10:16:34.096Z
**Acceptance:** User confirmed v1.2.594 works: standalone Local Models chat shows and runs the selected LM Studio model.
**Execution Scope Status:** ACTIVE
**Branch:** main
**Current Task:** local-models-selection.phase8.closeout.task1
**Expected Commit:** docs: close local models standalone chat selection scope
**Last Recorded Commit:** self
**Planning Source Disposition:** already_archived
**Planning Source Path:** doc/SolidWorks-WorkFlow/Plans/Archive/LocalModels_StandaloneChat_ModelSelection_Bugfix_Planning.md
...
```json
{
  ...
  "currentTaskId": "local-models-selection.phase8.closeout.task1",
  "expectedCommitMessage": "docs: close local models standalone chat selection scope",
  "debt": null
}
```
...
1. [DONE] `local-models-selection.phase0.plan.task1` Create the planning source and active execution plan for Local Models standalone chat model selection/status-panel repair (scope: `doc/SolidWorks-WorkFlow/Plans/Archive/LocalModels_StandaloneChat_ModelSelection_Bugfix_Planning.md, doc/TODO/todo-plan.md, doc/SolidWorks-WorkFlow/Docs_Index.md`; expected commit: `docs: start local models standalone chat selection fix`).
...
## Phase 6 - Follow-up Release Build (owner: Codex, updated: 2026-06-23)
### Stream: Package standalone chat binding follow-up
23. [DONE] `local-models-selection.phase6.release.scope.task1` Add release build tasks after explicit user confirmation to package the standalone Local Models chat binding follow-up (scope: `doc/TODO/todo-plan.md`; expected commit: `docs: add v1.2.594 release scope for local models follow-up`).
24. [DONE] `local-models-selection.phase6.release.scope.commit1` Git Commit: `docs: add v1.2.594 release scope for local models follow-up` (hash: self)
25. [DONE] `local-models-selection.phase6.release.docs.task1` Update release notes for v1.2.594 before packaging (scope: `README.md, CHANGELOG.md, doc/TODO/todo-plan.md`; expected commit: `docs: prepare v1.2.594 local models release notes`).
   - Evidence 2026-06-23: README current release and CHANGELOG entry prepared for v1.2.594 before running release scripts.
26. [DONE] `local-models-selection.phase6.release.docs.commit1` Git Commit: `docs: prepare v1.2.594 local models release notes` (hash: self)
27. [DONE] `local-models-selection.phase6.release.build.task1` Run the release build scripts and record produced artifacts/evidence (scope: `assets/**/manifest.json, package*.json, packages/*/package.json, doc/TODO/todo-plan.md`; expected commit: `build: release v1.2.594`).
   - Evidence 2026-06-23: `./scripts/build-all.sh` passed and produced provider/core/launcher/UI tarballs for v1.2.594 in `doc/tmp/releases/`.
   - Evidence 2026-06-23: `./scripts/build-release.sh --use-current-version --allow-dirty` passed, including `Step 7: Verifying SDK exclusions`, `Removing dev dependencies before packaging`, `✅ Package created`, and VSIX runtime package surface verification.
   - Artifact: `codeai-hub-1.2.594.vsix` (5.5M) created at repository root.
28. [DONE] `local-models-selection.phase6.release.build.commit1` Git Commit: `build: release v1.2.594` (hash: self)

## Phase 7 - User Workflow Acceptance Testing (owner: user, updated: 2026-06-23)
### Stream: User retest
29. [DONE] `local-models-selection.phase7.acceptance.task1` User verifies standalone Local Models chat shows the selected model in the lower status panel and runs that model on the next turn (scope: `doc/TODO/todo-plan.md`; expected commit: `chore: record local models selection user acceptance`).
   - Evidence 2026-06-23: user confirmed v1.2.594 fixes the Local Models standalone chat selection/status-panel/runtime binding flow: "Да, теперь все работает."
30. [DONE] `local-models-selection.phase7.acceptance.commit1` Git Commit: `chore: record local models selection user acceptance` (hash: self)

## Phase 8 - Scope Closeout (owner: Codex, updated: 2026-06-23)
### Stream: Archive + planning-doc disposition
31. [IN_PROGRESS] `local-models-selection.phase8.closeout.task1` After explicit user acceptance, archive this plan and decide planning-doc disposition (scope: `doc/TODO/Archive/**, doc/TODO/todo-plan.md, doc/SolidWorks-WorkFlow/Plans/**, doc/SolidWorks-WorkFlow/Docs_Index.md`; expected commit: `docs: close local models standalone chat selection scope`).
32. [TODO] `local-models-selection.phase8.closeout.commit1` Git Commit: `docs: close local models standalone chat selection scope` (hash: TBD)
33. [TODO] `local-models-selection.phase8.handoff.task1` Reserved post-closeout handoff anchor; do not execute automatically unless the user asks for another cycle.
````
