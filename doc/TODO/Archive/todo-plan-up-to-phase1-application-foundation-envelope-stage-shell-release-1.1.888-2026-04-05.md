...
4. [DONE] Git Commit: `feat: track application foundation envelope workflow order` (hash: `03a999581`)
5. [DONE] Compute `diagramModulesProgress.aggregateReady` and gate `application_foundation_envelope` on it in `/workflow-state`; scope: `packages/core/src/remote-bridge/handlers/diagram-modules-progress.ts`, `packages/core/src/remote-bridge/handlers/workflow-state-service.ts`, `packages/core/src/remote-bridge/handlers/workflow-state-service.test.ts`; expected commit message: `feat: gate application foundation envelope after diagram modules`
6. [DONE] Git Commit: `feat: gate application foundation envelope after diagram modules` (hash: `a69e2055d`)
7. [DONE] Add canonical artifact path and filesystem hydration support for `application-foundation-envelope.md`; scope: `packages/core/src/workflow/paths/workflow-paths-types.ts`, `packages/core/src/workflow/paths/workflow-artifact-paths.ts`, `packages/core/src/remote-bridge/handlers/workflow-state-filesystem-hydration.ts`; expected commit message: `feat: hydrate application foundation envelope artifacts`
8. [DONE] Git Commit: `feat: hydrate application foundation envelope artifacts` (hash: `f1ae28aa7`)
9. [DONE] Cover canonical envelope path resolution and cold-start artifact hydration in tests and workflow contract docs; scope: `packages/core/src/workflow/paths/workflow-artifact-paths.test.ts`, `packages/core/src/remote-bridge/handlers/workflow-state-service.test.ts`, `doc/SolidWorks-WorkFlow/Contracts/Workflow_CLI.md`; expected commit message: `test: cover application foundation envelope artifact hydration`
10. [DONE] Git Commit: `test: cover application foundation envelope artifact hydration` (hash: `6599c4321`)
...
6. [DONE] Git Commit: `feat: persist application foundation envelope artifacts` (hash: `45291588d`)
...
6. [DONE] Git Commit: `test: cover application foundation envelope prompt pack` (hash: `6ae948399`)
...
2. [DONE] Git Commit: `feat: add application foundation envelope toolbar step` (hash: `9341dc527`)
3. [DONE] Extend workflow tree labels and blocked-title rendering for the new stage; scope: `src/client/project-manager/components/layout/workspace-tree-model.ts`, `src/client/project-manager/components/layout/workspace-tree.tsx`; expected commit message: `feat: label application foundation envelope tree stage`
4. [DONE] Git Commit: `feat: label application foundation envelope tree stage` (hash: `86ca4d222`)
5. [DONE] Extend main-area stage priority resolution and workspace auto-select order for the new stage, and remove the now-unused shared continuity-chain export; scope: `src/client/project-manager/components/layout/use-main-area-workflow-state.ts`, `src/client/project-manager/components/layout/workspace-tree-auto-select.ts`, `src/client/project-manager/components/layout/workspace-tree-branch-nodes.ts`; expected commit message: `feat: prioritize application foundation envelope UI recovery`
6. [DONE] Git Commit: `feat: prioritize application foundation envelope UI recovery` (hash: `9936228b2`)
...
2. [DONE] Git Commit: `test: align application foundation envelope workflow fixtures` (hash: `a16b496a4`)
3. [DONE] Run targeted tests/builds for touched workflow/core/project-manager files, update this plan with the concrete results, and close verification; scope: targeted `tsx` tests, `npm run typecheck:webview`, `npm run build:project-manager`, `npm run build --workspace=@codeai-hub/core`; results: PASS (`npx tsx --test packages/core/src/remote-bridge/handlers/workflow-state-service.test.ts packages/core/src/workflow/paths/workflow-artifact-paths.test.ts packages/core/src/remote-bridge/handlers/idea-contract-service.application-foundation-envelope.test.ts packages/core/src/remote-bridge/handlers/http-api-router.artifact-upsert.test.ts`, `npx tsx --test src/client/project-manager/services/workflow-step-start-service.gating.test.ts src/client/project-manager/services/prompt-pack-builder.virtual-simulation.test.ts src/client/project-manager/components/layout/workspace-tree-diagram-branch-nodes.test.ts`, `npm run typecheck:webview`, `npm run build:project-manager`, `npm run build --workspace=@codeai-hub/core`); expected commit message: `test: verify application foundation envelope stage shell`
4. [DONE] Git Commit: `test: verify application foundation envelope stage shell` (hash: `ef8a9f747`)
...
8. [DONE] Git Commit: `docs(session): record application foundation envelope planning scope` (hash: `0b1b7f9ea`)
...
10. [DONE] Git Commit: `build(release): assemble application foundation envelope stage shell release` (hash: `9a6ab9557`)
11. [DONE] Run `./scripts/build-release.sh --use-current-version`, verify the final VSIX/package outputs, archive the completed execution plan, seed a new empty `todo-plan.md`, record the release session report, and close the phase; scope: `doc/TODO/Archive/todo-plan-up-to-phase1-application-foundation-envelope-stage-shell-release-1.1.888-2026-04-05.md`, `doc/TODO/todo-plan.md`, `doc/Sessions/Session042.md`; results: PASS (`./scripts/build-release.sh --use-current-version`), VSIX: `codeai-hub-1.1.888.vsix`, release tarballs present in `doc/tmp/releases/`; expected commit message: `docs(session): record application foundation envelope stage shell release`
12. [DONE] Git Commit: `docs(session): record application foundation envelope stage shell release` (hash: `TBD - this commit`)
