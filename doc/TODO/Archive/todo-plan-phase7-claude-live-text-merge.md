...
1. [DONE] Ran `./scripts/build-all.sh` from repo root. Versions bumped to 1.1.999; tarballs produced in `~/.codeai-hub/releases/` (`claude-module`, `codex-module`, `gemini-module`, `codeai-hub-core-darwin-arm64`, `project-manager`, `vscode-webview`, `CodeAIHubLauncher-macos-arm64`) and present in `doc/tmp/releases/`. Scope: no manual file edits (script owns version bumps).
2. [DONE] Git Commit: `chore: build 1.1.999 release assets` (hash: bf2e675c3)
3. [DONE] Ran `./scripts/build-release.sh --use-current-version` on clean tree. VSIX `codeai-hub-1.1.999.vsix` (2.2M) produced in repo root; SDK exclusions verified, dev deps removed before packaging, dev deps restored after. Scope: no file edits (script-driven).

### Stream 2.3 — Closeout
1. [DONE] Archived completed todo-plan as `doc/TODO/Archive/todo-plan-phase7-claude-live-text-merge.md`; created fresh empty `doc/TODO/todo-plan.md` with "no active scope" pointer; moved planning-doc to `doc/SolidWorks-WorkFlow/Plans/Archive/ClaudeLiveText_DialogBubbleMerge.md`; `Docs_Index.md` did not reference the plan path (checked), no update required. Session038.md is written and committed after this archive commit per CLAUDE.md §1. Scope: 3 files.
2. [DONE] Git Commit: `docs: archive claude live bubble merge execution scope` (hash: TBD)
