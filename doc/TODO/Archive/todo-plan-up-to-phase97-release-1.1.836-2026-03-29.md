...
1. [DONE] Зафиксировать planning scope для provider-confirmed observability: нормализовать только тот feedback, который реально пришёл обратно из runtime Claude/Codex/Gemini, и разрезать реализацию на отдельные provider streams. Scope: `doc/SolidWorks-WorkFlow/Plans/Archive/ProviderFeedback_ModelAndReasoning_Logging_Architecture.md`, `doc/TODO/todo-plan.md`. Expected commit: `docs(plan): define provider feedback scope`
...
14. [DONE] Git Commit: `test(observability): verify provider feedback logs` (hash: `ea7eff6a`)

## Phase 96 — Release Docs For v1.1.836 (owner: Oleksandr, updated: 2026-03-29)

### Stream: Sync release-facing docs with provider feedback scope
15. [DONE] Актуализировать release-facing документы под `v1.1.836`, чтобы README и CHANGELOG отражали provider-confirmed logging contract до финальной сборки. Scope: `README.md`, `CHANGELOG.md`. Expected commit: `docs(release): prepare v1.1.836`
16. [DONE] Git Commit: `docs(release): prepare v1.1.836` (hash: `b03e1db0`)
