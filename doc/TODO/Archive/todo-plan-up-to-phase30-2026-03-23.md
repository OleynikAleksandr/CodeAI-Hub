...
1. [DONE] Переписать source templates `diagram_modules` так, чтобы explanatory text был на русском языке, DSL terms остались английскими, а narrative-повторы между template / field reference / merge rules были убраны или сведены к одному authoritative wording (scope: `packages/agents/diagram-modules-agent/assets/module-inventory-template.md`, `packages/agents/diagram-modules-agent/assets/module-inventory-field-reference.md`, `packages/agents/diagram-modules-agent/assets/module-inventory-merge-rules.md`; expected commit: `fix(diagram-templates): localize and dedupe modules templates`).
2. [DONE] Git Commit: `fix(diagram-templates): localize and dedupe modules templates` (hash: `90e9b0a9`)
...
2. [DONE] Git Commit: `fix(diagram-layout): tighten module stage spacing` (hash: `eecaad51`)
