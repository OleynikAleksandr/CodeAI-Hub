...
- **Planning source:** `doc/SolidWorks-WorkFlow/Plans/SessionDialog_ThinkingProviderAccent_Release_1.2.31.md`
...
4. [DONE] Git Commit: `build: release 1.2.31` (hash: `e3a936aac`)
5. [DONE] Выполнить `./scripts/build-release.sh --use-current-version`, проверить VSIX/tarball артефакты и закрыть scope архивированием planning/todo; results: `build-release.sh --use-current-version` passed with `Step 7: Verifying SDK exclusions`, `Removing dev dependencies before packaging`, `✅ Package created`; VSIX: `codeai-hub-1.2.31.vsix`; tarballs: `doc/tmp/releases/*1.2.31*`; expected commit: `docs: close 1.2.31 release scope`
6. [TODO] Git Commit: `docs: close 1.2.31 release scope` (hash: TBD)

Build notes:
- `./scripts/build-all.sh` ✅
- Unified version after build: `1.2.31`
