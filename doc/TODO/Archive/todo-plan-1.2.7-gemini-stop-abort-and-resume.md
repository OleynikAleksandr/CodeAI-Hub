...
1. [DONE] Verify чистое дерево, запустить `./scripts/build-all.sh` + `./scripts/build-release.sh --use-current-version`; tarball'ы в `doc/tmp/releases/` и `~/.codeai-hub/releases/`; VSIX `codeai-hub-1.2.7.vsix` (2.2M) в корне.
2. [DONE] Git Commit: `chore: bump version to 1.2.7 for Gemini Stop abort + resume release` (hash: 2929cdf72)
