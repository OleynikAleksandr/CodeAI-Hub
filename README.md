...
- **Strict localization sync now recovers isolated missing structured entries before failing Save.** Runtime bundle materialization keeps the marker-preserving whole-bundle translation contract, but if the translation engine drops one segment inside that batch, CodeAI Hub retries only the missing entry and stitches it back into the localized bundle instead of persisting a partial-fallback bundle and rejecting synchronization.
- **Localization readiness is still strict.** Save still fails when a runtime bundle truly cannot be completed, but single-entry omissions from provider-owned translation engines no longer surface as false-negative `bundle is not ready` errors after the rest of the bundle translated correctly.
- **The shipped duplication / PM refresh line stays unchanged.** `1.2.21` is a hotfix on top of the finalized `1.2.20` release line; Claude/Codex finalization, replay-first usage ownership, and visibility-aware polling contracts remain intact.
