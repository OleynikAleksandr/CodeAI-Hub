...
- Release pipeline: локальный `build-all` должен поднять unified version до `1.1.729` и пересобрать provider/core/ui/launcher артефакты уже поверх этого baseline.
