...
- **Audit cleanup release.** Post-Session043 codebase audit (dead code + broken doc links + duplication analysis) produced a concrete debt list — 1.2.10 closes the actionable items. No runtime behaviour changes; no retest required. Four directions:
  - **A. Docs + config fix.** `doc/SolidWorks-WorkFlow/Docs_Index.md:80-82` now points at the real bundled template files (prior references to `templates/description/questionnaire-template.md` and friends did not match the actual `.codeai-hub/codeai-hub/description/` layout). `knip.json` exclusion for `packages/core/src/workflow/diagram-dsl/diagram-legacy-ownership-parser.ts` reviewed. Stale TODO in `packages/agents/spec-creator/` audited.
