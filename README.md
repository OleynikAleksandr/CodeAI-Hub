...
- **Первое сообщение в reopened Claude/Codex диалоге после Core restart больше не исчезает молча.** Follow-up к `BUG-2026-04-21-01` / релиз `1.2.39`. Тот фикс корректно ре-журналировал paper-binding в ядре (UI больше не залипал в "Agents is working…"), но dispatch считал флажок `providerSessionStatus: "ready"` за "провайдер уже готов" и слал сообщение сразу в `adapter.sendMessage` без `resumeSession`. Для Claude `ClaudeSDKManager.sendMessage` бросал generic `Error("Session <id> not found")`; для Codex `turn/start` уходил в свежий app-server child, который не знал thread id. Classifier помечал оба случая как retryable, но retry для generic ошибок подписан не был — сообщение молча пропадало, `turn_state: "running"` в UI не приходил, JSONL не обновлялся. Теперь каждый provider adapter бросает typed `ClaudeSessionStaleBindingError` / `CodexSessionStaleBindingError` (симметрично `GeminiSessionStaleBindingError` из `1.2.8`), а Core dispatch detector обобщён на shared set провайдерных `code`-строк — one-shot `invalidateProviderBinding + ensureSessionReadyForSend + resend` recovery срабатывает для всех трёх провайдеров.
