...
## Current Release — v1.1.912
- **First-open `Diagram Modules` autolayout now measures ownership header starts in flow coordinates instead of zoom-scaled screen pixels**: the React Flow measurement bridge converts rendered header height back through the current viewport zoom before producing `bodyStartY`, so module cards no longer start inside `Product Part` or `Cluster` header text when the canvas is fit-scaled.
- **Initial `Diagram Modules` packing now respects overlapping horizontal footprints, not only exact `x` columns**: wide `Cluster` boxes and standalone `Module` cards inside the same `Product Part` are packed by real horizontal overlap, so ownership bottoms stay safe even when sibling nodes use different seed `x` values.
- **The released initial layout contract is now explicitly two-layered**: top clearance comes from zoom-safe measured header boundaries, while bottom clearance comes from overlap-aware sibling packing plus deepest-child container resize; persisted sidecar layouts still keep their conservative preserve path for manual compositions.

Previous releases (summary): `1.1.800–1.1.911` — standalone file-link query decode hotfixes, left-sidebar active-stage sync, temporary `Description`-first workspace startup, workflow-state startup SSOT alignment, Diagram Modules canonical English naming under localized prose, Codex raw-rollout dialog semantics, Codex empty-terminal answer recovery, the short-lived `Foundation Envelope` rollout later retired in `1.1.906`, the heuristic-only Diagram Modules boundary wave in `1.1.907`, the measured sibling-gap repair wave in `1.1.908`, the measured ownership reflow wave in `1.1.909`, the shared visual-bounds and manual-drag stabilization wave in `1.1.910`, the initial hierarchical packer split in `1.1.911`, and earlier localization/provider/release stabilization waves.
