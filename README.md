...
## Current Release — v1.1.481
- **Project Manager fix**: анкета описания остаётся редактируемой до отправки (при возврате в workspace не открывается как read-only markdown).
- **Release 1.1.481**: Built via `./scripts/build-all.sh` + `./scripts/build-release.sh --use-current-version`.

- **Artifact bundle**
  - VSIX: `codeai-hub-1.1.481.vsix`
  - Launcher: `CodeAIHubLauncher-macos-arm64-1.1.481.tar.bz2`
  - Core: `codeai-hub-core-darwin-arm64-1.1.481.tar.bz2`
  - Providers: `claude-module-1.1.481.tar.bz2`, `codex-module-1.1.481.tar.bz2`, `gemini-module-1.1.481.tar.bz2`
  - UI: `vscode-webview-1.1.481.tar.bz2`, `project-manager-1.1.481.tar.bz2`
