...
## Current Release — v1.1.440
- **Workflow file-first**: Core больше не применяет `outputSchema` для workflow стадий, артефакты пишутся через file-first поток.
- **Project Manager**: single-turn prompt pack (инструкция + анкета + шаблон + target path) и live workflow gating.
- **Workflow gates runner**: запуск гейтов по событию `workflow.stage.completed`.
- **Release 1.1.440**: Built via `./scripts/build-all.sh` + `./scripts/build-release.sh --use-current-version`.

- **Artifact bundle**
  - VSIX: `codeai-hub-1.1.440.vsix`
  - Launcher: `CodeAIHubLauncher-macos-arm64-1.1.440.tar.bz2`
  - Core: `codeai-hub-core-darwin-arm64-1.1.440.tar.bz2`
  - Providers: `claude-module-1.1.440.tar.bz2`, `codex-module-1.1.440.tar.bz2`, `gemini-module-1.1.440.tar.bz2`
  - UI: `vscode-webview-1.1.440.tar.bz2`, `web-client-1.1.440.tar.bz2`, `project-manager-1.1.440.tar.bz2`
