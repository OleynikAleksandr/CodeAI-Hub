...
- **Hotfix к релизу 1.2.40: Diagram Modules Artifacts panel теперь действительно помещается по горизонтали под auto-fit.** В `1.2.40` inner composition div получил `width: max-content + minWidth: 100%` в попытке выставить natural width для `scrollWidth` measurement. На практике intrinsic-sizing keyword позволил prose (purpose text, long titles) и `1fr` column tracks внутри ProductPart / Cluster / Module cards раскатываться в unwrappable single lines — natural width раздулась до тысяч пикселей, и auto-fit сваливался прямо в нижний порог `0.25`. На workspace пользователя композиция уходила за правую границу panel даже при Cmd+Ctrl+0 (100% user-zoom) и Cmd+scroll → 25%. Теперь inner div вернулся к естественному grid sizing: `scrollWidth` на нормально-сайзнутом grid уже возвращает `max(clientWidth, rightmost-child.right)`, что ровно то, что нужно auto-fit measurement'у, когда реальные card min-content (`minWidth: 220`, `minmax(240px, 1fr)`) выталкивают content за track. Source-level regression assertion инвертирован на `max-content === false`, чтобы keyword не вернулся молча.
