...
- **Codex app-server reasoning now materializes incrementally instead of landing as one late block.** The active Codex line emits readable `thinking` bubbles from real-time `item/reasoning/summaryTextDelta` and optional `item/reasoning/textDelta`, while `item/completed` is only a flush/fallback path for the unseen tail.
- **Codex transport diagnostics are restored in `~/.codeai-hub/logs/codex`.** The active app-server process now writes rotate-safe JSONL `sdk-codex-app-server-*.jsonl` files with JSON-RPC requests/responses/notifications, protocol records, stderr, and malformed stdout lines, alongside the existing session `description.jsonl` and provider-home rollout/history artifacts.
- **Codex module builds are now clean by default.** `packages/Codex_AppServer_Module` clears `dist/` before compile, so removed `*.test.*` outputs cannot leak into `codex-module-<version>.tar.bz2` or the packaged VSIX release line.
