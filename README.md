...
## Current Release — v1.1.870
- **Four-category localization model**: Settings now expose the approved categories `UI Labels`, `UI Helper Text`, `Messages for the User`, and `Artifacts for the User`, while clearing any selector returns the category to `Default Language (English)`.
- **Full user-facing text classification**: Settings shell text, Session feedback, Project Manager navigation/help, and questionnaire surfaces are now explicitly marked and routed through the new localization categories, so Russian materialization reaches the actual user-facing copy instead of legacy mixed buckets.
- **Artifact-language workflow boundary**: runtime prompt packs now thread `Artifacts for the User` language into Description, Virtual Simulation, and Diagram Modules outputs, while bundled prompts, appendices, and other internal agent instructions remain English-only.

Previous releases (summary): `1.1.800–1.1.869` — localization packaging hotfixes, host-hydrated browser localization runtime, searchable localization controls, shared Project Manager localization consumption, persistent localization foundation, thinking display sync, public CI bootstrap, staged core restart UX, Claude auth façade closure, Gemini final-answer deduplication, post-tool terminal-leg fix, adaptive post-tool watchdog, history-visible recoverable failure, architecture gate 500 lines, session-scoped Stop, provider rebind after Stop, provider-neutral applied turn config, PM label sync hardening, provider failure recovery, Gemini SDK 0.35.0 compatibility, detachable diagram window, layout/collision work, and earlier workflow/parser stabilization.
