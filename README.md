...
## Current Release — v1.1.774
- `Diagram Modules` now accepts the live outline continuation format for `product-parts/<part-id>.md` (`# Product Part: ...`, `## Purpose`, `## Cluster Inventory`, `## Direct Standalone Modules Under This Part`) instead of falling back to the old `# Module Inventory` parser and crashing on the first materialized part.
- The shared staged `Product Part` parser stays backward-compatible with the earlier table-based continuation format from `1.1.773`, so the retest fix does not break already-supported staged files while new outline files arrive from the live agent.
- Compatibility aggregate coverage now explicitly locks the outline path too: runtime must still emit a valid `module-inventory.md` from the same live continuation files that power the progressive graph.
