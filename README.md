...
## Current Release — v1.1.877
- **Gemini provider availability is restored for global `@google/gemini-cli@0.36.x` installs**: the runtime bridge now supports the bundle-only CLI layout and the relocated scheduler module instead of assuming the older `dist/src/config/*` package structure.
- **Gemini compatibility bootstrap is now safe inside Core**: provider startup reads `~/.gemini/settings.json` plus workspace `.gemini/settings.json` directly and avoids importing Gemini CLI bundle chunks that can register conflicting telemetry globals inside the host process.
- **Earlier Claude isolation and internal-prompt fixes remain in force**: Claude provider sessions still run in full SDK isolation mode, and packaged workflow prompts remain English-only for `Internal Agent Instructions`.
