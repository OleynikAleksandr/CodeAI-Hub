...
- **Codex runtime now ships through the new `codex app-server` module without changing the external provider contract.** Core still exposes the same `codexCli` provider id and the same provider slot under `~/.codeai-hub/providers/codex`, but the bundled/runtime path now resolves to `@codeai-hub/codex-app-server-module` instead of the legacy SDK-stream module.
- **Release packaging is aligned with the app-server cutover.** `build-core`, `build-codex-module`, `build-all`, and VSIX packaging now bundle the new Codex workspace package while preserving the existing `codex-module-<version>.tar.bz2` release artifact contract expected by manifests and installer paths.
- **Strict localization sync recovery from `1.2.21` remains part of the shipped line.** Marker-preserving runtime bundle materialization still retries isolated missing structured entries before Save fails, so the Codex transport switch does not roll back the localization hotfix.
