...
## Current Release — v1.2.4 (diagnostic)
- **Stop → Continue input lock — PM-side diagnostic trace**: the 1.2.3 trace on Claude Stop → Continue retest proved that Core emits `turn_state=running` correctly, first from `dispatchUserMessage` and again from `turn_started`, with no intervening `idle` before the reply stream. The real cause is on the PM side: after Stop the active `sessionId` in the UI state and the `sessionId` that Core binds the new turn to are not the same, so the UI continues reading `connectionState` from the old, now-dead session. 1.2.4 adds PM-side trace logs prefixed `pmdiag_` via the existing `logDiagnostic` transport, now routed to a dedicated file `~/.codeai-hub/logs/project-manager/project-manager.log` (override via `CODEAI_PROJECT_MANAGER_LOG_FILE`). Instrumentation covers: `pmdiag_api_stop_session` and `pmdiag_api_send_session_message` for every outbound click, `pmdiag_workspace_snapshot_apply` with a full per-session summary on each `workspace:snapshot` push, `pmdiag_active_session_changed` on every `setActiveSessionId` transition with a truncated call-site stack, and `pmdiag_dialog_active_session_changed` when the dialog controller swaps `session.id`. Core `stopdiag_` logs from 1.2.3 stay in place; both tiers are removed together in 1.2.5 once the fix lands.
- **Codex `Stop` observation (from 1.2.3 diag)**: on Codex, `adapter.closeSession` does not abort the active turn — it waits for natural `turn_completed`. Multiple Stop clicks stack in Core until turn completion, then all run at once. This is a separate baseline bug from the Claude Stop → Continue input lock; fix planned for 1.2.5 alongside PM input-panel debounce.

### 1.2.3 (diagnostic)
- Core-only `stopdiag_` trace logs on stop-action / stop-rebind / message-dispatch / `emitTurnStateEvent` (with caller stack) / provider-event-router in `~/.codeai-hub/logs/core/core.log`. Baseline verification for the Stop → Continue input lock regression; narrowed the root cause to the PM side (see 1.2.4 above).
