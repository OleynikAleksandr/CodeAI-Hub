...
- **Realtime session binding**: RemoteBridge теперь транслирует событие `session:binding`, поэтому webview моментально узнаёт реальный `sessionId` провайдера и реагирует на его смены без перезагрузки.
- **Info Panel insights**: верхняя панель сессии показывает статус привязки (`pending`, `ready`, `failed`) и полный `sessionId`, помогая отлаживать CLI во время разработки.
- **API consistency**: сериализация сессий в extension host и core всегда включает binding-метаданные, что синхронизирует статус во всех клиентах (VS Code, CEF, будущий PWA).
