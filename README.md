...
- **Development Tree sidebar теперь стабильно показывает правильную структуру для `diagram_modules` артефактов.** Раньше Core-side парсер `development-tree-snapshot.ts` вызывал `NEXT_SECTION_RE.exec(...)` напрямую на module-level `/g`-regex: `lastIndex` сохранялся между вызовами в long-lived Core процессе и давал чередование hit/null на одном и том же артефакте. Когда clamp слетал, строки `## Simple Relations` (`| from | to | type | label |`) попадали в standalone-скан, и в sidebar появлялись фантомные "standalone" модули (на реальном workspace user'а — 4 кластерных модуля в `From`-столбце). Любое expand/collapse кластера в sidebar триггерило `/workflow-state` refetch и перещёлкивало состояние. Парсер переписан: все `/g`-regex потребляются через `.matchAll()` (lastIndex-free iterator) или factory-функции, `NEXT_SECTION_SEARCH_RE` теперь non-global для `.search()`, а `MODULE_ROW_RE` ужесточён до строго 2-column — 4-колоночные Simple Relations rows физически не матчатся даже если clamp когда-либо опять соскользнёт.
- **Diagram Modules Artifacts panel теперь авто-подгоняется по ширине окна.** Раньше `DiagramEditorFacade` применял `transform: scale(${zoom})` только по Cmd/Ctrl+scroll пользователя — при уменьшении ширины Artifacts panel composition product parts уходила за правую границу container'а. Теперь effective render scale = `autoFitScale × userZoom`, где `autoFitScale = min(1, containerWidth / naturalWidth)` через `ResizeObserver` + natural `scrollWidth` композиции, с нижним порогом `0.25`. Manual Cmd/Ctrl+scroll становится overlay поверх auto-fit базы, а Cmd/Ctrl+0 сбрасывает только user overlay без поломки auto-fit. Внутренняя композиция выставляет `width: max-content + min-width: 100%`, чтобы `scrollWidth` возвращал intrinsic natural size независимо от transform.
