...
- **Separate scope still open** — the Codex `adapter.closeSession` observation from the 1.2.3 trace (close does not abort the active turn; Stop clicks stack until `turn_completed`) is a distinct baseline bug. Fix planned for 1.2.6 together with a PM input-panel Stop-button debounce and a Gemini Stop → Continue retest.
