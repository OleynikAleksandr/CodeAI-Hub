...
## Current Release — v1.1.882
- **Cold-start localization now boots from persistent user-space snapshots**: both `Settings` WebView and the standalone `Project Manager` preload the last resolved localization runtime snapshot instead of painting English fallbacks first.
- **Settings startup localization is injected before JavaScript boot**: the extension host now embeds the persisted browser bootstrap payload into the generated webview HTML, so localized UI/help text is available on the very first Settings paint after restart.
- **Project Manager now preloads localization before `root.render(...)`**: PM fetches `/api/v1/localization/bootstrap` from Core during startup, which removes the visible English-to-target-language transition in Help and other localized surfaces on cold launch.

Previous releases (summary): `1.1.800–1.1.881` — four-category localization release, localization packaging hotfixes, host-hydrated browser localization runtime, searchable localization controls, shared Project Manager localization consumption, persistent localization foundation, thinking display sync, public CI bootstrap, staged core restart UX, Claude auth façade closure, Gemini final-answer deduplication, post-tool terminal-leg fix, adaptive post-tool watchdog, history-visible recoverable failure, architecture gate 500 lines, session-scoped Stop, provider rebind after Stop, provider-neutral applied turn config, PM label sync hardening, provider failure recovery, Gemini SDK 0.35.0 compatibility, detachable diagram window, layout/collision work, glossary-file editing, localization closeout, and earlier workflow/parser stabilization.
