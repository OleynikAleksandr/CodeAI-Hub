...
## Current Release — v1.1.896
- **`Foundation Envelope` cold-start restore now reuses the real dialog history**: after restart, PM/Core no longer create a fresh empty dialog for the already existing `foundation_envelope` provider session, so the user sees the persisted conversation instead of `No messages yet.`.
- **Duplicate continuity roots are now resolved safely**: if an older history-backed dialog and a newer empty duplicate both exist for the same `Foundation Envelope` provider session, restore now prefers the history-backed dialog rather than the newer stale duplicate.
- **Workflow new-step guardrails now cover this exact regression class**: rollout SSOT explicitly forbids local stage-normalizer drift and requires duplicate-root/history-backed continuity restore tests for every new workflow step.
