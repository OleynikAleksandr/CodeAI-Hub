...
## Current Release — v1.1.365
- **Idea Collector output rendering**: the UI now surfaces `suggested_response` from structured outputs so interviews appear as expected.
...
- **Release 1.1.365**: Built via `./scripts/build-all.sh` + `./scripts/build-release.sh --use-current-version` for the Idea Collector pipeline.

- **Artifact bundle**
  - VSIX: `codeai-hub-1.1.365.vsix`
  - Launcher: `CodeAIHubLauncher-macos-arm64-1.1.365.tar.bz2`
  - Core: `codeai-hub-core-darwin-arm64-1.1.365.tar.bz2`
  - Providers: `claude-module-1.1.365.tar.bz2`, `codex-module-1.1.365.tar.bz2`, `gemini-module-1.1.365.tar.bz2`
  - UI: `vscode-webview-1.1.365.tar.bz2`, `web-client-1.1.365.tar.bz2`, `project-manager-1.1.365.tar.bz2`
