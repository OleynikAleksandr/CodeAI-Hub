...
Merged reasoning bubbles now keep untranslated English source out of the visible
projection for as long as any merged segment is still pending translation. When
...
Before each live translation request, Core reads the current user
`do-not-translate-terms.txt` glossary and the bundled global preserve glossary,
replaces matched terms with `[[CAIHUB_TERM_N]]` markers, sends the protected
text to Gemini, then restores the original terms after the model responds. The
translation prompt also says that glossary terms are a non-exhaustive priority
reminder, so Gemini must still preserve other product, workflow, role, mode,
setting, command, file, provider, model, API, code, project, and technical
terms.

Retest focus: install the VSIX, select `OpenRouter · Gemini 2.5 Flash Lite` as
the Reasoning Translation Engine, add terms such as `shell`,
`description agent`, and `workflow` to the editable glossary, trigger a new GLM
reasoning translation, and confirm those terms remain in English without
rebuilding after the glossary edit.

**Previous Release — v1.2.607** (Stable Gemini Flash Lite Localization Engine)
...
OpenRouter turns now receive the CodeAI Hub Codex workflow system profile and
the executable local workflow tool surface reused from GLM Native. Tool calls
streamed by OpenRouter models are executed locally and fed back into the next
Chat Completions request. OpenRouter is also available in workflow provider
selection, including Quality Gates.

Retest focus: install the VSIX, restart Project Manager, keep the OpenRouter
API key/model configured, start a standalone OpenRouter chat with a free
chat/instruct model, then start a workflow/provider path that needs tools and
confirm OpenRouter can read/use local context instead of behaving like bare
chat only.

**Previous Release — v1.2.600** (OpenRouter Stream + Picker Fix)

This release packages the OpenRouter follow-up fixes from live retesting.

OpenRouter reasoning chunks are now coalesced before they enter the thinking
message pipeline, so token-level reasoning deltas no longer render as
half-words on separate lines or trigger per-token translations. The New chat
provider picker also lists OpenRouter only once.

Retest focus: install the VSIX, restart Project Manager, start a standalone
OpenRouter chat with a general chat/instruct model, and confirm reasoning is
displayed as a readable thinking block while the assistant answer still streams.
Open the New chat provider picker and confirm OpenRouter appears once.

**Previous Release — v1.2.599** (OpenRouter Model Retest Build)
...
OpenRouter now appears in the Description provider picker, and standalone
OpenRouter chat turns receive the API key/base URL saved in Settings. The
runtime keeps those credentials out of the public applied-turn config while
still using them for the OpenRouter request.

Retest focus: install the VSIX, restart Project Manager, confirm OpenRouter is
visible in the Description provider picker, enter an OpenRouter API key, select
an exact model slug and endpoint row, then start a standalone OpenRouter chat.
The turn should stream instead of reporting `OpenRouter API key is not
configured`.

**Previous Release — v1.2.597** (OpenRouter Chat Provider)

This release packages the OpenRouter standalone chat provider.

Standalone chat now supports OpenRouter as a separate provider with Settings
API key storage, live OpenRouter model slug search, exact model selection, and
optional endpoint-tag routing shown as `Provider - endpointTag`. The provider
is available for general chat/review/discussion sessions only; managed
workflow stages keep their existing provider lists.

Retest focus: install the VSIX, restart Project Manager, enter an OpenRouter
API key, paste/search an exact model slug such as `openai/gpt-5-nano`, select
one endpoint row when available, start a standalone OpenRouter chat, and
confirm the lower status panel shows the selected slug/endpoint while the turn
streams.

**Previous Release — v1.2.596** (Local Models Runtime Load Fix)
...
Standalone Local Models chats now seed session creation with the concrete
LM Studio model selected from Settings/start-card discovery instead of the
legacy `local-model` fallback. This prevents new chats from binding to
`local-model` when LM Studio only exposes real ids such as
`google/gemma-4-e4b-qat` or `qwen/qwen3.6-27b`.

Retest focus: install the VSIX, restart the app, choose a non-first LM Studio
model for Local Models, open a new standalone Local Models chat, confirm the
lower status panel shows that exact model, and confirm the next turn runs it.

**Previous Release — v1.2.593** (Local Models Selection Fix)

This release packages the Local Models standalone chat model-selection fix.

Local Models turns now honor the selected LM Studio model id, including
`lmstudio:<modelKey>` ids, and fail clearly when an explicit selected model is
not available instead of silently running the first discovered local model. The
lower status panel now renders the Local Models selected model identity.

Retest focus: install the VSIX, restart the app, choose a non-first LM Studio
model for Local Models, open a standalone Local Models chat, confirm the lower
status panel shows that model, and confirm the next turn runs that model.

**Previous Release — v1.2.592** (Kimi Session Routing Fix)
...
Kimi sessions now publish a post-turn context-window token snapshot from the
native Kimi session `wire.jsonl` when ACP omits `usage_update`, so the lower
`Токены:` chip can show non-zero usage after a turn. Kimi 5h/Weekly quota rows
remain separate.

Retest focus: install the VSIX, restart the app, open a Kimi session, send a
message, and confirm the lower status panel shows non-zero `Токены:` usage.

**Previous Release — v1.2.590** (Standalone Chat Typography Fix)
...
GLM `edit_file_by_anchor` now exposes a schema that clearly requires string
anchors from `read_file_anchored`, not line numbers. `browser_fetch` now
detects the standard macOS Google Chrome executable path and supports
...
the integrated tool smoke-test and confirm `edit_file_by_anchor` uses string
anchors cleanly and `browser_fetch` returns rendered text through Chrome.

**Previous Release — v1.2.587** (GLM Expanded Tools Retest)

This release packages the expanded GLM Native standalone tool surface.

GLM now receives executable tools for anchored file reads and edits, exact
string edits, best-effort code symbol navigation, rendered browser fetch,
structured Git wrappers, and structured test command results. The GLM tooling
SSOT in `doc/SolidWorks-WorkFlow/Modules/GLM_Native.md` documents the current
tool contract. Kimi keeps its native ACP tool surface; CodeAI-owned expanded
tools need a separate ACP/MCP bridge before they can execute there.

Retest focus: install the VSIX, restart the app, open standalone GLM, ask it
to smoke-test every listed tool, and confirm anchored edits, Git wrappers,
browser fetch, and code navigation return explicit useful results or explicit
limitations.

**Previous Release — v1.2.586** (GLM Tool Reliability Retest)
...
GLM standalone chats now receive provider-owned function tools for shell
commands, workspace search, file read/write, patch application, web search,
web fetch, and workflow artifact writes. The GLM system prompt no longer
describes those tools as Codex-native, and the old copied Codex baseline was
removed from the GLM module.

Retest focus: install the VSIX, restart the app, open standalone GLM, ask it
to inspect the workspace and search the web, and confirm it uses executable
GLM tools instead of reporting missing Codex/web/browser tools.

**Previous Release — v1.2.584** (GLM EPIPE Retry Test)
