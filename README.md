...
- **Session model label (Claude)**: alias `default` в Session UI отображается как `Sonnet` (а не `Default`).
...
- **Release packaging**: собран и проверен артефакт `codeai-hub-1.1.562.vsix` вместе с обновлёнными tarball в `~/.codeai-hub/releases/`.
