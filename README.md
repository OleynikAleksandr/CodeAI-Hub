...
- **Reopened workflow dialog больше не залипает в `Agents is working, please wait...` после cold-start Core.** Раньше Core на cold-start материализовал runtime session только для `lastActive` stage — остальные reopened workflow dialogs (например `virtual_simulation` / `diagram_modules`) оставались без записи в `workspace:snapshot`. PM `createInitialSnapshot` для workflow session стартовал с `connectionState: "running"` и никогда не разблокировался, потому что snapshot update для отсутствующей runtime session не приходил. Теперь `RemoteBridgeDialogCommandRouter.handleDialogList` через новый `materializeContinuityEntries` helper создаёт stub runtime session для каждой continuity entry с `turnState: "idle"` / `continuityLockActive: false` / `bindingStatus: "ready"`, и PM reconciliation автоматически переводит UI в unlocked.
- **Stop-кнопка на reopened workflow dialog теперь работает.** Симметричный симптом той же асимметрии: `SessionRequestHandlerStopAction.handleStop` возвращал `"Session not found"` без эмиссии `turn_state: "idle"`, потому что `sessionManager.getSession(sessionId)` был undefined. После materializer session и paper-binding существуют в Core с первого `dialog:list`, Stop корректно инвалидирует binding и разблокирует UI.
- **External Codex contract не меняется.** Materializer работает целиком внутри Core; `thread/resume` остаётся ленивым (стартует на первом user message через existing `resolveProviderSessionId`), provider adapter turn в момент materialize не запускается. Codex app-server `closeSession` безопасен для stub — method только `turn/interrupt`-ит active turn (которого у stub нет) и удаляет entry из internal Map.
