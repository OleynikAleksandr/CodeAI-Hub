...
### Fixed
- **Workspace auto-attach triggers**: корректно распознаются русские триггеры и пути в конце строки; теперь можно указать пути и написать «прочитай/изучи/ознакомься» в любой части сообщения (без `/read`).
