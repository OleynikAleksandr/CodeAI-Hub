...
### Fixed
- **Codex turn timeout**: прерывание зависших ответов при отсутствии событий > 180s, чтобы UI не висел без ответа.
...
### Fixed
- **Core**: `session:create` выбирает активный провайдер, чтобы анкета Description запускала Idea Collector при неактивном CLI по умолчанию.
...
### Changed
- **Project Manager**: увеличены треугольники дерева, выровнены оси маркеров (triangle/dot) и уменьшен шаг смещения уровней.
...
### Changed
- **Release rebuild**: refreshed artifacts after the auto-run + run-aware path updates (no code changes).
...
### Fixed
- **Workspace auto-attach triggers**: корректно распознаются русские триггеры и пути в конце строки; теперь можно указать относительные пути к файлам в workspace и написать «прочитай/изучи/ознакомься» в любой части сообщения/на любой строке (без `/read`), до 3 файлов и 60KB/файл (только текстовые расширения).

## [1.1.376] - 2026-01-02

### Added
- **Workspace auto-attach**: Core автоматически прикрепляет содержимое файлов workspace по явным триггерам (например, «прочитай: …») без команды `/read` (с лимитами по файлам/размеру).
